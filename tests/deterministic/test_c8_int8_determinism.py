...
"""
Test suite for the c8 (cache_int8) decoder attention kernel determinism.

Background:
  The c8 kernel (multiquery_attention_c8_impl.cuh) uses force_no_partition
  (line 1238) to ensure deterministic floating-point accumulation when
  FD_DETERMINISTIC_MODE=1. This test verifies that the nosplit path
  is correctly dispatched for the INT8 quantized KV cache variant.

How the c8 INT8 path is triggered:
  - dim_head=128, blocksize=64, cache_quant_type="cache_int8" -> selects c8 kernel (IsFP8=false)
...
Run:
  python -m pytest tests/deterministic/test_c8_int8_determinism.py -v
"""
...
def run_c8_int8_decoder_test(
    ...
    """
    Run encoder prefill + N decoder runs with cache_int8 quantized KV cache.
    ...
    # INT8 quantized KV cache: uint8 dtype, same shape as c16
    cache_k = paddle.zeros((max_block_num, kv_num_head, blocksize, dim_head), dtype="uint8")
    cache_v = paddle.zeros((max_block_num, kv_num_head, blocksize, dim_head), dtype="uint8")

    # Scale tensors: channel-wise, shape [kv_num_head * dim_head]
    # Using scale=1.0 (identity) for simplicity; determinism check doesn't require meaningful values
...
class TestC8Int8Determinism(unittest.TestCase):
    """
    Test the c8 INT8 decoder kernel under FD_DETERMINISTIC_MODE=1.
    ...
    def test_short_kv_nosplit(self):
        ...
        results = run_c8_int8_decoder_test(
            batch_size=1,
            q_num_head=16,
            kv_num_head=2,
            dim_head=128,
