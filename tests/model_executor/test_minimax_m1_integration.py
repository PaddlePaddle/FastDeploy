"""
MiniMax-M1 integration tests — gated by environment variables.

These tests require multiple GPUs and model weights. They are SKIPPED by default
and intended for Baidu's multi-GPU CI runners or manual validation.

Gate variables:
    FD_TEST_MINIMAX_M1=1       Enable these tests (required)
...
CI Job:    NOT for GPU-h1z1-2Cards (only 2 cards). Needs dedicated multi-GPU runner.
"""
...
MINIMAX_M1_ENABLED = os.environ.get("FD_TEST_MINIMAX_M1", "0") == "1"
MODEL_PATH = os.environ.get("MINIMAX_M1_MODEL_PATH", "MiniMax/MiniMax-M1")
PORT = int(os.environ.get("MINIMAX_M1_PORT", "8190"))

pytestmark = pytest.mark.skipif(
    not MINIMAX_M1_ENABLED,
    reason="MiniMax-M1 integration tests disabled (set FD_TEST_MINIMAX_M1=1 to enable)",
)
