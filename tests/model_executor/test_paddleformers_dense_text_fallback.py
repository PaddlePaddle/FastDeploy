...
def mock_distributed_layers(monkeypatch):
    ...
    monkeypatch.setattr(
        "fastdeploy.model_executor.models.paddleformers.base.ColumnParallelLinear",
        MockLinearLayer
    )
    monkeypatch.setattr(
        "fastdeploy.model_executor.models.paddleformers.base.RowParallelLinear",
        MockLinearLayer
    )
    monkeypatch.setattr(
        "fastdeploy.model_executor.models.paddleformers.base.MergedColumnParallelLinear",
        MockLinearLayer
    )
    monkeypatch.setattr(
        "fastdeploy.model_executor.models.paddleformers.base.ReplicatedLinear",
        MockLinearLayer
    )
    ...
    monkeypatch.setattr(
        "fastdeploy.model_executor.models.paddleformers.base.Attention",
        lambda *args, **kwargs: mock_attention
    )
    
    # Also mock ParallelLMHead on causallm module (it imports it separately)
    monkeypatch.setattr(
        "fastdeploy.model_executor.models.paddleformers.causallm.ParallelLMHead",
        MockLMHead
    )
...
def mock_paddleformers(monkeypatch):
    ...
    monkeypatch.setattr(
        "paddleformers.transformers.AutoConfig.from_pretrained",
        lambda model, **kwargs: mock_config
    )
    ...
    monkeypatch.setattr(
        "paddleformers.transformers.AutoModel.from_config",
        lambda config, **kwargs: mock_model
    )
...
class TestPaddleFormersRegistration:
    ...
    def test_model_registered_in_registry(self):
        ...
        assert "PaddleFormersForCausalLM" in supported, \
            f"PaddleFormersForCausalLM not found in: {supported}"
    ...
    def test_fallback_resolution_auto_mode(self):
        ...
        backend = registry._try_resolve_paddleformers(
            "LlamaForCausalLM", mock_model_config, is_fallback=True
        )
    ...
    def test_explicit_paddleformers_mode(self):
        ...
        backend = registry._try_resolve_paddleformers(
            "Qwen2ForCausalLM", mock_model_config, is_fallback=False
        )
        assert backend == "PaddleFormersForCausalLM"
...
class TestModelInitialization:
    ...
    def test_model_init_with_mocked_layers(
        self, mock_distributed_layers, mock_paddleformers, real_fd_config
    ):
        """Test PaddleFormersForCausalLM initialization without GPU."""
        from fastdeploy.model_executor.models.paddleformers import PaddleFormersForCausalLM
...
class TestLoadWeights:
    ...
    def test_load_weights_execution(
        self, mock_distributed_layers, mock_paddleformers, real_fd_config
    ):
        ...
        # Create mock weights
        mock_weights = [
            ("model.layers.0.weight", paddle.randn([100, 100]))
        ]
...
class TestForwardMethod:
    ...
    def test_forward_with_mock_inputs(
        self, mock_distributed_layers, mock_paddleformers, real_fd_config
    ):
...
class TestAttentionInstances:
    ...
    def test_attention_instances_created_per_layer(
        self, mock_distributed_layers, mock_paddleformers, real_fd_config
    ):
