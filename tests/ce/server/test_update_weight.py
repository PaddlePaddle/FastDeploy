...
def do_completions(req_id_holder, result_holder, stream=True):
    ...
    try:
        resp = requests.post(
            f"{BASE_URL}/v1/completions", json=payload, stream=stream, timeout=120
        )
        if stream:
...
def do_chat_completions(req_id_holder, result_holder, stream=True, min_tokens=1800, max_tokens=None):
    ...
    try:
        resp = requests.post(
            f"{BASE_URL}/v1/chat/completions", json=payload, stream=stream, timeout=120
        )
        if stream:
            for line in resp.iter_lines():
                if not line:
...
def test_abort_partial_requests():
    ...
    for i in running_indices[:abort_count]:
        ...
        assert finish_reason == "abort", \
            f"请求 {req_id} 被 abort 但 finish_reason={finish_reason}, 期望 abort"
    ...
    assert blocks_before == blocks_after, \
        f"Block 数不一致: before={blocks_before}, after={blocks_after}"
...
def test_abort_completions_return_tokens():
    ...
    for i in range(n):
        t = threading.Thread(
            target=do_completions, args=(req_id_holders[i], result_holders[i], True)
        )
        t.start()
        threads.append(t)
    ...
    for i in range(n):
        ...
        if result_holders[i].get("finish_reason") != "abort":
            continue
        assert len(text) > 0, \
            f"请求 {req_id_holders[i].get('id')} abort 后返回的 text 为空"
        assert token_count > 0, \
            f"请求 {req_id_holders[i].get('id')} abort 后 token_count=0"
        print(f"test_abort_completions_return_tokens: 请求 {req_id_holders[i].get('id')} "
...
def test_abort_non_stream_completions_return_tokens():
    ...
    for i in range(n):
        ...
        text = result_holders[i].get("text", "")
        assert len(text) > 0, \
            f"请求 {req_id} abort 后返回的 text 为空"
        print(f"test_abort_non_stream_completions_return_tokens: 请求 {req_id} "
              f"text 长度={len(text)}")
...
def test_abort_non_stream_chat_completions_return_tokens():
    ...
    for i in range(n):
        ...
        assert len(content) > 0, \
            f"请求 {req_id} abort 后返回的 content 为空"
        print(f"test_abort_non_stream_chat_completions_return_tokens: 请求 {req_id} "
