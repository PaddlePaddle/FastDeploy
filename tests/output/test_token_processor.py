...
class TestTokenProcessorLogprobs(unittest.TestCase):
    ...
    def test_process_pooling_request(self):
        """Test processing pooling request"""
        self.task_mock.pooling_params = MagicMock()
        stream_data = MagicMock()
        stream_data.pooler_output = np.array([0.1, 0.2, 0.3])
        stream_data.batch_id = 0

        result = self.processor._process_batch_output_use_zmq([stream_data])

        self.assertEqual(len(result), 1)
        self.assertTrue(result[0].finished)
        self.assertEqual(result[0].outputs.data, [0.1, 0.2, 0.3])
