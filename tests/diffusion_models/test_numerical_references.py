...
def _sigma_schedule_numpy(num_steps: int, shift: float = 1.0) -> np.ndarray:
    """Pure NumPy: compute flow matching sigma schedule.

    sigmas = linspace(1, 0, num_steps+1)
    if shift != 1: sigmas = shift * s / (1 + (shift-1) * s)
    """
