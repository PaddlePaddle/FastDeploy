...
video_url_1 = "https://bj.bcebos.com/v1/paddlenlp/datasets/paddlemix/demo_video/example_video.mp4"
video_url_2 = "https://paddle-qa.bj.bcebos.com/FastDeploy/example.mpg"
        ...
        "messages": [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "视频内容是什么?分别简短回复我"},
                ],
            },
            {"role": "assistant", "content": "我不清楚"},
            {"role": "user", "content": "你再看看"},
...
def test_temp_with_top_p(t, p):
    ...
    # 语义稳定性断言
    # =========================
    weak_semantic_keywords = ["无法", "不能确定", "难以判断", "可能"]
    dynasty_keywords = [
        "静态",
        "物品",
        "桌",
...
# ===================== stop =====================
@pytest.mark.parametrize("stop", ["的", ["，", "有", "在"], ["一"]])
...
def test_metadata():
    ...
    j = send_request(URL, data).json()
    # assert len(j["choices"][0]["message"]["content"]) >=1500
    assert j["usage"]["completion_tokens"] >= 1500
...
def test_empty_messages():
    data = {
        ...
        "messages": [
            {
                "role": "user",
                "content": [{"type": "video_url", "video_url": {"url": ""}}, {"type": "text", "text": ""}],
            }
        ],
    }
    j = send_request(URL, data).json()
...
def test_stream_meta():
    ...
    chunks, text, _, _ = send_request_stream(URL, data)
    print(chunks)
    assert chunks[-1]["usage"]["completion_tokens"] > data["min_tokens"]
