...
if TEST_MODE == 'standalone':
    ...
    spec = importlib.util.spec_from_file_location(
        "multimodal_utils",
        os.path.join(os.path.dirname(__file__), '../../fastdeploy/multimodal/utils.py')
    )
...
class TestProcessTransparency(unittest.TestCase):
    ...
    def test_process_transparency_with_exif_transpose(self, mock_exif_transpose):
        """Test that EXIF orientation is corrected."""
        # Mock exif_transpose to return the same image
        mock_exif_transpose.return_value = self.rgb_image

        result = process_transparency(self.rgb_image)

        # Verify exif_transpose was called
        mock_exif_transpose.assert_called_once_with(self.rgb_image)

    def test_process_transparency_logs_transparent_background(self):
        """Test that transparent background detection is logged."""
        if TEST_MODE != 'standalone':
        ...
        mock_logger.info.assert_called_once_with("Image has transparent background, adding white background.")

    def test_process_transparency_no_log_for_opaque(self):
        """Test that opaque images don't trigger transparency log."""
        if TEST_MODE != 'standalone':
            self.skipTest("Logger mocking only available in standalone mode")
        ...
        # Verify logger was not called for opaque image
