...
"""Unit tests for Hackathon 10th Spring No.39."""
...
class _GpuOpsStub(types.ModuleType):
    """Catch-all module: returns registered sub-modules or raises."""
...
if _NEED_STUB:
    ...
    # Real GPU ops available — alias stubs to real modules so per-test
    # ``patch.object(_gpu_ops_stub, ...)`` targets the actual objects ``mb``
    # holds references to, instead of an unused stub instance.
    _gpu_ops_stub = sys.modules[_GPU_OPS]
    _deep_gemm_stub = sys.modules.get(_DEEP_GEMM) or types.ModuleType(_DEEP_GEMM)
...
def _mock_gpu_ops(extra_modules=None):
    """Patch sys.modules + the common GPU/Triton entry points used by apply().

    Per-test code only needs to add its own specialised patches (e.g.
    ``moe_topk_select``) inside the ``with`` block.
    """
...
class TestMarlinWeightOnlyMoEMethod:
    ...
    def test_apply_topk(self):
        """apply() with default topk_method='topk' after full weight processing."""
        layer = _DummyLayer()
        m = _init(layer)
    ...
    def test_apply_noaux_tc(self):
        """apply() with topk_method='noaux_tc' after full weight processing."""
        layer = _DummyLayer()
        ...
        with _mock_gpu_ops(extra_modules={_MOE_MOD: _moe_stub}):
            m.process_loaded_weights(layer, dict(zip(("up", "down"), _make_weights(layer))))
            out = m.apply(layer, x, gate, topk_ids_hookfunc=lambda topk_ids: None)
        assert list(out.shape) == [2, 64]
