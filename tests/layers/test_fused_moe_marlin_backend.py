...
"""Tests for fused_moe_marlin_backend module — Hackathon 10th Spring No.39."""
...
class _GpuOpsStub(types.ModuleType):
    """Fallback module stub — resolves sub-modules or returns ``None``."""
...
class _StubFusedMoE(paddle.nn.Layer):
    """Lightweight FusedMoE stub for marlin weight-only method."""
...
def _build_expert_weights(layer):
    u = [
...
class TestScaleAndPermHelpers:
    """Scale permutations and MoE wrapper utilities."""
    ...
    def test_gptq_marlin_moe_repack(self):
        """Validate per-expert repack loop with a stubbed C++ op."""
    ...
    def test_marlin_moe_permute_scales(self):
        """Verify per-expert permutation is consistent with single-expert call."""
...
class TestMarlinMoEWeightOps:
    """Weight creation, processing, and inference application."""

    def test_create_and_process(self):
        """Validate create_weights followed by process_loaded_weights for shape and dtype."""
        layer = _StubFusedMoE()
        ...
        with (
            ...
            m.process_loaded_weights(layer, dict(zip(("up", "down"), _build_expert_weights(layer))))
    ...
    def test_apply_topk(self):
        """Validate apply() under the default topk routing method."""
        layer = _StubFusedMoE()
        m = _create_method(layer)
    ...
    def test_apply_noaux_tc(self):
        """Validate apply() under the noaux_tc routing method."""
        layer = _StubFusedMoE()
