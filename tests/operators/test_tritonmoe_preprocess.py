...
class TestTritonMoePreprocessEdgeCases(unittest.TestCase):
    ...
    def test_empty_topk_ids(self):
        ...
        print("[empty_topk_ids] GOLDEN:        num_tokens_post_pad=0")
...
class TestTritonMoePreprocessRealistic(unittest.TestCase):
    ...
    def test_deterministic_with_fixed_seed(self):
        ...
        s_match = paddle.equal_all(paddle.sort(s1[:valid]), paddle.sort(s2[:valid])).item()

        print(
            f"[deterministic] run1: num_tokens_post_pad={int(n1.item())}"
            f"  expert_ids[:5]={e1[:5].tolist()}"
            f"  sorted_ids[:5]={s1[:5].tolist()}"
        )
        print(
            f"[deterministic] run2: num_tokens_post_pad={int(n2.item())}"
