...
class TestNgramMatchMixed(unittest.TestCase):
    def setUp(self):
        ...
        self.draft_tokens_copy = paddle.zeros(
            shape=[self.max_bsz, self.max_draft_tokens + 1],
            dtype="int64",
        )
        self.seq_lens_this_time = paddle.full(shape=[self.max_bsz, 1], fill_value=-1, dtype="int32")
        self.seq_lens_this_time_copy = paddle.zeros(shape=[self.max_bsz, 1], dtype="int32")
        ...
        # 设置具体数据
        self.input_ids[:, :10] = paddle.to_tensor(np.arange(0, 10), dtype="int64")
        self.input_ids_len[:] = 10
        pre_ids_np = np.array([10, 9, 8, 7, 6, 10, 9, 8, 7], dtype="int64")
        self.pre_ids[:, : pre_ids_np.shape[0]] = paddle.to_tensor(pre_ids_np, dtype="int64")
        self.step_idx[:] = 8

        self.draft_token_num[:] = 5
        self.draft_tokens[:, :2] = paddle.to_tensor(np.array([8, 7]), dtype="int64")
    ...
    def test_ngram_match_mixed(self):
        hybrid_mtp_ngram(
            ...
            self.draft_tokens,
            self.draft_tokens_copy,
            self.seq_lens_this_time,
            self.seq_lens_this_time_copy,
        ...
        np.testing.assert_allclose(self.seq_lens_this_time.cpu().numpy(), self.ref_seq_lens_this_time)
        np.testing.assert_allclose(self.draft_tokens.cpu().numpy(), self.ref_draft_tokens)
