...
def get_qkv_and_qkv_concat_tensor(bs, q_num_head, kv_num_head, seq_len, head_dim, place, dtype):
    ...
    q = paddle.to_tensor(query, place=place, dtype=dtype, stop_gradient=False) / 10.0
    key = np.random.random([bs, kv_num_head, seq_len, head_dim])
    k = paddle.to_tensor(key, place=place, dtype=dtype, stop_gradient=False) / 10.0
    value = np.random.random([bs, kv_num_head, seq_len, head_dim])
    v = paddle.to_tensor(value, place=place, dtype=dtype, stop_gradient=False) / 10.0
...
class TestDecodeAppendAttention(unittest.TestCase):
    ...
    def prefill(self):
        ...
        qkv_np = np.random.random([token_num, (self.q_num_head + 2 * self.kv_num_head) * self.head_dim]) / 10.0
