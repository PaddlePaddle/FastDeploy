...
"""Benchmark script comparing append_attention vs decode_unified_attention (C8/FP8) performance.
...
Usage:
    python benchmark_decode_attention_c8.py                    # run all cases, both ops
    python benchmark_decode_attention_c8.py --op append        # run only append_attention
    python benchmark_decode_attention_c8.py --op decode        # run only decode_attention
    python benchmark_decode_attention_c8.py --case 0           # run only case index 0
"""
...
def make_cache_and_scales(case, block_tables):
    """Allocate zero-filled KV cache (uint8 for fp8) with dummy scales. No prefill needed."""
    ...
    cache_k = paddle.zeros(shape=cache_shape, dtype="uint8")
    cache_v = paddle.zeros(shape=cache_shape, dtype="uint8")

    # Dummy scales (all ones) - sufficient for benchmarking timing
    cache_scale_shape = (case.kv_num_head,)
    cache_k_scale = paddle.ones(shape=cache_scale_shape, dtype=case.dtype)
    cache_v_scale = paddle.ones(shape=cache_scale_shape, dtype=case.dtype)
    cache_k_out_scale = paddle.ones(shape=cache_scale_shape, dtype=case.dtype)
    cache_v_out_scale = paddle.ones(shape=cache_scale_shape, dtype=case.dtype)

    return cache_k, cache_v, cache_k_scale, cache_v_scale, cache_k_out_scale, cache_v_out_scale


def do_prefill(case, block_tables, rotary_embs, place):
    """Run prefill with append_attention and return cache_k, cache_v and scales after prefill."""
    _, k, v, enc_qkv = get_qkv_and_qkv_concat_tensor(
        case.batch_size,
        case.q_num_head,
        case.kv_num_head,
        case.seq_len,
    ...
    cache_k, cache_v, cache_k_scale, cache_v_scale, cache_k_out_scale, cache_v_out_scale = make_cache_and_scales(
        case, block_tables, k, v
    )
    ...
    append_attention_op(
        ...
        False,  # rope_3d
        case.max_model_len,
        case.quant_max_bound,
        case.quant_min_bound,
        -1,
        64,
        16,
        1024,
...
def run_decode_attention(
    ...
    out = decode_unified_attention(
        ...
        buffer["max_len_tensor_cpu"],
        None,  # attn_mask
        cache_k_scale,  # k_quant_scale
        cache_v_scale,  # v_quant_scale
        cache_k_out_scale,  # k_dequant_scale
        cache_v_out_scale,  # v_dequant_scale
        None,  # cache_k_zp
        None,  # cache_v_zp
        None,  # mask_offset
        None,  # sinks
        paddle.empty([dec_qkv.shape[0], case.q_num_head * case.head_dim], dtype=dec_qkv.dtype),  # fmha_out
        case.cache_quant_type,
        case.max_model_len,
        case.quant_max_bound,
        case.quant_min_bound,
...
def benchmark_case(case, op="both"):
    """Run a single case: use empty cache (no prefill) for fast benchmarking."""
    ...
    # Lightweight cache + scales (no prefill, dummy data)
    cache_k, cache_v, cache_k_scale, cache_v_scale, cache_k_out_scale, cache_v_out_scale = make_cache_and_scales(
        case, block_tables
    )
    ...
    if op in ("both", "decode"):
        # Need fresh cache for decode (append_attention above modified it)
        cache_k, cache_v, cache_k_scale, cache_v_scale, cache_k_out_scale, cache_v_out_scale = make_cache_and_scales(
            case, block_tables
        )
        paddle.device.cuda.synchronize()
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark append_attention vs decode_unified_attention (C8/FP8)")
    parser.add_argument(
