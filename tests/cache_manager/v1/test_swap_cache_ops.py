...
Unit tests for swap_cache_all_layers and swap_cache_all_layers_batch operators.
...
class TestSwapCacheAllLayersCorrectness(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.device_id = 0
        self.num_blocks = 32  # Number of blocks to transfer in each test
...
class TestSwapCacheAllLayersBatchCorrectness(unittest.TestCase):
    """Test correctness of swap_cache_all_layers_batch operator."""
    ...
    def test_h2d_transfer_correctness(self):
        """Test Host->Device (load) transfer correctness."""
        (
            gpu_k_tensors,
            gpu_v_tensors,
            k_ptrs,
        ...
        # Perform H2D transfer using batch operator
        swap_cache_all_layers_batch(
            gpu_k_tensors,
            k_ptrs,
            self.config.total_block_num,
        ...
        self.assertTrue(k_md5_ok, "K cache MD5 mismatch after H2D transfer (batch)")
        self.assertTrue(v_md5_ok, "V cache MD5 mismatch after H2D transfer (batch)")
        self.assertTrue(k_data_ok, "K cache data mismatch after H2D transfer (batch)")
        self.assertTrue(v_data_ok, "V cache data mismatch after H2D transfer (batch)")
    ...
    def test_d2h_transfer_correctness(self):
        ...
        # Verify data in CPU memory (use uint16 to match bfloat16 storage)
        bytes_per_layer = bytes_per_block * self.num_blocks
        k_md5_ok = True
        v_md5_ok = True
        ...
        self.assertTrue(k_md5_ok, "K cache MD5 mismatch after D2H transfer (batch)")
        self.assertTrue(v_md5_ok, "V cache MD5 mismatch after D2H transfer (batch)")
...
class TestSwapCacheAllLayersBatchPerformance(unittest.TestCase):
    """Test performance of swap_cache_all_layers_batch operator."""
    ...
    def test_h2d_bandwidth(self):
        """Test H2D transfer bandwidth for batch operator."""
        (
            gpu_k_tensors,
            gpu_v_tensors,
            k_ptrs,
        ...
        print("\n swap_cache_all_layers_batch H2D Performance:")
        print(f"  Data size: {total_bytes / (1024**3):.2f} GB")
        print(f"  Avg time: {avg_time:.2f} ms")
        print(f"  Bandwidth: {bandwidth_gbps:.2f} GB/s")
    ...
    def test_d2h_bandwidth(self):
        """Test D2H transfer bandwidth for batch operator."""
        (
            gpu_k_tensors,
            gpu_v_tensors,
            k_ptrs,
...
class TestSwapCacheComparison(unittest.TestCase):
    """Compare performance between swap_cache_all_layers and swap_cache_all_layers_batch."""
    ...
    def test_batch_vs_nonbatch_performance(self):
        """Compare batch operator vs non-batch operator."""
        (
            gpu_k_tensors,
            gpu_v_tensors,
            k_ptrs,
        ...
        # Benchmark non-batch
        avg_time_nonbatch, _ = benchmark_transfer(
            swap_cache_all_layers,
            gpu_k_tensors,
            gpu_v_tensors,
            k_ptrs,
        ...
        # Benchmark batch
        avg_time_batch, _ = benchmark_transfer(
            swap_cache_all_layers_batch,
            gpu_k_tensors,
            gpu_v_tensors,
            k_ptrs,
        ...
        bandwidth_nonbatch = (total_bytes / (1024**3)) / (avg_time_nonbatch / 1000)
        bandwidth_batch = (total_bytes / (1024**3)) / (avg_time_batch / 1000)
        speedup = avg_time_nonbatch / avg_time_batch

        print("\n Performance Comparison (H2D):")
        print(f"  Data size: {total_bytes / (1024**3):.2f} GB")
        print(f"  swap_cache_all_layers: {avg_time_nonbatch:.2f} ms ({bandwidth_nonbatch:.2f} GB/s)")
        print(f"  swap_cache_all_layers_batch: {avg_time_batch:.2f} ms ({bandwidth_batch:.2f} GB/s)")
        print(f"  Speedup: {speedup:.2f}x")

        # Performance comparison is informational; batch vs non-batch depends on workload
        # Batch is typically faster for many layers with larger transfer sizes
        # We only assert that both achieve reasonable bandwidth (> 1 GB/s)
        self.assertGreater(bandwidth_nonbatch, 1.0, "Non-batch operator bandwidth too low")
        self.assertGreater(bandwidth_batch, 1.0, "Batch operator bandwidth too low")


class TestSwapCacheAllLayersBatchMultiRound(unittest.TestCase):
    """Test swap_cache_all_layers_batch with multiple evict/load rounds."""
    ...
    def setUp(self):
        ...
        self.device_id = 0
        self.num_blocks = 32
        self.num_rounds = 5  # number of evict->load rounds

    def test_multi_round_swap_correctness(self):
        """
        Simulate multiple rounds of D2H (evict) + H2D (load) with random
        non-consecutive block IDs and random tensor values.

        Round flow:
          1. Initialize GPU with random data at random (non-consecutive) block positions.
          2. For each round:
             a. D2H: evict GPU -> CPU
             b. Zero out GPU tensors
             c. H2D: load CPU -> GPU
             d. Verify GPU data at gpu_block_ids matches original via MD5 + allclose
        """
        (
            gpu_k_tensors,
            gpu_v_tensors,
            ...
            self.config,
            self.num_blocks,
            use_random=True,  # random tensor values (not constant per layer)
            shuffle_blocks=True,  # non-consecutive block IDs
            seed=2025,
        )

        print(f"\ngpu_block_ids (sample): {gpu_block_ids[:8]}...")
        print(f"cpu_block_ids (sample): {cpu_block_ids[:8]}...")

        # Step 1: load initial data onto GPU (H2D)
        # max_block_num_cpu = self.num_blocks (CPU pinned memory holds exactly num_blocks slots)
        # max_block_num_gpu is derived internally from gpu tensor shape (total_block_num)
        swap_cache_all_layers_batch(
            gpu_k_tensors,
            k_ptrs,
            self.num_blocks,  # max_block_num_cpu
            gpu_block_ids,
            cpu_block_ids,
            self.device_id,
            mode=1,
        ...
        for round_idx in range(self.num_rounds):
            ...
            # Verify CPU memory MD5 matches original
            cpu_k_ok = True
            cpu_v_ok = True
            for layer_idx in range(self.config.num_layers):
                k_np = np.zeros(self.num_blocks * self.config.kv_cache_dim, dtype=np.uint16)
                ...
                ctypes.memmove(v_np.ctypes.data, v_ptrs[layer_idx], bytes_per_layer)
                k_np = k_np.reshape(
                    self.num_blocks, self.config.num_heads, self.config.block_size, self.config.head_dim
                )
                v_np = v_np.reshape(
                    self.num_blocks, self.config.num_heads, self.config.block_size, self.config.head_dim
                )
            ...
            self.assertTrue(cpu_k_ok, f"Round {round_idx+1}: K cache MD5 mismatch in CPU after D2H")
            self.assertTrue(cpu_v_ok, f"Round {round_idx+1}: V cache MD5 mismatch in CPU after D2H")
            print(f"  D2H (evict) CPU verify: K={'PASS' if cpu_k_ok else 'FAIL'}, V={'PASS' if cpu_v_ok else 'FAIL'}")

            # Step 2b: Zero out GPU tensors to ensure clean state
            for t in gpu_k_tensors + gpu_v_tensors:
                t.fill_(0)
            paddle.device.cuda.synchronize()

            # Step 2c: H2D load (CPU -> GPU)
            swap_cache_all_layers_batch(
                gpu_k_tensors,
                k_ptrs,
                self.num_blocks,  # max_block_num_cpu
            ...
            # Step 2d: Verify GPU data at gpu_block_ids matches source at cpu_block_ids
            k_md5_ok, k_data_ok = verify_transfer_correctness(
                gpu_k_tensors,
                src_k_data,
                [m[0] for m in md5_sums],
                self.num_blocks,
                self.config,
            ...
            v_md5_ok, v_data_ok = verify_transfer_correctness(
                gpu_v_tensors,
                src_v_data,
                [m[1] for m in md5_sums],
                self.num_blocks,
                self.config,
                gpu_block_ids=gpu_block_ids,
                src_block_ids=cpu_block_ids,
            )
            self.assertTrue(k_md5_ok, f"Round {round_idx+1}: K cache MD5 mismatch on GPU after H2D")
            self.assertTrue(v_md5_ok, f"Round {round_idx+1}: V cache MD5 mismatch on GPU after H2D")
            self.assertTrue(k_data_ok, f"Round {round_idx+1}: K cache data mismatch on GPU after H2D")
            self.assertTrue(v_data_ok, f"Round {round_idx+1}: V cache data mismatch on GPU after H2D")
            print(
                f"  H2D (load)  GPU verify: K={'PASS' if k_md5_ok and k_data_ok else 'FAIL'}, "
                f"V={'PASS' if v_md5_ok and v_data_ok else 'FAIL'}"
            )

        print(f"\nAll {self.num_rounds} rounds passed.")
