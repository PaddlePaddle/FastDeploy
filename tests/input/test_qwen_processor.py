...
from fastdeploy.input.image_processors.qwen_processor import (
    MAX_PIXELS,
    MIN_PIXELS,
    OPENAI_CLIP_MEAN,
    OPENAI_CLIP_STD,
...
class TestImageProcessorInit(unittest.TestCase):
    ...
    def test_default_params(self):
        """Default init sets expected Qwen VL attributes."""
        proc = ImageProcessor()
        self.assertEqual(proc.patch_size, 14)
        self.assertEqual(proc.merge_size, 2)
        self.assertEqual(proc.temporal_patch_size, 2)
        self.assertEqual(proc.min_pixels, MIN_PIXELS)
        self.assertEqual(proc.max_pixels, MAX_PIXELS)
        self.assertEqual(proc.image_mean, OPENAI_CLIP_MEAN)
        self.assertEqual(proc.image_std, OPENAI_CLIP_STD)
    ...
    def test_custom_params(self):
        ...
        self.assertEqual(proc.max_pixels, 8192)
...
class TestImageProcessorPreprocess(unittest.TestCase):
    ...
    def _make_rgb_image(self, h=56, w=56):
    ...
    def test_preprocess_single_image(self):
        ...
        img = self._make_rgb_image(56, 56)
    ...
    def test_preprocess_with_override_params(self):
        ...
        img = self._make_rgb_image(84, 84)
    ...
    def test_preprocess_do_rescale_only(self):
        ...
        img = self._make_rgb_image(28, 28)
    ...
    def test_preprocess_resize_needed(self):
        ...
        # Image not a multiple of patch_size * merge_size = 28
    ...
    def test_preprocess_multiple_frames(self):
        ...
        frames = [self._make_rgb_image(28, 28) for _ in range(4)]

        result = proc.preprocess(frames)

        grid_thw = result["grid_thw"]
        # grid_t = 4 / temporal_patch_size=2 = 2
    ...
    def test_preprocess_temporal_padding(self):
        ...
        # 3 frames - not divisible by temporal_patch_size=2, pads to 4
        frames = [self._make_rgb_image(28, 28) for _ in range(3)]
    ...
    def test_preprocess_already_scaled_warning(self):
        ...
        with patch("fastdeploy.input.image_processors.qwen_processor.data_processor_logger") as mock_logger:
            proc.preprocess(img)
...
class TestImageProcessorRegistration(unittest.TestCase):
    ...
    def test_registered_in_registry(self):
        """ImageProcessor is registered under QWEN_VL key."""
        ...
        processor_cls = ImageProcessorRegistry.get(QWEN_VL)
