  ...
  public class ConvertResult{
    ...
    public static FD_DetectionResult ConvertDetectionResultToCResult(DetectionResult detection_result){
      ...
      if (fd_detection_result.masks.size != 0)
      {
          ...
          {
              Marshal.StructureToPtr(masks[i], fd_detection_result.masks.data + i * Marshal.SizeOf(masks[0]), true);
          }
      }
    ...
    public static DetectionResult ConvertCResultToDetectionResult(FD_DetectionResult fd_detection_result){
      ...
      FD_OneDimArraySize [] boxes = new FD_OneDimArraySize[fd_detection_result.boxes.size];
                ...
                for (int i = 0; i < (int)fd_detection_result.boxes.size; i++) {
                    boxes[i] =(FD_OneDimArraySize)Marshal.PtrToStructure(fd_detection_result.boxes.data + i * Marshal.SizeOf(boxes[0]), typeof(FD_OneDimArraySize));
      ...
      for (int i = 0; i < (int)fd_detection_result.masks.size; i++) {
                    fd_masks[i] = (FD_Mask)Marshal.PtrToStructure(fd_detection_result.masks.data + i * Marshal.SizeOf(fd_masks[0]), typeof(FD_Mask));
