...
int main(int argc, char* argv[]) {
  ...
  if (config_info["precision_compare"] == "true") {
    ...
    model_ppyolov7.GetPreprocessor().Run(&fd_images, &input_tensors);
    input_tensors[0].name = "image";
    input_tensors[1].name = "scale_factor";
    input_tensors[2].name = "im_shape";
    input_tensors.pop_back();
    model_ppyolov7.Infer(input_tensors, &output_tensors);
    model_ppyolov7.GetPostprocessor().Run(output_tensors, &batch_res);
    // Save tensor to -> disk.
    auto& tensor_dump = output_tensors[0];
    std::string det_tensor_path = "ppyolov7_tensor.txt";
