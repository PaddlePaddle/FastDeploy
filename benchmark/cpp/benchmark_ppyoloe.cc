...
int main(int argc, char* argv[]) {
  ...
  option.mnn_option.out_orders = {{"tmp_17", 0}, {"concat_14.tmp_0", 1}};
  option.tnn_option.in_orders = {{"image", 0}, {"scale_factor", 1}};
  option.tnn_option.out_orders = {{"tmp_17", 0}, {"concat_14.tmp_0", 1}};
  option.ncnn_option.in_orders = {{"image", 0}, {"scale_factor", 1}};
  option.ncnn_option.out_orders = {{"tmp_17", 0}, {"concat_14.tmp_0", 1}};
  ...
  if (config_info["precision_compare"] == "true") {
    ...
    model_ppyoloe.GetPreprocessor().Run(&fd_images, &input_tensors);
    input_tensors[0].name = "image";
    input_tensors[1].name = "scale_factor";
    input_tensors[2].name = "im_shape";
    input_tensors.pop_back();
    model_ppyoloe.Infer(input_tensors, &output_tensors);
    model_ppyoloe.GetPostprocessor().Run(output_tensors, &batch_res);
    // Save tensor to -> disk.
    auto& tensor_dump = output_tensors[0];
    std::string det_tensor_path = "ppyoloe_tensor.txt";
