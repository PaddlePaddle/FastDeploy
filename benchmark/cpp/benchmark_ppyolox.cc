...
int main(int argc, char* argv[]) {
  ...
  if (config_info["backend"] == "mnn") {
    ...
    option.mnn_option.out_orders = {{"tmp_100", 0}, {"transpose_9.tmp_0", 1}};
    ...
    option.tnn_option.out_orders = {{"tmp_100", 0}, {"transpose_9.tmp_0", 1}};
  } else if (config_info["backend"] == "ncnn") {
    // WARN: PaddleYOLOX not support for NCNN now!
    ...
    option.ncnn_option.out_orders = {{"tmp_100", 0}, {"transpose_9.tmp_0", 1}};
  }
  ...
  if (config_info["precision_compare"] == "true") {
    ...
    model_ppyolox.GetPreprocessor().Run(&fd_images, &input_tensors);
    input_tensors[0].name = "image";
    input_tensors[1].name = "scale_factor";
    input_tensors[2].name = "im_shape";
    input_tensors.pop_back();
    model_ppyolox.Infer(input_tensors, &output_tensors);
    model_ppyolox.GetPostprocessor().Run(output_tensors, &batch_res);
    // Save tensor to -> disk.
    auto& tensor_dump = output_tensors[0];
    std::string det_tensor_path = "ppyolox_tensor.txt";
