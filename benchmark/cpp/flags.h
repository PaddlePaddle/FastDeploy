...
DEFINE_bool(use_fp16, false, "Force to override fp16 option in config file.");
    ...
    *model_format = fastdeploy::ModelFormat::MNN_MODEL;
    if (use_quant_model) {
      if (!GetModelResoucesNameFromDir(FLAGS_model, model_name, "quant.mnn")) {
        std::cout << "Can not find quant MNN model resources." << std::endl;
        return false;
      }
    } else if (!GetModelResoucesNameFromDir(FLAGS_model, model_name, "mnn")) {
      std::cout << "Can not find MNN model resources." << std::endl;
      return false;
    }
  } else if (config_info["backend"] == "tnn") {
    *model_format = fastdeploy::ModelFormat::TNN_MODEL;
    if (!(GetModelResoucesNameFromDir(FLAGS_model, model_name, "tnnmodel") &&
          GetModelResoucesNameFromDir(FLAGS_model, params_name, "tnnproto"))) {
      std::cout << "Can not find TNN model resources." << std::endl;
      return false;
    }
  } else if (config_info["backend"] == "ncnn") {
    *model_format = fastdeploy::ModelFormat::NCNN_MODEL;
    if (!(GetModelResoucesNameFromDir(FLAGS_model, model_name, "opt.bin") &&
          GetModelResoucesNameFromDir(FLAGS_model, params_name, "opt.param"))) {
