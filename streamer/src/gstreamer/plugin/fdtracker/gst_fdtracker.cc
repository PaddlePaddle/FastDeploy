...
gst_fdtracker_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  ...
  std::vector<Bbox_cache> cur_frame_bbox;
  std::vector<Bbox_cache> previous_frame;
  bool is_first_frame = false;

  NvDsBatchMeta *batch_meta = gst_buffer_get_nvds_batch_meta (buf);

  if(!fdtracker->previous_frame->empty()) {
    previous_frame = fdtracker->previous_frame->back();
  } else {
    is_first_frame = true;
  }

  if(fdtracker->previous_frame->size() >= 100) {
    fdtracker->previous_frame->pop();
  }
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    l_frame = l_frame->next) {
	  ...
	  tracker_bbox = obj_meta->tracker_bbox_info.org_bbox_coords;
//	  std::cout << "detector_bbox:\nleft " << detector_bbox.left << "\n"
//		    << "top " << detector_bbox.top << "\n" << "width "
//		    << detector_bbox.width << "\n" << "height "
//		    << detector_bbox.height << std::endl;
//        std::cout << "tracker id: " << obj_meta->object_id << std::endl;
//        std::cout << "tracker_bbox:\nleft " << tracker_bbox.left << "\n"
//                  << "top " << tracker_bbox.top << "\n" << "width "
//                  << tracker_bbox.width << "\n" << "height "
//                  << tracker_bbox.height << std::endl;

	  Bbox_cache cur_bbox = {detector_bbox.left, detector_bbox.top,
	                          detector_bbox.left+detector_bbox.width,
				  detector_bbox.top+detector_bbox.height};

	  float max_iou = 0.0;
	  Bbox_cache max_iou_bbox;
	  for(auto previous_bbox : previous_frame) {
	    float temp_iou = IOU(cur_bbox, previous_bbox);
	    if(temp_iou > max_iou) {
	      max_iou = temp_iou;
	      max_iou_bbox = previous_bbox;
	    }
	  }
	  if(max_iou < 0.7 || is_first_frame) {
	    cur_bbox.tracker_id = fdtracker->min_tracker_id++;
	  } else {
	    cur_bbox.tracker_id =  max_iou_bbox.tracker_id;
	  }
	  obj_meta->tracker_bbox_info.org_bbox_coords.left = cur_bbox.x1;
          obj_meta->tracker_bbox_info.org_bbox_coords.top = cur_bbox.y1;
          obj_meta->tracker_bbox_info.org_bbox_coords.width = cur_bbox.x2-cur_bbox.x1;
	  obj_meta->tracker_bbox_info.org_bbox_coords.height = cur_bbox.y2-cur_bbox.y1;
	  obj_meta->object_id = max_iou_bbox.tracker_id;
	  std::string tracker_id_string = " " + std::to_string(max_iou_bbox.tracker_id);
	  obj_meta->text_params.display_text = strcat(obj_meta->text_params.display_text, tracker_id_string.c_str());
          cur_frame_bbox.push_back(cur_bbox);
      }
  }
  fdtracker->previous_frame->push(cur_frame_bbox);
