...
// bool AddNvUriSrcBins(GstElement* pipeline, YAML::Node& properties) {
...
//   g_object_get_property(G_OBJECT(streammux),
//                         "nvbuf-memory-type",
//                         &value);
//   std::cout << "********* " << value.g_type << std::endl;
//   std::cout << "*********-- " << g_type_name(value.g_type) << std::endl;
//   return true;
// }
...
//     if (elem_name == "app") {
//       std::cout << "---------- type: " << elem.second["enable-perf-measurement"].Type() << std::endl;
//       auto enable_perf = elem.second["enable-perf-measurement"].as<bool>();
//       if (enable_perf) {
//         auto interval = elem.second["perf-measurement-interval-sec"].as<int>();
//       }
//     } else if (elem_name == "nvurisrcbin_list") {
//       AddNvUriSrcBins(pipeline, elem.second);
