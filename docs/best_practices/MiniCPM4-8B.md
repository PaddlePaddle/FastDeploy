...
**Tips:**
1. MiniCPM4.1-8B is a dense 8B parameter model — a single GPU suffices for inference at all supported quantization levels.
...
- `--max-model-len`: Maximum number of tokens for the deployed service. MiniCPM4.1 supports up to 32,768 tokens. Larger values increase GPU memory usage. (LongRoPE for extended context is not yet supported in FastDeploy.)
