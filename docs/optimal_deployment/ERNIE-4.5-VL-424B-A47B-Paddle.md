...
> **Input cache_kv ratio**
- **Parameters：**`--kv-cache-ratio`
- **Description：** Controls the allocation ratio of GPU memory for the KV cache. The default value is 0.75, meaning 75% of the KV cache memory is allocated to the input.
- **Recommendation：** The theoretical optimal value should be set to the average input length / (average input length + average output length) of the application scenario. If you cannot determine it, keeping the default value is fine.
...
###  3.2 Partial outputs are incomplete or empty
Check the logs for the following alerts:
> Message: 'recovery stop signal found at task chatcmpl-xx' \n Arguments: ('token_ids: [-3]',)

If the above log appears, it means a request triggered the recover stop return. Reducing the kv-cache-ratio can alleviate this issue.  

If extreme performance is not required, the following method can be used to ensure complete results and avoid the problem:  

      Configure --kv-cache-ratio=1.0 and --static-decode-blocks=max_output_length/64. For example, with a 128K output, configure --static-decode-blocks=2048.