...
```plain
Span: POST /v1/chat/completions              # 接口入口
│
├── Span: API Server(host: | pid:84037)
│   ├── Span: preprocess                     # 预处理
│   │   └── Span: encode                     # encode编码
│   │
│   └── Span: Insert Task to Scheduler(host: | pid:83851)
│       ├── Span: submit_to_infer_engine     # request-->engine通信
│       └── Span: insert_task_to_scheduler   # 任务分配
│
├── Span: Scheduler Task to Work(host: | pid:82760)
|   ├── Span: schedule_queue_wait            # 调度队列等待耗时
│   └── Span: schedule_dispatch              # 显存资源分配
│
├── Span: Token Processor(host: | pid:82764)
│   ├── Span: prefill                         # prefill阶段
│   └── Span: decode_loop                     # decode阶段
```

### 5.3 可用的 Span 名枚举（`TraceSpanName`）
