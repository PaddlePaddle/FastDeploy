...
| L2 | 请求/响应内容和调度 | 请求参数、处理后的请求、调度信息（入队/拉取/完成）、响应内容（超长内容会被截断） |
| L3 | 完整数据 | 完整的请求和响应数据、原始接收请求 |

默认级别为 L0，只记录关键事件。设置更高级别会包含更详细的信息，但也会增加日志量。
...
| `FD_LOG_REQUESTS_LEVEL` | `0` | 请求日志级别，范围 0-3 |
