# 10分钟完成 ERNIE-4.5-0.3B-PT 模型部署

本文档讲解如何部署ERNIE-4.5-0.3B-PT模型，在开始部署前，请确保你的硬件环境满足如下条件：
...
为了快速在各类硬件部署，本文档采用 ```ERNIE-4.5-0.3B-PT``` 模型作为示例，可在大部分硬件上完成部署。
...
>💡 注意：在 ```--model``` 指定的路径中，若当前目录下不存在该路径对应的子目录，则会尝试根据指定的模型名称（如 ```baidu/ERNIE-4.5-0.3B-PT```）查询AIStudio是否存在预置模型，若存在，则自动启动下载。默认的下载路径为：```~/xx```。关于模型自动下载的说明和配置参阅[模型下载](../supported_models.md)。
