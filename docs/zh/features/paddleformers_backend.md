...
**步骤 1**：按照 [NVIDIA GPU 安装指南](./../../get_started/installation/nvidia_gpu.md) 完成 FastDeploy 安装。

**步骤 2**：源码安装开发版 PaddleFormers（替换 FastDeploy 自带版本）：
...
# 如果出现包不存在的情况，可以通过 --index-url https://mirrors.ustc.edu.cn/pypi/simple 解决

# 根据 CUDA 版本选择（会自动安装 PaddlePaddle 和 paddlefleet）
# CUDA 12.6
