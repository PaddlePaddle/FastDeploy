...
在 2.2 版本之前，CUDAGraph 未默认开启，2.2 版本开始 Custom all-reduce 算子默认开启。
...
在2.2版本之前需要通过`--use-cudagraph`启用。2.2版本开始 CUDAGraph 已经默认开启，对于暂时不能兼容 CUDAGraph 的功能（投机解码、强化学习训练、多模模型推理）CUDAGraph 会自动关闭。也可以通过设置 `--graph-optimization-config` 手动控制 CUDAGraph。
