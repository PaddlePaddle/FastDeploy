...
**注：**
1. 在启动命令后指定`--tensor-parallel-size 8` 即可修改部署卡数
2. 表格中未列出的硬件，可根据显存大小进行预估是否可以部署
3. 量化精度推荐wint4(可以单机8卡部署)。
...
**启用方式：**
自2.2版本开始（包括develop分支），Prefix Caching已经默认开启。使用`--no-enable-prefix-caching`关闭前缀缓存。
...
**启用方式：**
自2.2版本开始（包括develop分支），Chunked Prefill已经默认开启。使用`export FD_DISABLE_CHUNKED_PREFILL=1`关闭Chunked Prefill。
