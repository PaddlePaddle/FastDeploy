...
For PD (Prefill-Decode) disaggregated deployment, FastDeploy provides a high-performance Golang-based [Router](https://github.com/PaddlePaddle/FastDeploy/tree/develop/fastdeploy/golang_router) to implement request reception and scheduling. The usage method and scheduling flow are as follows:
...
A high-performance version of the Router is now available as a Golang binary bundled in the FastDeploy Python package. See the [Router documentation](../online_serving/router.md) for details.
...
Start the Router service. The `--splitwise` parameter specifies the scheduling mode as disaggregated deployment. Log information is output to `log_router/router.log`. For Router usage details, refer to the [Router documentation](../online_serving/router.md).
