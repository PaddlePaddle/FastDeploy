...
export FD_MOBA_ENCODER_TOP_K_LEFT=80
export FD_MOBA_ENCODER_TOP_K_RIGHT=100
export FD_MOBA_DECODER_TOP_K_LEFT=120
export FD_MOBA_DECODER_TOP_K_RIGHT=140
export FD_MOBA_USE_ENCODER_SEQ_LIMIT=10240
export FD_MOBA_USE_DECODER_SEQ_LIMIT=15360
...
* `FD_MOBA_ENCODER_TOP_K_LEFT=80, FD_MOBA_ENCODER_TOP_K_RIGHT=100` indicates that the range of top - k is between 80 and 100 when the encoder is sparse.
* `FD_MOBA_DECODER_TOP_K_LEFT=120, FD_MOBA_DECODER_TOP_K_RIGHT=140` indicates that the range of top - k is between 120 and 140 when the decoder is sparse.
* `FD_MOBA_USE_ENCODER_SEQ_LIMIT=10240`: Queries with less than 10240 tokens will not be sparse when the encoder is sparse.
* `FD_MOBA_USE_DECODER_SEQ_LIMIT=15360`: Queries with less than 15360 tokens will not be sparse when the decoder is sparse.
* Setting `FD_MOBA_MLP_WEIGHT_PATH`: The MLP weights generated by training have a shape of [num_kv_head, moba_block_size, head_dim].
