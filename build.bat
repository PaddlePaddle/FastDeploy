...
REM Phase 1: Compile custom ops
REM TODO: Add init step (clean build/, *.egg-info, install setuptools_scm)
REM TODO: Add version_info step (write fastdeploy/version.txt)

echo.
echo [1] Building custom ops...
pushd custom_ops
REM TODO: Use --install-lib to install ops to a temp directory, then copy_ops
REM       to fastdeploy\model_executor\ops\gpu\ (mirrors build.sh flow).
REM       Current approach installs directly to site-packages which works for
REM       development but the wheel won't bundle the compiled ops.
...
REM Phase 2: Package FastDeploy wheel
echo.
...
echo.
echo Build complete.
REM TODO: Add cleanup step (remove build/, *.egg-info, ops tmp dir)
exit /b 0
