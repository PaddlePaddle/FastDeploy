...
void BindPPOCR(pybind11::module& m) {
  auto ppocr_module = m.def_submodule("ppocr", "Module to deploy PaddleOCR");

  pybind11::class_<vision::ppocr::DBDetector, FastDeployModel>(ppocr_module,
                                                               "DBDetector")
      .def(pybind11::init<std::string, std::string, RuntimeOption, Frontend>())
      .def("predict",
           [](vision::ppocr::DBDetector& self, pybind11::array& data) {
             auto mat = PyArrayToCvMat(data);
             std::vector<vision::OCRPredictResult> res;
