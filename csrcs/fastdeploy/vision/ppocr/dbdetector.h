...
#include "fastdeploy/vision/utils/ocrpost/ocr_postprocess_op.h"
...
class FASTDEPLOY_DECL DBDetector : public FastDeployModel {
 public:
  ...
  // 模型预测接口，即用户调用的接口
  virtual bool Predict(cv::Mat& im, std::vector<OCRPredictResult>& ocr_results);
 ...
 private:
  ...
  bool Preprocess(cv::Mat& mat, cv::Mat& resize_img, FDTensor* outputs,
  ...
  // OCR后处理类
  fastdeploy::vision::utils::PostProcessor post_processor_;
  ...
  // PPOCR的dynamic_shape需要在开启TensorRT之后进行设置.
};
