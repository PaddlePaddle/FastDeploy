...
def generate_html(benchmark_data, config):
    ...
    # 生成量化选择器按钮
    quant_buttons = '\n'.join(
        f'                <div class="seg-btn" data-val="{q}" onclick="setQuant(\'{q}\')">{q.upper()}</div>'
        for q in quants
    )
...
function updateSelectors() {{
    ...
    document.getElementById('cfg-fd-quant').textContent = currentQuant === 'bf16' ? 'BF16' : 'FP8 (block_wise)';
    document.getElementById('cfg-sg-quant').textContent = currentQuant === 'bf16' ? 'BF16' : 'FP8';
    const d = getData();
    if (d && d.fd) {{
...
function updateConclusion() {{
    ...
    const quantLabel = currentQuant === 'bf16' ? 'BF16 无量化' : currentQuant.toUpperCase() + ' 在线量化';
