...
if __name__ == "__main__":
    ...
    print(uie.predict(["2月8日上午北京冬奥会自由式滑雪女子大跳台决赛中中国选手谷爱凌以188.25分获得金牌！"]))
    ...
    uie.set_schema(['肿瘤的大小', '肿瘤的个数', '肝癌级别', '脉管内癌栓分级'])
    print(
        uie.predict([
            "（右肝肿瘤）肝细胞性肝癌（II-III级，梁索型和假腺管型），肿瘤包膜不完整，紧邻肝被膜，侵及周围肝组织，未见脉管内癌栓（MVI分级：M0级）及卫星子灶形成。（肿物1个，大小4.2×4.0×2.8cm）。"
        ]))

    uie.set_schema({"地震触发词": ["地震强度", "时间", "震中位置", "震源深度"]})
    print(
        uie.predict([
            '中国地震台网正式测定：5月16日06时08分在云南临沧市凤庆县(北纬24.34度，东经99.98度)发生3.5级地震，震源深度10千米。'
        ]))

    uie.set_schema({"评价维度": ["观点词", "情感倾向[正向，负向]"]})
    print(uie.predict(['店面干净，很清静，服务员服务热情，性价比很高，发现收银台有排队']))

    uie.set_schema(["情感倾向[正向，负向]"])
    print(uie.predict(['这个产品用起来真的很流畅，我非常喜欢']))
