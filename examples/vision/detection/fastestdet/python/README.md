...
> ```python
> FastestDet.predict(image_data, conf_threshold=0.65, nms_iou_threshold=0.45)
> ```
>
> 模型预测结口，输入图像直接输出检测结果。
>
...
> > * **is_no_pad**(bool): 通过此参数让图片是否通过填充的方式进行resize, `is_no_pad=True` 表示不使用填充的方式，默认值为`is_no_pad=True`
