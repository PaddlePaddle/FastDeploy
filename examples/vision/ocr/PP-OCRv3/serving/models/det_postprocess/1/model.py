...
class TritonPythonModel:
    ...
    def postprocess(self, infer_outputs, im_infos):
        """
        Parameters
        ----------
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            results = self.postprocess(infer_outputs, im_infos)
            #temp_return_results = np.array(results, dtype=np.int32)
            final_rec_results = []
            for i_batch in range(len(results)):
                box_list = results[i_batch]
                image_list = []
                if len(box_list) == 0:
