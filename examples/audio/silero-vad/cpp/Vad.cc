...
int Vad::getSampleRate() const {
    return sample_rate;
}

int Vad::getFrameMs() const {
    return frame_ms;
}

float Vad::getThreshold() const {
    return threshold;
}

int Vad::getMinSilenceDurationMs() const {
    return min_silence_duration_ms;
}

int Vad::getSpeechPadMs() const {
    return speech_pad_ms;
}

const wav::WavReader &Vad::getWavReader() const {
    return wavReader;
}

const std::vector<int16_t> &Vad::getData() const {
    return data;
}

const std::vector<float> &Vad::getInputWav() const {
    return inputWav;
}

int64_t Vad::getWindowSizeSamples() const {
    return window_size_samples;
}

int Vad::getSrPerMs() const {
    return sr_per_ms;
}

int Vad::getMinSilenceSamples() const {
    return min_silence_samples;
}

int Vad::getSpeechPadSamples() const {
    return speech_pad_samples;
}

std::string Vad::ModelName() const {
    return "VAD";
}
...
bool Vad::Initialize() {
    ...
    // InitRuntime
    if (!InitRuntime()) {
        fastdeploy::FDERROR << "Failed to initialize fastdeploy backend." << std::endl;
        return false;
    }
    return true;
}
...
void Vad::setAudioCofig(int sr, int frame_ms, float threshold, int min_silence_duration_ms, int speech_pad_ms){
    if (initialized) {
        fastdeploy::FDERROR<<"setAudioCofig must be called before init"<<std::endl;
...
bool Vad::Preprocess(std::vector<float> audioWindowData) {
    ...
    inputTensor.SetExternalData(input_node_dims, fastdeploy::FDDataType::FP32, audioWindowData.data());
    inputTensor.name = "input";
    srTensor.SetExternalData(sr_node_dims, fastdeploy::FDDataType::INT64, sr.data());
    srTensor.name = "sr";
    hTensor.SetExternalData(hc_node_dims, fastdeploy::FDDataType::FP32, _h.data());
    hTensor.name = "h";
    cTensor.SetExternalData(hc_node_dims, fastdeploy::FDDataType::FP32, _c.data());
    cTensor.name = "c";
...
bool Vad::Predict() {
    if (wavReader.sample_rate() != sample_rate) {
        fastdeploy::FDERROR
        << "The sampling rate of the audio file is not equal to the sampling rate set by the program. "
        << "Please make it equal. "
        << "You can modify the audio file sampling rate, "
        << "or use setAudioCofig to modify the program's sampling rate and other configurations."
        << std::endl;
        throw std::runtime_error("The sampling rate of the audio file is not equal to the sampling rate set by the program.");
    }
    for (int64_t j = 0; j < wavReader.num_samples(); j += window_size_samples) {
        std::vector<float> r{&inputWav[0] + j, &inputWav[0] + j + window_size_samples};
        Preprocess(r);
        if (!Infer(inputTensors, &outputTensors)) {
            fastdeploy::FDERROR << "Failed to inference while using model:" << ModelName() << "."
                                << std::endl;
            return false;
        }
        Postprocess();
...
std::vector<std::map<std::string, float>> Vad::getResult(
        float removeThreshold, float expandHeadThreshold, float expandTailThreshold, float mergeThreshold) {
    ...
    while (startIter != speakStart.end()) {
        if (removeThreshold < audioLength && *endIter - *startIter < removeThreshold) {
    ...
    *endIter = fmin(*endIter + expandTailThreshold, audioLength);
    ...
    for (int i = 0; i < speakStart.size(); ++i) {
        result.emplace_back(std::map<std::string, float>({{"start", speakStart[i]}, {"end", speakEnd[i]}}));
    }
    return result;
}
