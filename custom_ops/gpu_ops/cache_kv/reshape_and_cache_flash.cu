...
// ═══════════════════════════════════════════════════════════════════
template <typename T>
__device__ __forceinline__ void vec128_copy_1d(
    const T* __restrict__ src, T* __restrict__ dst,
    int n_elems, int tid, int stride) {
...
__global__ void reshape_and_cache_flash_cute_kernel(
  ...
  scalar_t* key_dst   = key_cache   + block_idx * block_stride
                                    + block_offset * page_stride;
  scalar_t* value_dst = value_cache + block_idx * block_stride
                                    + block_offset * page_stride;
  ...
  auto gK_src = make_tensor(
      make_gmem_ptr(key_src),
      make_layout(make_shape(num_heads, Int<head_dim>{}),
                  make_stride(Int<head_dim>{}, Int<1>{})));
  auto gV_src = make_tensor(
      make_gmem_ptr(value_src),
      make_layout(make_shape(num_heads, Int<head_dim>{}),
                  make_stride(Int<head_dim>{}, Int<1>{})));
  auto gK_dst = make_tensor(
      make_gmem_ptr(key_dst),
      make_layout(make_shape(num_heads, Int<head_dim>{}),
                  make_stride(head_stride, Int<1>{})));
  auto gV_dst = make_tensor(
      make_gmem_ptr(value_dst),
      make_layout(make_shape(num_heads, Int<head_dim>{}),
                  make_stride(head_stride, Int<1>{})));
...
inline Fp8KVCacheDataType get_fp8_kv_cache_data_type(const std::string& dtype_str) {
  ...
  if (head_dim == 64) {
    reshape_and_cache_flash_cute_kernel<64, scalar_t>
        <<<grid, block, 0, stream>>>(
            ...
            key_stride, value_stride, num_heads, block_size);
    ...
    PD_THROW("Unsupported head_dim: ", head_dim, ". Only 64 and 128 are supported.");
  }
}
  ...
  if (head_dim == 64) {
   reshape_and_cache_flash_cute_fp8_kernel<64, scalar_t>
        <<<grid, block, 0, stream>>>(
            ...
            cache_block_stride, cache_page_stride, cache_head_stride,
...
std::vector<paddle::Tensor> ReshapeAndCacheFlash(
  ...
  if (kv_dt == Fp8KVCacheDataType::kAuto) {
    ...
    LaunchNonFP8Kernel<DataType_>(
        reinterpret_cast<const DataType_*>(key_ptr),
...
std::vector<paddle::Tensor> ReshapeAndCacheFlashKernel(
  ...
  switch (key.type()) {
    case paddle::DataType::BFLOAT16: {
      return ReshapeAndCacheFlash<paddle::DataType::BFLOAT16>(
          key, value, key_cache, value_cache, slot_mapping,
          k_scale, v_scale, kv_cache_dtype);
    }
    case paddle::DataType::FLOAT16: {
      return ReshapeAndCacheFlash<paddle::DataType::FLOAT16>(
          key, value, key_cache, value_cache, slot_mapping,
          k_scale, v_scale, kv_cache_dtype);
    }
    case paddle::DataType::FLOAT32: {
      return ReshapeAndCacheFlash<paddle::DataType::FLOAT32>(
          key, value, key_cache, value_cache, slot_mapping,
          k_scale, v_scale, kv_cache_dtype);
    }
    default: {
      PD_THROW("Unsupported input dtype: ", key.type(),
