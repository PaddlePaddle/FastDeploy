...
__global__ void recover_decode_task(bool *stop_flags,
  ...
  if (thread_idx < bsz) {
    if (is_block_step[thread_idx] == true) {
      ...
      if (block_table_now[step_seq_lens_decoder[thread_idx] / block_size] !=
          -1) {
  ...
  if (thread_idx < bsz) {
    if (is_block_step[thread_idx] == true) {
      ...
      int max_possible_block_idx =
          (step_seq_lens_decoder[thread_idx] + num_extra_tokens) / block_size;
      max_possible_block_idx = min(max_possible_block_idx, block_num_per_seq);
      if (block_table_now[max_possible_block_idx] != -1) {
        // can be recovered for decoding
        int64_t *draft_tokens_now =
            draft_tokens + thread_idx * draft_tokens_len;
        const int64_t *step_draft_tokens_now =
            step_draft_tokens + thread_idx * draft_tokens_len;
        is_block_step[thread_idx] = false;
