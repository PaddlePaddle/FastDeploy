...
std::vector<paddle::Tensor> MoeFusedHadamardQuantFp8(
  ...
  paddle::Tensor out =
      GetEmptyTensor({numel / intermediate_size, intermediate_size},
  ...
  if (tiled) {
    MoeFusedHadamardQuantFp8TiledKernel<<<block_per_grid,
                                          thread_per_block,
                                          0,
                                          stream>>>(
        ...
        reinterpret_cast<__nv_fp8_e4m3*>(
            out.mutable_data<phi::dtype::float8_e4m3fn>()),
        top_k,
        intermediate_size,
        numel);
  } else {
    MoeFusedHadamardQuantFp8Kernel<<<block_per_grid,
                                     thread_per_block,
                                     0,
                                     stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(
            input.data<phi::dtype::bfloat16>()),
        scale.data<float>(),
        topk_ids.data<int64_t>(),
        reinterpret_cast<__nv_fp8_e4m3*>(
            out.mutable_data<phi::dtype::float8_e4m3fn>()),
...
PD_BUILD_STATIC_OP(moe_fused_hadamard_quant_fp8)
    ...
    .Outputs({"output"})
    .Attrs({"top_k: int", "intermediate_size: int", "tiled: bool"})
...
paddle::Tensor MoeFusedHadamardQuantFp8Func(const paddle::Tensor& input,
                                            ...
                                            const int intermediate_size,
                                            const bool tiled) {
  return MoeFusedHadamardQuantFp8(
      input, scale, topk_ids, top_k, intermediate_size, tiled)[0];
}
...
std::vector<paddle::Tensor> FusedHadamardQuantFp8(const paddle::Tensor& input,
                                                  const float scale) {
  int64_t numel = input.numel();
  paddle::Tensor out = GetEmptyTensor(
      input.dims(), paddle::DataType::FLOAT8_E4M3FN, input.place());
  constexpr int64_t thread_per_block = 256;
  int64_t block_per_grid = (numel + thread_per_block - 1) / thread_per_block;
  auto stream = input.stream();
