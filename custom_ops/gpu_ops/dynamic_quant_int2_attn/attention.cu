...
void __global__ multi_block_gqa_attention_kernel(Block_attn_params params) {
  ...
  const int c16_cache_len =
      seq_len_decoder < c16_cache_max_len
          ? seq_len_decoder
          : c16_remain_seq_len + seq_len_decoder % kBlockSize;
  ...
  const int data_num_per_block =
      cache_bits == 2 ? params.data_num_per_block : kBlockSize * kHeadDim;

  const int32_t q_offset = bidb * head_num * kHeadDim + q_head_idx * kHeadDim;

  Tensor gQ = make_tensor(
      make_gmem_ptr(reinterpret_cast<const input_type*>(params.q_input) +
                    q_offset),
  ...
  const int* block_table =
      params.block_table + bidb * params.max_num_blocks_per_seq + block_idx;
  const int32_t physical_block_number = block_table[0];
  int cache_offset;
  if constexpr (cache_bits == 2) {
    cache_offset = (physical_block_number * kv_head_num + kv_head_idx) *
                   data_num_per_block;
  } else {
    cache_offset = (bidb * c16_cache_max_len + block_idx * kBlockSize) *
  ...
  Tensor gK_tensor = make_tensor(
      make_gmem_ptr(reinterpret_cast<const input_type*>(params.cache_k_c16) +
                    cache_offset),
      Shape<Int<kBlockSize>, Int<kHeadDim>>{},
      make_stride(kHeadDim * kv_head_num, _1{}));

  Tensor gV_tensor = make_tensor(
      make_gmem_ptr(reinterpret_cast<const input_type*>(params.cache_v_c16) +
                    cache_offset),
      Shape<Int<kBlockSize>, Int<kHeadDim>>{},
      make_stride(kHeadDim * kv_head_num, _1{}));
  ...
  Tensor sQ = make_tensor(make_smem_ptr(reinterpret_cast<input_type*>(smem_)),
                          SmemLayoutQ{});
  Tensor sQK = make_tensor(make_smem_ptr(reinterpret_cast<input_type*>(smem_)),
                           SmemLayoutQK{});
  Tensor sK_tensor = make_tensor(sQ.data() + size(sQ), SmemLayoutKV{});
  Tensor sV_tensor =
      make_tensor(sK_tensor.data() + size(sK_tensor), SmemLayoutKV{});
  Tensor sVt = make_tensor(sV_tensor.data(), SmemLayoutVtransposed{});
  Tensor sVtNoSwizzle =
      make_tensor(sV_tensor.data(), SmemLayoutVtransposedNoSwizzle{});
  ...
  Tensor cK =
      make_identity_tensor(make_shape(size<0>(sK_tensor), size<1>(sK_tensor)));
  ...
  const int remain_seq_len = cache_bits == 2
                                 ? c2_cache_len - partition_idx * kBlockN
                                 : c16_cache_len - partition_idx * kBlockN;
  ...
  for (int n = 0; n < kTileN; ++n) {
    ...
    if constexpr (cache_bits == 2) {
      ...
      if (cur_remain_seq_len < kBlockSize) {
        copy<false, true>(
            gmem_tiled_copy, tVgV, tVsV, tKcK, cur_remain_seq_len);
      } else {
...
#pragma unroll
    ...
    if constexpr (cache_bits == 2) {
      gemm_v<input_type, scale_type>(acc_o,
                                     tSrQK,
                                     tSsQK,
                                     tOrVt,
                                     sV,
  ...
  uint32_t max_sum_offset = bidb * max_partition_num * head_num +
                            (tidx + q_head_idx) * max_partition_num +
                            store_partition_idx;
...
#pragma unroll
  ...
  Tensor gO = make_tensor(
      make_gmem_ptr(
          reinterpret_cast<output_type*>(params.partition_attn_out) +
          ((bidb * max_partition_num + store_partition_idx) * head_num +
           q_head_idx) *
              kHeadDim),
      Shape<Int<kBlockM>, Int<kHeadDim>>{},
      Stride<Int<kHeadDim>, _1>{});
...
__global__ void __launch_bounds__(Kernel_traits::kNReduceThreads)
    multi_block_attention_reduce_kernel(Block_attn_params params) {
  using output_type = typename Kernel_traits::output_type;
  ...
  const int32_t partition_num = (c2_cache_len + kBlockN - 1) / kBlockN +
                                (c16_cache_len + kBlockN - 1) / kBlockN;
  const int32_t max_partition_num = params.max_num_partitions;

  const int32_t offset =
      bidb * head_num * max_partition_num + head_idx * max_partition_num;
...
#pragma unroll
  ...
  global_max_logit =
      BlockAllReduce<float, MaxOp<float>, kNReduceThreads>(global_max_logit);

  float* share_sum_scale =
      reinterpret_cast<float*>(shared_mem + sizeof(float) * max_partition_num);
  ...
  global_exp_sum =
      BlockAllReduce<float, SumOp<float>, kNReduceThreads>(global_exp_sum);
  const float inv_global_exp_sum = fdividef(1.0f, global_exp_sum);

  output_type* partition_attn_out =
      reinterpret_cast<output_type*>(params.partition_attn_out) +
      bidb * head_num * max_partition_num * kHeadDim + head_idx * kHeadDim;

  output_type* attn_out = reinterpret_cast<output_type*>(params.attn_out) +
                          params.cu_seq_q[bidb] * head_num * kHeadDim +
                          head_idx * kHeadDim;
...
#pragma unroll
  for (int i = warp_id; i < partition_num; i += kNReduceWarps) {
    int2 sub_logits = *reinterpret_cast<int2*>(
        &partition_attn_out[i * head_num * kHeadDim +
                            lane_id * kNReducePacksize]);
    float scale = share_sum_scale[i];
#pragma unroll
    for (int k = 0; k < kNReducePacksize; ++k) {
      acc[k] +=
          static_cast<float>(reinterpret_cast<output_type*>(&sub_logits)[k]) *
          scale;
    }
  }

  float* acc_warp = reinterpret_cast<float*>(
      shared_mem + 2 * sizeof(float) * ((max_partition_num + 3) / 4 * 4));

  *reinterpret_cast<int4*>(acc_warp + warp_id * kHeadDim +
                           lane_id * kNReducePacksize) =
      *reinterpret_cast<int4*>(&acc);
...
#pragma unroll
  ...
  reinterpret_cast<output_type*>(acc_warp)[tidx] =
      static_cast<output_type>(warp_sum);

  __syncthreads();

  if (warp_id == 0) {
    reinterpret_cast<int2*>(attn_out)[lane_id] =
        reinterpret_cast<int2*>(acc_warp)[lane_id];
  }
}
...
void run_block_attn(Block_attn_params& params, cudaStream_t stream) {
  ...
  using Kernel_traits = Block_attn_kernel_traits<kGqaGroupSize,
                                                 kNWarps,
                                                 kTileN,
                                                 kHeadDim,
                                                 input_type,
                                                 output_type,
                                                 scale_type>;
  const int smem_size_c2 =
      Kernel_traits::kShareMemSizeC2 + params.data_num_per_block * 2;

  constexpr auto kernel_c2 =
      &multi_block_gqa_attention_kernel<Kernel_traits, 2>;
  if (smem_size_c2 >= 48 * 1024) {
    cudaFuncSetAttribute(
        kernel_c2, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size_c2);
  }
  kernel_c2<<<grid, Kernel_traits::kNThreads, smem_size_c2, stream>>>(params);

  const int smem_size_c16 = Kernel_traits::kShareMemSizeC16;

  grid.x = (params.c16_remain_seq_len + Kernel_traits::kBlockSize +
            Kernel_traits::kTileN * Kernel_traits::kBlockSize - 1) /
           (Kernel_traits::kTileN * Kernel_traits::kBlockSize);

  constexpr auto kernel_c16 =
      &multi_block_gqa_attention_kernel<Kernel_traits, 16>;
  if (smem_size_c16 >= 48 * 1024) {
    cudaFuncSetAttribute(
        kernel_c16, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size_c16);
  }
  ...
  int32_t reduce_shared_mem_size =
      ...
      (Kernel_traits::kNReduceWarps * Kernel_traits::kNReduceThreads) *
          sizeof(float);
  constexpr int32_t pack_size =
      16 / sizeof(typename Kernel_traits::output_type);
...
void DecoderAttention(const paddle::Tensor& q_input,
  ...
  const int c16_cache_len =
      cache_seq_len < c16_cache_max_len
          ? cache_seq_len
          : c16_remain_seq_len + cache_seq_len % kBlockSize;
  const int c2_cache_len = cache_seq_len - c16_cache_len;
  const uint32_t max_num_partitions =
      (c16_cache_len + max_seq_per_block - 1) / max_seq_per_block +
      (c2_cache_len + max_seq_per_block - 1) / max_seq_per_block;
  const int bsz = decoder_seqs_len.dims()[0];

  paddle::Tensor partition_attn_out =
      paddle::empty({bsz, head_num, max_num_partitions, head_dim},
                    paddle::DataType::FLOAT16,
                    q_input.place());
  paddle::Tensor maxs = paddle::empty({bsz, head_num, max_num_partitions},
                                      paddle::DataType::FLOAT32,
                                      q_input.place());
  paddle::Tensor sums = paddle::empty({bsz, head_num, max_num_partitions},
                                      paddle::DataType::FLOAT32,
                                      q_input.place());
  ...
  params.attn_out =
      const_cast<phi::dtype::bfloat16*>(attn_out.data<phi::dtype::bfloat16>());
  ...
  if (gqa_group_size == 5) {
    run_block_attn<input_type, output_type, scale_type, stage, 5>(
        params, q_input.stream());
  } else if (gqa_group_size == 8) {
    run_block_attn<input_type, output_type, scale_type, stage, 8>(
        params, q_input.stream());
  } else if (gqa_group_size == 14) {
    run_block_attn<input_type, output_type, scale_type, stage, 14>(
        params, q_input.stream());
  } else {
...
PD_BUILD_OP(dynamic_quant_int2_decoder_attention)
    ...
    .SetKernelFn(PD_KERNEL(dynamic_quant_int2::DecoderAttention));
