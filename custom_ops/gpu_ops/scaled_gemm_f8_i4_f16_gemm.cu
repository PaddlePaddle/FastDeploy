...
struct Params {
  ...
  Params(ConstPointer _act,
         ConstPointer _weight,
         ConstPointer _scales,
         ConstPointer _zeros,
         ConstPointer _bias,
         Pointer _out,
         float _alpha,
...
void scaled_gemm_f8_i4_f16_launcher(Params& params,
  ...
  auto runner = std::make_shared<
      kernels::cutlass_kernels::CutlassFpAIntBGemmRunner<__nv_fp8_e4m3,
                                                         cutlass::uint4b_t,
                                                         QuantOp,
                                                         half,
                                                         T,
  ...
  if (getenv("FLAGS_fastdeploy_op_configs")) {
    ...
    if (fastdeploy_op_configs == "tune") {
      ...
      ConfigManager.update("scaled_gemm_f8_i4_f16",
                           params.m,
                           params.n,
                           params.k,
      ...
      if (!std::filesystem::exists(fastdeploy_op_configs)) {
        PADDLE_THROW(
            phi::errors::Fatal("Warning: The file \"" + fastdeploy_op_configs +
                               "\" does not exist in the specified path."));
      } else {
        auto& ConfigManager =
            ConfigManager::get_instance(fastdeploy_op_configs);
        auto best_config_string = ConfigManager.get_best_config(
            "scaled_gemm_f8_i4_f16", params.m, params.n, params.k);
...
std::vector<paddle::Tensor> scaled_gemm_f8_i4_f16(
  ...
  if (out_dtype == "float16") {
    scale_ptr =
        reinterpret_cast<const void*>(scale.data<phi::dtype::float16>());
    if (zero_points) {
      zero_points_ptr =
    ...
    if (groupsize > 0) {
      Params params(x_ptr,
                    ...
                    K,
                    groupsize);
      scaled_gemm_f8_i4_f16_launcher<
          half,
    ...
    scale_ptr =
        reinterpret_cast<const void*>(scale.data<phi::dtype::bfloat16>());
    if (zero_points) {
      zero_points_ptr =
...
PD_BUILD_STATIC_OP(scaled_gemm_f8_i4_f16)
    .Inputs({"x",
             "y",
             "scale",
             paddle::Optional("zero_points"),
             paddle::Optional("bias")})
    .Attrs({"groupsize: int", "out_scale: float", "out_dtype: std::string"})
    .Outputs({"out"})
