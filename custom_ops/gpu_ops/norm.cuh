...
#pragma unroll
    for (uint32_t j = 0; j < VEC_SIZE; j++) {
      output_vec[j] =
          float(input_vec[j]) * rms_rcp * (weight_bias + float(weight_vec[j]));
    }
    if ((i * num_threads + thread_id) * VEC_SIZE < d) {
      Store(output_vec,
            output + bx * stride_output + i * num_threads * VEC_SIZE +
                thread_id * VEC_SIZE);
    }
  }
#if (__CUDACC_VER_MAJOR__ >= 12 && defined(__CUDA_ARCH__) && \
...
template <uint32_t VEC_SIZE, typename T>
__global__ void QKRMSNormKernel(T* __restrict__ input,
                                T* __restrict__ weight,
                                T* __restrict__ output,
                                const uint32_t d,
                                const uint32_t batch_size,
                                const uint32_t num_heads,
                                const uint32_t stride_input_n,
                                const uint32_t stride_input_h,
                                const uint32_t stride_output_n,
                                const uint32_t stride_output_h,
                                float weight_bias,
                                float eps) {
  ...
  for (uint32_t job_idx = worker_idx; job_idx < num_jobs;
       job_idx += num_workers) {
    // clear buffer
    float sum_sq = 0.f;

    // map back to batch-idx and head-idx; layout [batch_size, num_heads,
    // head_dim]
    ...
    for (uint32_t i = 0; i < rounds; i++) {
      ...
      Fill(&input_vec, static_cast<T>(0.f));
      if ((i * num_threads + thread_id) * VEC_SIZE < d) {
        Load(input + batch_idx * stride_input_n + head_idx * stride_input_h +
...
#pragma unroll
    ...
    float rms_rcp = rsqrt(sum_sq / float(d) + eps);
...
#pragma unroll
      ...
      if ((i * num_threads + thread_id) * VEC_SIZE < d) {
        Store(output_vec,
              output + batch_idx * stride_output_n +
                  head_idx * stride_output_h + i * num_threads * VEC_SIZE +
                  thread_id * VEC_SIZE);
      }
    }
  }
...
template <typename T>
cudaError_t QKRMSNorm(T* input,
                      T* weight,
                      T* output,
                      uint32_t batch_size,
                      uint32_t num_heads,
                      uint32_t d,
                      uint32_t stride_input_n,
                      uint32_t stride_input_h,
                      uint32_t stride_output_n,
                      uint32_t stride_output_h,
                      float eps = 1e-5,
                      bool enable_pdl = false,
                      cudaStream_t stream = 0) {
  const uint32_t vec_size = std::gcd(16 / sizeof(T), d);
