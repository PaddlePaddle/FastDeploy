...
std::vector<paddle::Tensor> Fp8Int4WeightQuantizeKernel(
    const paddle::Tensor& input, int groupsize, std::string scale_dtype) {
  ...
  paddle::Tensor scale;
  auto packed_int4 = paddle::full(
      {shape[0] * shape[1] / 2}, 0, paddle::DataType::INT8, paddle::CPUPlace());
  if (scale_dtype == "bfloat16") {
    if (groupsize > 0) {
      scale = paddle::full({shape[0] / groupsize * shape[1]},
                           1.0,
                           paddle::DataType::BFLOAT16,
                           paddle::CPUPlace());
      group_wise_scale(scale.data<phi::dtype::bfloat16>(),
      ...
      scale = paddle::full(
          {shape[1]}, 1.0, paddle::DataType::BFLOAT16, paddle::CPUPlace());
      per_channel_scale(scale.data<phi::dtype::bfloat16>(),
                        input_cpu.data<float>(),
                        k,
                        n,
                        7.0f);
    ...
    if (groupsize > 0) {
      scale = paddle::full({shape[0] / groupsize * shape[1]},
                           ...
                           paddle::CPUPlace());
      group_wise_scale(scale.data<phi::dtype::float16>(),
                       input_cpu.data<float>(),
                       k,
                       n,
                       7.0f,
      ...
      scale = paddle::full(
          {shape[1]}, 1.0, paddle::DataType::FLOAT16, paddle::CPUPlace());
      per_channel_scale(scale.data<phi::dtype::float16>(),
                        input_cpu.data<float>(),
                        k,
                        n,
                        7.0f);
  ...
  auto out = paddle::full(
      {shape[1] / 2, shape[0]}, 0, paddle::DataType::INT8, paddle::CPUPlace());
...
std::vector<paddle::DataType> Fp8Int4WeightQuantizeInferDtype(
  ...
  if (scale_dtype == "bfloat16") {
    ...
    PD_THROW(
        "scaled_gemm_f8_i4_f16_half_gemm_fused only support bfloat16 and "
        "float16 output");
  }
...
PD_BUILD_STATIC_OP(scaled_gemm_f8_i4_f16_weight_quantize)
    .Inputs({"input"})
    .Attrs({"groupsize: int", "scale_dtype: std::string"})
