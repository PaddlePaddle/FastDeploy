      ...
      if (prefill_one_step_stop) {
        ...
        if (stop_flags[bid]) {
          if (seq_lens[bid] == 0) {
            ...
            // If stop_flags was already set before sampling (e.g., EOS from a previous
            // step), replace with EOS. But if the sampled token itself is NOT an EOS
            // (e.g., stop was triggered by length_cond externally), preserve the token.
