...
struct Int8Nf4InterleavedMma<ElementA,
  ...
  static_assert(platform::is_same<ElementB, int8_t>::value ||
  ...
  using AccessTypeNF4LookUpTable =
      cutlass::Array<int32_t, kAlignmentNF4LookUpTable>;
  using IteratorNF4LookUpTableThreadMap =
      transform::PitchLinearStripminedThreadMap<layout::PitchLinearShape<4, 1>,
                                                16 / kAlignmentNF4LookUpTable,
                                                kAlignmentNF4LookUpTable>;
  using IteratorNF4LookUpTable =
      cutlass::transform::threadblock::PredicatedTileAccessIterator<
  ...
  using Converter = FastInterleavedAndBiasedNumericArrayConverterNf4<
      ElementA,
      ElementB,
  ...
  using ThreadblockMma =
      cutlass::gemm::threadblock::Int8Nf4InterleavedMmaMultistage<
...
struct Int8Nf4InterleavedMma<
    ...
    typename platform::enable_if<(ArchTag::kMinComputeCapability >=
                                  80)>::type> {
  static_assert(platform::is_same<ElementA, int8_t>::value, "Element A int8_t");
 ...
 private:
  ...
  // static_assert(ColumnsInterleaved==4 || cutlass::platform::is_same<ElementB,
  // int8_t>::value, "####");
 ...
 public:
  ...
  using AccessTypeNF4LookUpTable =
      cutlass::Array<int32_t, kAlignmentNF4LookUpTable>;
  using IteratorNF4LookUpTableThreadMap = transform::
      PitchLinearStripminedThreadMap<layout::PitchLinearShape<4, 1>, 1, 4>;
  using IteratorNF4LookUpTable =
      cutlass::transform::threadblock::PredicatedTileAccessIterator<
          cutlass::MatrixShape<1, 16>,
          int32_t,
  ...
  // static_assert(MmaCore::MmaPolicy::Operator::FragmentB::kElements==64,"MmaCore::MmaPolicy::Operator::FragmentB::kElements
  // == 32");
  using Converter = FastInterleavedAndBiasedNumericArrayConverterNf4<
      ElementA,
      ElementB,
