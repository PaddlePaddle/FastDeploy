...
static std::vector<CutlassTileConfig> get_candidate_tiles_w4a4(
    const bool is_weight_only,
    const bool is_weight_only_encoder,
    const bool simt_configs_only,
    const int sm) {
  VLOG(1) << "get_candidate_tiles_w4a4 sm: " << sm;
...
static std::vector<CutlassGemmConfig> get_candidate_configs_nf4(
    ...
    const bool is_weight_only_encoder,
    const bool simt_configs_only) {
  std::vector<CutlassTileConfig> tiles = get_candidate_tiles_w4a4(
      is_weight_only, is_weight_only_encoder, simt_configs_only, 80);

  std::vector<CutlassGemmConfig> candidate_configs;
  const int min_stages = 3;
  const int max_stages = sm >= 80 ? 5 : 3;
...
static CutlassGemmConfig estimate_best_config_from_occupancies_w4a4(
  ...
  if (occupancies.size() != candidate_configs.size()) {
    throw std::runtime_error(
        "[FT Error][estimate_best_config_from_occupancies_w4a4] occpancies and "
    ...
    VLOG(1) << "m <= 64 , decoder config";
      ...
      for (int ii = 0; ii < candidate_configs.size(); ++ii) {
        ...
        TileShape tile_shape =
            get_cta_shape_for_config_w4a4(candidate_config.tile_config);
        int occupancy = occupancies[ii];
        // std::cout<<"cutlass heuristic ii: "<<"ii"<<", calculate occupancy:
        // "<<occupancy<<std::endl;
        if (occupancy == 0) {
          continue;
        }
