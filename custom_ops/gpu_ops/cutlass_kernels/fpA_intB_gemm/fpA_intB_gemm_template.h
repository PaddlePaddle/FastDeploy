...
void generic_mixed_gemm_kernelLauncher(
  ...
  using CutlassActivationType =
      typename CudaToCutlassTypeAdapter<ActivationType>::type;
  using CutlassWeightType = typename CudaToCutlassTypeAdapter<WeightType>::type;
  using CutlassScaleZeroType =
      typename CudaToCutlassTypeAdapter<ScaleZeroType>::type;
  ...
  using EpilogueOp = typename cutlass_extensions::Epilogue<CutlassOutputType,
                                                           ElementsPerAccessC,
                                                           ElementAccumulator,
                                                           EpilogueTag>::Op;
  ...
  using TaggedOperator =
      typename cutlass::arch::TagOperator<Operator, QuantOp>::TaggedOperator;
  ...
  if constexpr (cutlass::isFinegrained(QuantOp)) {
    if constexpr (cutlass::platform::is_same<CutlassActivationType,
                                             cutlass::float_e4m3_t>::value) {
      if (group_size != 128) {
        throw std::runtime_error(
    ...
    if constexpr (QuantOp ==
                  cutlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_ONLY) {
      if (weight_zero_points != nullptr) {
        throw std::runtime_error(
            "Weight zero pointer must be a nullptr for scale only fine "
            "grained");
      }
    } else if constexpr (QuantOp == cutlass::WeightOnlyQuantOp::
                                        FINEGRAINED_SCALE_AND_ZEROS) {
      if (weight_zero_points == nullptr) {
        throw std::runtime_error(
            "Weight zero pointer must be valid for scale and bias fine "
            "grained");
      }
    }
  } else {
    if (group_size != k) {
      throw std::runtime_error(
          "Invalid group size for per column scaling kernels.");
    }
  ...
  typename Gemm::Arguments args(
      {m, n, k},
      group_size,
      {reinterpret_cast<CutlassActivationType*>(const_cast<ActivationType*>(A)),
       k},
      {reinterpret_cast<CutlassWeightType*>(const_cast<WeightType*>(B)), ldb},
      {reinterpret_cast<CutlassScaleZeroType*>(
           const_cast<ScaleZeroType*>(weight_scales)),
       ld_scale_zero},
      {reinterpret_cast<CutlassScaleZeroType*>(
  ...
  if (GemmKernel::kInterleave > 1 && ((k % MixedGemmArchTraits::ThreadblockK) ||
    ...
    throw std::runtime_error("Assertion: k[" + std::to_string(k) +
                             "] must be multiple of threadblockK[" +
  ...
  if (gemm.get_workspace_size(args) > workspace_bytes) {
    std::cerr
        << "Requested split-k but workspace size insufficient. Falling back to "
  ...
  if (can_implement != cutlass::Status::kSuccess) {
    std::string err_msg =
        "fp8_int4 cutlass kernel will fail for params. Error: " +
  ...
  if (init_status != cutlass::Status::kSuccess) {
    std::string err_msg =
        "Failed to initialize cutlass fp8_int4 gemm. Error: " +
...
template <typename ActivationType,
          ...
          typename ThreadblockShape,
          typename WarpShape>
void dispatch_gemm_config(ActivationType const* A,
                          WeightType const* B,
                          ScaleZeroType const* weight_scales,
                          ScaleZeroType const* weight_zero_points,
                          BiasType const* biases,
  ...
  switch (gemm_config.stages) {
    case 2:
      throw std::runtime_error(
          "[filter_and_run_mixed_gemm] Cutlass fp8_int4 gemm not supported for "
          "arch " +
          std::to_string(arch::kMinComputeCapability) +
          " with stages set to 2");
      break;
    case 3:
      generic_mixed_gemm_kernelLauncher<ActivationType,
                                        WeightType,
                                        ScaleZeroType,
                                        BiasType,
                                        OutputType,
...
void dispatch_gemm_to_cutlass(ActivationType const* A,
  ...
  // fp8_int4. We also only instantiate configs here where threadblockShapeM ==
  // warpShapeM since those usually perform the best for mixed type gemms.
  constexpr int tile_shape_k =
      128 * 8 / cutlass::sizeof_bits<ActivationType>::value;
  switch (gemm_config.tile_config) {
    ...
    case cutlass_extensions::CutlassTileConfig::Undefined:
      throw std::runtime_error(
          "[fp8_int4][dispatch_gemm_to_cutlass] gemm config undefined.");
      break;
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
      throw std::runtime_error(
          "[fp8_int4][dispatch_gemm_to_cutlass] gemm config should have "
          "already been set by "
          "heuristic.");
      break;
    default:
      ...
      throw std::runtime_error(
          "[fp8_int4][dispatch_gemm_to_cutlass] Config is invalid for mixed "
          "type GEMM.");
      break;
  }
}
...
template <typename ActivationType,
          ...
          typename BiasType,
          typename OutputType>
CutlassFpAIntBGemmRunner<ActivationType,
                         WeightType,
                         QuantOp,
                         ScaleZeroType,
                         BiasType,
...
void CutlassFpAIntBGemmRunner<ActivationType,
                              WeightType,
                              QuantOp,
                              ScaleZeroType,
                              BiasType,
...
void CutlassFpAIntBGemmRunner<
  ...
  if (gemmConfig.tile_config ==
    ...
    auto best_config =
        estimate_best_config_from_occupancies(configs,
                                              occupancies,
                                              m,
                                              n,
                                              k,
...
CutlassFpAIntBGemmRunner<ActivationType,
                         ...
                         ScaleZeroType,
                         BiasType,
                         OutputType>::getConfigs(int k) const {
  // printf(__PRETTY_FUNCTION__);
  cutlass_extensions::CutlassGemmConfig::CandidateConfigTypeParam
      config_type_param = cutlass_extensions::CutlassGemmConfig::
          CandidateConfigTypeParam::HOPPER;
  config_type_param = static_cast<
      cutlass_extensions::CutlassGemmConfig::CandidateConfigTypeParam>(
      config_type_param | cutlass_extensions::CutlassGemmConfig::
                              CandidateConfigTypeParam::WEIGHT_ONLY);
...
template <typename ActivationType,
          ...
          typename BiasType,
          typename OutputType>
size_t CutlassFpAIntBGemmRunner<ActivationType,
                                WeightType,
                                QuantOp,
                                ScaleZeroType,
                                BiasType,
                                OutputType>::getWorkspaceSize(int const m,
                                                              int const n,
                                                              int const k) {
  // printf(__PRETTY_FUNCTION__);
