...
void SwapCacheImpLayout(const std::vector<paddle::Tensor>& cache_gpu_tensors, // gpu
    ...
    const cudaMemcpyKind copy_kind = (mode == 0) ? cudaMemcpyDeviceToHost : cudaMemcpyHostToDevice;
    ...
    for(int layer_idx=0; layer_idx < cache_gpu_tensors.size(); layer_idx++){
        ...
        for(auto block_id: swap_block_ids_gpu){
            ...
            auto *cache_cpu_ptr_now = cache_cpu_ptr + cpu_block_id * cache_stride * layer_number + layer_idx * cache_stride;
            cudaError_t status = cudaMemcpyAsync(
                (copy_kind == cudaMemcpyDeviceToHost) ? cache_cpu_ptr_now : cache_gpu_ptr_now,
                (copy_kind == cudaMemcpyDeviceToHost) ? cache_gpu_ptr_now : cache_cpu_ptr_now,
...
void SwapCacheLayout(const std::vector<paddle::Tensor>&  cache_gpu_tensors, // gpu
