      ...
      for (int v_id = tid; v_id < hidden_size_int4; v_id += blockDim.x) {
        ...
        if (up_gate_proj_in_scale) {
          for (int i = 0; i < vec_size; i++) {
            float quant_value = max_bound * up_gate_proj_in_scale[expert_now] *
            ...
            if constexpr (std::is_same<OutT, int8_t>::value) {
              ...
              if (RoundType == 0) {
                ...
                res_vec[i] = static_cast<OutT>(
                    ClipFunc<float>(round(quant_value), min_bound, max_bound));
              }
            } else {
              // w4afp8
        ...
        Store<OutT, vec_size>(
            res_vec, &permute_x[dst_token_idx * hidden_size + v_id * vec_size]);
      }
