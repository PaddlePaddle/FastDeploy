...
cuda_kernel(const scalar_t *__restrict__ topk_ids,
  ...
  if (compute_token_padded_sum && threadIdx.x == 0) {
    int32_t sum = 0;
...
std::vector<paddle::Tensor> count_tokens_per_expert_func(const paddle::Tensor &topk_ids,
  ...
  int *h_token_padded_sum;
  if (compute_token_padded_sum) {
    cudaHostAlloc((void **)&h_token_padded_sum,
                  sizeof(int),
  ...
  cuda_kernel<<<1, 1024, num_experts * sizeof(int32_t), stream>>>(
      ...
      topk_ids_numel,
      num_experts,
      compute_token_padded_sum);

  paddle::Tensor token_padded_sum;

  if (compute_token_padded_sum) {
    cudaMemcpyAsync(h_token_padded_sum, token_nums_this_rank_padded.data<int>(), sizeof(int), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    token_padded_sum = paddle::full({1}, *h_token_padded_sum, paddle::DataType::INT32, paddle::CPUPlace());
  } else {
    token_padded_sum = paddle::full({1}, 0, paddle::DataType::INT32, paddle::CPUPlace());
  }
  return {token_nums_per_expert, token_padded_sum};
}
