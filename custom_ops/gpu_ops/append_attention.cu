...
std::vector<paddle::DataType> AppendAttentionWithOuputInferDtype(
    const paddle::DataType& qkv_dtype,
    const paddle::DataType& key_cache_dtype,
    const paddle::DataType& value_cache_dtype,
    const paddle::DataType& seq_lens_encoder_dtype,
...
PD_BUILD_STATIC_OP(append_attention_with_output)
    .Inputs({"qkv",
             ...
             paddle::Optional("kv_signal_data"),
             "fmha_out"})
    .Outputs({"key_cache_out", "value_cache_out", "fmha_out_out"})
    .SetInplaceMap({{"key_cache", "key_cache_out"},
                    {"value_cache", "value_cache_out"},
                    {"fmha_out", "fmha_out_out"}})
    .Attrs({"compute_type: std::string",
            "cache_quant_type: std::string",
            "use_neox_rotary_style: bool",
            "rope_3d: bool",
    ...
    .SetInferDtypeFn(PD_INFER_DTYPE(AppendAttentionWithOuputInferDtype));