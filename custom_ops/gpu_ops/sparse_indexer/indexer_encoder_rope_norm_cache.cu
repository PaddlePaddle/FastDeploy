...
__global__ void GQAVariableLengthRotarySplitKernel(
  ...
  const int offset =
      (q_num_head + kv_num_head) * last_dim;  // for all q,k,v
  ...
  for (int gloabl_hi = global_warp_idx; gloabl_hi < all_head_num;
    ...
    const int64_t base_idx =
        token_idx * (q_num_head + kv_num_head) * last_dim + hi * last_dim +
        h_bias;
...
__global__ void cache_kernel(
  ...
  const uint32_t offset =  hidden_size;
  for (uint32_t linear_index = global_thread_idx * VecSize,
    ...
    const uint32_t ori_idx =
        token_idx * (num_heads + kv_num_heads) * head_size +
...
void CascadeAppendWriteCacheKVQKV(
  ...
  const uint32_t elem_nums =
      num_tokens * kv_num_heads * head_dim;
...
std::vector<paddle::Tensor> IndexerEncoderRopeNormWriteCache(
  ...
  const int head_dim = cache_quant_type == "cache_int4_zp"
                           ...
                           : key_cache_dims[3];
  const int num_heads =
      qkv_dims[qkv_dims.size() - 1] / head_dim -  kv_num_heads;
...
// PD_BUILD_STATIC_OP(indexer_encoder_rope_norm_write_cache)
//     .Inputs({"qkv",
//              "key_cache",
//              "cu_seqlens_q",
//              "cu_seqlens_k",
...
//     .Outputs({"q", "k", "qkv_out", "key_cache_out"})
...
//             "rope_3d: bool"})
//     .SetKernelFn(PD_KERNEL(IndexerEncoderRopeNormWriteCache));
