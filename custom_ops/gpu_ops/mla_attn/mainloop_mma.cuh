...
CUTLASS_DEVICE void mma_f16(const Params& mainloop_params,
  ...
  const int chunk_num_this_seq =
      cute::ceil_div(kv_len, mainloop_params.chunk_size_device[0]);
  ...
  Tensor sK =
      make_tensor(make_smem_ptr(shared_storage.smem_kv.data()), SmemLayoutK{});
  ...
  Tensor s_scale = make_tensor(make_smem_ptr(shared_storage.smem_scale.data()),
                               SmemLayoutRow{});
  ...
  const int end_tile_idx =
      cute::ceil_div(
          min(start_len + mainloop_params.chunk_size_device[0], kv_len),
          BLOCK_SHAPE_KV) -
      1;
...
#pragma unroll
      for (int w_i = 0; w_i < 2; ++w_i) {
        const int token_group_idx =
            warp_idx * 16 + (thread_idx % 32) / 4 + 8 * w_i;
        ...
        if (token_idx < qo_len) {
          ...
          const int write_idx =
              bid * bid_offset + token_idx * Ktraits::GROUP_SIZE + head_idx;
...
CUTLASS_DEVICE void mma_f16_two_stages(const Params& mainloop_params,
  ...
  Tensor sVt_s2 = make_tensor(
      make_smem_ptr(shared_storage.smem_kv.data() + Ktraits::NUM_PER_STAGE),
      SmemLayoutVtOneStage{});
  Tensor sVt_s3 = make_tensor(
      make_smem_ptr(shared_storage.smem_kv.data() + 2 * Ktraits::NUM_PER_STAGE),
      SmemLayoutVtOneStage{});
  Tensor sVt_s4 = make_tensor(
      make_smem_ptr(shared_storage.smem_kv.data() + 3 * Ktraits::NUM_PER_STAGE),
      SmemLayoutVtOneStage{});
  Tensor sPSS =
      make_tensor(make_smem_ptr(shared_storage.smem_p.data()), SmemLayoutP{});
  Tensor mM = make_tensor(make_gmem_ptr(mainloop_params.m_ptr),
...
#pragma unroll
    ...
    // gather qk gemm res
    cute::copy(
        smem_tiled_copy_P, tPrP, tPsP(_, _, _, smem_pipe_read_kv.index() % 2));
    ...
    for (int masking_step = n_masking_steps; kv_tile_idx >= start_tile_idx;
      ...
      if (smem_pipe_read_kv_cur.index() == 0) {
        gemm</*init=*/false, /*wg_wait=*/-1>(
            tiled_mma_pv_ss,
            tOrP_CS2(_, _, _, smem_pipe_read_kv_cur.index() % 2),
            tOrV1(_, _, _, _0{}),
            tOrO);
...
#pragma unroll
      ...
      cute::copy(smem_tiled_copy_P,
                 tPrP,
                 tPsP(_, _, _, smem_pipe_read_kv.index() % 2));
      cute::copy(scale_o, tScalesScale(_, smem_pipe_read_kv.index() % 2));
      // r2s fence wgmma
      cutlass::arch::fence_view_async_shared();
    ...
    if (smem_pipe_read_kv.index() == 0) {
      gemm</*init=*/false, /*wg_wait=*/-1>(
          tiled_mma_pv_ss,
          tOrP_CS2(_, _, _, smem_pipe_read_kv.index() % 2),
          tOrV1(_, _, _, _0{}),
          tOrO);
