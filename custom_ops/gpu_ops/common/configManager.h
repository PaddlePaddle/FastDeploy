...
class ConfigManager {
 public:
  static ConfigManager& get_instance(
      const std::string& config_path = "fastdeploy_op_configs.json") {
    static ConfigManager instance(config_path);
    return instance;
  }

  std::string get_best_config(const std::string& op_name,
                              const size_t m,
                              const size_t n,
                              const size_t k) {
    initialize();
    std::string mnk_string = op_name + "-" + std::to_string(update_m(m)) + "x" +
  ...
  void update(const std::string& op_name,
              const size_t m,
              const size_t n,
              const size_t k,
  ...
  void initialize() {
    ...
    if (file.is_open()) {
      try {
        ...
        std::cerr << "Error reading configs from " << config_path_ << " : "
                  << e.what() << std::endl;
