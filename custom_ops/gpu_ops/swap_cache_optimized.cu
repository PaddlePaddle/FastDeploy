 ...
 * 2. swap_cache_all_layers_batch: Multi-layer batch transfer with single kernel launch
 *
 ...
 * @param item_size_bytes Size of the item to transfer in bytes (must be 8-byte aligned)
 */
...
__global__ void swap_cache_per_layer_kernel(
    ...
    const char* src_now = static_cast<const char*>(src_ptr) + src_block_id * item_size_bytes;
 ...
 * @param dst_layer_tbl Layer base table for destination memory (array of pointers)
 * @param src_block_ids Array of source block IDs
 * @param dst_block_ids Array of destination block IDs
...
__global__ void swap_cache_all_layers_batch_kernel(
    ...
    for (int64_t i = 0; i < items_per_warp; ++i) {
        ...
        for (int64_t layer_id = 0; layer_id < num_layers; ++layer_id) {
            const char* src_ptr = reinterpret_cast<const char*>(src_layer_tbl[layer_id]) +
...
void SwapCachePerLayerImpl(
    const paddle::Tensor& cache_gpu,
    ...
    const int64_t item_size_bytes = num_heads * block_size * head_dim * sizeof(DataType_);
    ...
    for (size_t i = 0; i < swap_block_ids_gpu.size(); ++i) {
        if (swap_block_ids_gpu[i] < 0 || swap_block_ids_gpu[i] >= max_block_num_gpu) {
        ...
        if (swap_block_ids_cpu[i] < 0 || swap_block_ids_cpu[i] >= max_block_num_cpu) {
    ...
    checkCudaErrors(cudaMallocAsync(&d_src_block_ids, num_blocks * sizeof(int64_t), stream));
    checkCudaErrors(cudaMallocAsync(&d_dst_block_ids, num_blocks * sizeof(int64_t), stream));
    checkCudaErrors(cudaMemcpyAsync(d_src_block_ids, swap_block_ids_gpu.data(),
                                   num_blocks * sizeof(int64_t), cudaMemcpyHostToDevice, stream));
    checkCudaErrors(cudaMemcpyAsync(d_dst_block_ids, swap_block_ids_cpu.data(),
                                   num_blocks * sizeof(int64_t), cudaMemcpyHostToDevice, stream));

    // Configure kernel launch
    constexpr int kWarpsPerBlock = 4;
    const int threads_per_block = kWarpsPerBlock * WARP_SIZE;
    const int num_blocks_grid = (num_blocks + kWarpsPerBlock - 1) / kWarpsPerBlock;
    ...
    swap_cache_per_layer_kernel<D2H>
        <<<num_blocks_grid, threads_per_block, 0, stream>>>(
            src_ptr, dst_ptr, d_src_block_ids, d_dst_block_ids,
            num_blocks, item_size_bytes);
...
void SwapCacheAllLayersBatchImpl(
    ...
    for (int64_t layer_id = 0; layer_id < num_layers; ++layer_id) {
        if (D2H) {
            ...
            h_dst_layer_tbl[layer_id] = static_cast<uintptr_t>(cache_cpu_ptrs[layer_id]);
        } else {
            h_src_layer_tbl[layer_id] = static_cast<uintptr_t>(cache_cpu_ptrs[layer_id]);
    ...
    checkCudaErrors(cudaMallocAsync(&d_src_layer_tbl, num_layers * sizeof(uintptr_t), stream));
    checkCudaErrors(cudaMallocAsync(&d_dst_layer_tbl, num_layers * sizeof(uintptr_t), stream));
    checkCudaErrors(cudaMemcpyAsync(d_src_layer_tbl, h_src_layer_tbl.data(),
                                   num_layers * sizeof(uintptr_t), cudaMemcpyHostToDevice, stream));
    checkCudaErrors(cudaMemcpyAsync(d_dst_layer_tbl, h_dst_layer_tbl.data(),
                                   num_layers * sizeof(uintptr_t), cudaMemcpyHostToDevice, stream));
    ...
    const int64_t items_per_warp = (num_blocks + kBlockQuota * kWarpsPerBlock - 1) /
                                   (kBlockQuota * kWarpsPerBlock);
    const int num_blocks_grid = (num_blocks + items_per_warp * kWarpsPerBlock - 1) /
    ...
    swap_cache_all_layers_batch_kernel<D2H>
        <<<num_blocks_grid, threads_per_block, 0, stream>>>(
            ...
            d_src_block_ids, d_dst_block_ids,
            num_layers, num_blocks, items_per_warp, item_size_bytes);
...
void SwapCachePerLayer(
    ...
    switch (cache_gpu.dtype()) {
        case paddle::DataType::BFLOAT16:
            if (mode == 0) {
                SwapCachePerLayerImpl<paddle::DataType::BFLOAT16, true>(
                    cache_gpu, cache_cpu_ptr, max_block_num_cpu,
                    swap_block_ids_gpu, swap_block_ids_cpu, stream);
            } else {
        ...
        case paddle::DataType::UINT8:
            if (mode == 0) {
                SwapCachePerLayerImpl<paddle::DataType::UINT8, true>(
                    cache_gpu, cache_cpu_ptr, max_block_num_cpu,
                    swap_block_ids_gpu, swap_block_ids_cpu, stream);
            } else {
        ...
        case paddle::DataType::BFLOAT16:
            if (mode == 0) {
                SwapCacheAllLayersBatchImpl<paddle::DataType::BFLOAT16, true>(
                    cache_gpu_tensors, cache_cpu_ptrs, max_block_num_cpu,
                    swap_block_ids_gpu, swap_block_ids_cpu, stream);
            } else {
...
PD_BUILD_STATIC_OP(swap_cache_all_layers_batch)
    ...
    .Outputs({"cache_dst_outs"})
    .SetInplaceMap({{"cache_gpu_tensors", "cache_dst_outs"}})
