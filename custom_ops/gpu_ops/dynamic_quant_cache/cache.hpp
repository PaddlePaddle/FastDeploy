...
template <typename T, typename ScaleType, int kBlockSize, int kHeadDim, int kThreads, bool is_encoder>
__device__ void write_c2_cache_kernel(
        T *k_input,
        T *v_input,
        uint8_t *cache_k_c2,
        uint8_t *cache_v_c2,
    ...
    using SmemLayoutVtransposed = decltype(
        composition(SmemLayoutKV{}, make_layout(Shape<Int<kHeadDim>, Int<kBlockSize>>{}, GenRowMajor{})));
    ...
    using GmemTiledCopyKV = decltype(
        make_tiled_copy(Copy_Atom<Gmem_copy_struct, T>{},
        GmemLayoutAtom{},
        Layout<Shape<_1, _8>>{}));
    ...
    uint32_t *cache_v_smem = reinterpret_cast<uint32_t *>(smem_ + kHeadDim * kBlockSize * sizeof(T));
    ...
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 4; j+=2) {
            int4 value1 = *reinterpret_cast<int4*>(raw_pointer_cast(tKsK(_, j, i).data()));
            int4 value2 = *reinterpret_cast<int4*>(raw_pointer_cast(tKsK(_, j + 1, i).data()));
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                min_value = HalfMin<T>()(min_value, neigh_min_value);
                if (lane_idx < 8) {
                    const int scale_idx = warp_idx * warp_per_scale + lane_idx * 4 + k + i * 32 + j * 32;
    ...
    for (int i = 0; i < scale_k_num / 2; i+=2) {
        ...
        for (int k = 0; k < 2; ++k) {
            #pragma unroll
            for (int j = 0; j < 2; ++j) {
                pakc_half cur_value = reinterpret_cast<pakc_half*>(tSrK(_, _, i + k).data())[j];
                pakc_half next_value = reinterpret_cast<pakc_half*>(tSrK(_, _, i + k).data())[j + 2];
                ...
                cur_value = __hfma2_relu(cur_value, cur_quant_value, cur_quant_zp) * fp8_quant_factor;
                next_value = __hfma2_relu(next_value, next_quant_value, next_quant_zp) * fp8_quant_factor;

                cur_value = HalfMin<T>()(cur_value, max_blound);
                next_value = HalfMin<T>()(next_value, max_blound);

                uint32_t fp8_value = dynamic_quant_cache::Convert_to_fp8<T, cutlass::float_e4m3_t>()(
    ...
    for (int j = 0; j < 4; ++j) {
        for (int i = 0; i < 2; ++i) {
            int4 value = *reinterpret_cast<int4*>(raw_pointer_cast(tVsV(_, j, i).data()));
            ...
            if (col == 0) {
                ...
                reinterpret_cast<T*>(s_max)[j * 16 + row_idx + i * 128 + tidx % 8 / 4 * 64] = T(max_value_f);
                reinterpret_cast<T*>(s_min)[j * 16 + row_idx + i * 128 + tidx % 8 / 4 * 64] = T(min_value_f);
            }
        }
    }
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            value[0] = __hfma2_relu(value[0], s_quant[scale_idx], s_zp[scale_idx]) * fp8_quant_factor;
            value[1] = __hfma2_relu(value[1], s_quant[scale_idx + 4], s_zp[scale_idx + 4]) * fp8_quant_factor;
    ...
    const int cache_offset = physical_block_number * kv_head_num * data_num_per_block + bidh * data_num_per_block;

    const int kPackSize = 16 / sizeof(uint8_t);

    for (int i = tidx * kPackSize; i < data_num_per_block; i += kThreads * kPackSize) {
        *reinterpret_cast<int4*>(cache_k_c2 + cache_offset + i) = *reinterpret_cast<int4*>(reinterpret_cast<uint8_t*>(cache_k_smem) + i);
    }

    for (int i = tidx * kPackSize; i < data_num_per_block; i += kThreads * kPackSize) {
        *reinterpret_cast<int4*>(cache_v_c2 + cache_offset + i) = *reinterpret_cast<int4*>(reinterpret_cast<uint8_t*>(cache_v_smem) + i);
    }
