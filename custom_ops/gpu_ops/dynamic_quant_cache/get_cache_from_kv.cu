...
template <typename T, typename ScaleType, int kBlockSize, int kHeadDim, int kThreads>
void __global__ get_kv_from_cache_kernel(
        T *k_input,
        T *v_input,
        uint8_t *cache_k_c2,
        uint8_t *cache_v_c2,
    ...
    int cache_offset = physical_block_number * kv_head_num * data_num_per_block + bidh * data_num_per_block;
    ...
    for (int i = tidx * kPackSizeInt8; i < data_num_per_block; i += kThreads * kPackSizeInt8) {
        *reinterpret_cast<int4*>(cache_smem + i) = *reinterpret_cast<int4*>(cache_global + cache_offset + i);
    }
    ...
    if (bidh < kv_head_num) {
        ...
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            uint32_t c2_value = reinterpret_cast<uint32_t*>(cache_smem)[tidx + i * 128];
        ...
        const int store_idx = (cu_seq_k[bidb] + token_idx) * kv_head_num * kHeadDim + (bidh - kv_head_num) * kHeadDim + copy_col_idx;
        for (int i = copy_row_idx; i < kBlockSize; i += all_rows) {
            const int cur_idx = i * data_per_row + copy_col;
            int div_256 = cur_idx / 256 * 256;
            int mod_256 = cur_idx % 256;
            const int src_idx = mod_256 / 16 + div_256 + mod_256 % 8 / 4 * 16 + mod_256 % 4 * 32+ copy_col / 8 * 128;
            *reinterpret_cast<int4*>(v_input + store_idx + i * kv_head_num * kHeadDim) = reinterpret_cast<int4*>(cache_store_smem)[src_idx];
        }
    }
}
...
void  get_kv_from_cache(
    ...
    get_kv_from_cache_kernel<T, ScaleType, kBlockSize, kHeadDim, kThreads><<<gird_dim, kThreads, smem_size, stream>>>(
        k_input,
        v_input,
        cache_k_c2,
        cache_v_c2,
