...
void __global__ write_encoder_c2_cache_kernel(
        T *k_input,
        T *v_input,
        uint8_t *cache_k_c2,
        uint8_t *cache_v_c2,
...
template <typename T, typename ScaleType, int kBlockSize, int kHeadDim, int kThreads>
void __global__ write_encoder_c16_cache_kernel(
        const T *k_input,
        const T *v_input,
        T *cache_k_c16,
        T *cache_v_c16,
...
void write_encoder_cache(
    ...
    write_encoder_c2_cache_kernel<T, ScaleType, kBlockSize, kHeadDim, kThreads><<<gird_dim, kThreads, smem_size, stream>>>(
        k_input,
        v_input,
        cache_k_c2,
        cache_v_c2,
    ...
    write_encoder_c16_cache_kernel<T, ScaleType, kBlockSize, kHeadDim, kThreads><<<gird_dim, kThreads, smem_size, stream>>>(
        k_input,
        v_input,
        cache_k_c16,
        cache_v_c16,
...
PD_BUILD_OP(dynamic_quant_cache_write_encoder)
    ...
    .Outputs({"cache_k_c2_out", "cache_v_c2_out", "cache_k_c16_out", "cache_v_c16_out"})
    .SetInplaceMap({
        {"cache_k_c2", "cache_k_c2_out"},
