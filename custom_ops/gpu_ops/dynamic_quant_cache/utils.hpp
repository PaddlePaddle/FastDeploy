...
struct Convert_to_fp8<cutlass::half_t, cutlass::float_e4m3_t> {
    __forceinline__ __device__ uint32_t operator()(const uint32_t src1, const uint32_t src2) {
        uint32_t dst;
        asm volatile( \
            "{\n" \
            ".reg .b16 lo;\n" \
...
struct Convert_to_fp8<cutlass::bfloat16_t, cutlass::float_e4m3_t> {
    __forceinline__ __device__ uint32_t operator()(const uint32_t src1, const uint32_t src2) {
        float2 res1 = __bfloat1622float2(reinterpret_cast<const nv_bfloat162 &>(src1));
        float2 res2 = __bfloat1622float2(reinterpret_cast<const nv_bfloat162 &>(src2));
        uint32_t dst;
        asm volatile( \
            "{\n" \
            ".reg .b16 lo;\n" \
            ...
            "mov.b32 %0, {lo, hi};\n" \
            "}" \
            : "=r"(dst) : "f"(res1.x), "f"(res1.y), "f"(res2.x), "f"(res2.y));
        return dst;
    }
};
...
struct Convert_from_fp8<cutlass::float_e4m3_t, cutlass::bfloat16_t> {
    __forceinline__ __device__ int2 operator()(const uint32_t src) {
        ...
        *reinterpret_cast<nv_bfloat162 *>(&dst.x) = __float22bfloat162_rn(res_float1);
        *reinterpret_cast<nv_bfloat162 *>(&dst.y) = __float22bfloat162_rn(res_float2);
        return dst;
    }
};
...
__forceinline__ __device__ void copy_kv(const int tidx, const int data_num_per_block, const uint8_t *g_mem, uint8_t *s_mem) {
    constexpr int32_t kPackSize = 16 / sizeof(uint8_t);

    for (int i = tidx * kPackSize; i < data_num_per_block; i += kNThreads * kPackSize) {
        Gmem_copy_struct::copy(*reinterpret_cast<const cute::uint128_t *>(g_mem + i), *reinterpret_cast<cute::uint128_t *>(s_mem + i));
    }
}
...
inline __device__ void apply_rotary_embedding(Vec<T, PackSize>& vec, Vec<out_T, PackSize>& out, Vec<float, PackSize / 2>& cos, Vec<float, PackSize / 2>& sin) {
    ...
    for (int i = 0; i < PackSize / 2; i++) {
        ...
        out.data.elt[2 * i] = static_cast<out_T>(cos_inv_freq * v1 - sin_inv_freq * v2);
        out.data.elt[2 * i + 1] = static_cast<out_T>(sin_inv_freq * v1 + cos_inv_freq * v2);
    }
}
