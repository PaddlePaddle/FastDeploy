...
template <int kGqaGroupSize_, int kNWarps_, int kTileN_, int kHeadDim_, typename input_type_, typename output_type_, typename scale_type_>
struct Block_attn_kernel_traits {
    ...
    using GmemTiledCopy = decltype(
        make_tiled_copy(Copy_Atom<Gmem_copy_struct, input_type>{},
        GmemLayoutAtom{},
        Layout<Shape<_1, _8>>{}));

    using GmemTiledCopyO = decltype(
        make_tiled_copy(Copy_Atom<UniversalCopy<cutlass::uint128_t>, output_type>{},
        GmemLayoutAtom{},
        Layout<Shape<_1, _8>>{}));
    ...
    static constexpr int kShareMemSizeC2 = size(SmemLayoutQ{}) * sizeof(input_type);

    static constexpr int kShareMemSizeC16 = (size(SmemLayoutQ{}) + size(SmemLayoutKV{}) * 2) * sizeof(input_type);
};
}
