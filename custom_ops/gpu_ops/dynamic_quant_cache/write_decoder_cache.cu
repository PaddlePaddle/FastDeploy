...
template <typename input_type, typename output_type, typename ScaleType, int kBlockSize, int kHeadDim>
__global__ void write_decoder_c16_cache_kernel(
        const input_type * qkv_out,
    ...
    src.load_from(qkv_out + cu_seq_q[bidb] * (head_num + 2 * kv_head_num) * kHeadDim + bias_idx);
...
std::vector<paddle::Tensor> WriteDecoderCache(
    ...
    if (qkv_out.dtype() == paddle::DataType::BFLOAT16) {
        ...
        write_decoder_c16_cache<input_type, phi::dtype::float16, scale_type>(
            ...
            qkv_bias ? const_cast<input_type*>(qkv_bias.get().data<input_type>()) : nullptr,
...
PD_BUILD_OP(dynamic_quant_cache_write_decoder)
    ...
    .Outputs({"q_input", "cache_k_c2_out", "cache_v_c2_out", "cache_k_c16_out", "cache_v_c16_out"})
    .SetInplaceMap({
