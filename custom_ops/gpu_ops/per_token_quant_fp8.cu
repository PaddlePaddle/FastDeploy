...
#pragma unroll
      ...
      float scale;
      float s = MAX_VALUE / max_value_thread;
      // inf → clamp to float max or float max pow2
      if (isinf(s)) {
        if (true)
          scale = __int_as_float(0x7f000000);  // 2^127
        else
          scale = __int_as_float(0x7f7fffff);  // FLT_MAX
      } else {
        scale = s;
      }
      ...
      if (true) {
        if (scale != 0.0f) {
          ...
          int32_t unbiased_exp = int(exp) - 127;
          scale = ldexpf(1.0f, unbiased_exp);
        }
      }
...
#pragma unroll
      for (int vid = 0; vid < NUM_PER_THREADS; vid++) {
        float v = load_vec_float[vid] * scale;
        v = fminf(fmaxf(v, -MAX_VALUE), MAX_VALUE);
        res_vec[vid] = static_cast<phi::dtype::float8_e4m3fn>(v);
      }
      // store
      if (is_valid_data)
      ...
      if (lane_id == 0) {
        quanted_scale_now[iter] = 1.0f / scale;
      }
    }
  }
}
