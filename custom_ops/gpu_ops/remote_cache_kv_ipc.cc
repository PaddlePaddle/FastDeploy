...
RemoteCacheKvIpc::open_shm_and_get_complete_signal_meta_data(
...
#ifdef _WIN32
  PD_THROW(
      "open_shm_and_get_complete_signal_meta_data: POSIX shared memory is not "
  ...
  PADDLE_ENFORCE_NE(
      signal_ptr,
      MAP_FAILED,
      phi::errors::InvalidArgument("MAP_FAILED for cache_kv_complete_signal."));
  ...
  *write_signal_ptr = -1;
  ...
  PADDLE_ENFORCE_NE(
      identity_ptr,
      MAP_FAILED,
      phi::errors::InvalidArgument("MAP_FAILED for prefill_identity."));
...
void CUDART_CB
RemoteCacheKvIpc::save_cache_kv_complete_signal_layerwise(void* meta_data) {
