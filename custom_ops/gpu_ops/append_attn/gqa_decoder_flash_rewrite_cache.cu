...
__global__ void flash_rewrite_cache(
    ...
    for (int64_t linear_index = global_id; linear_index < (elem_cnt/8); linear_index += step) {
        ...
        int batch_id      = bid / (indexer_top_k * kv_heads);
        int indexer_kv_id = bid % (indexer_top_k * kv_heads) / kv_heads * 2; //每个warp读两个token 2*head_dim
        int head_id       = bid % (indexer_top_k * kv_heads) % kv_heads;
...
__global__ void indexer_rewrite_cache(
    const __nv_bfloat16 * cache_k,
    __nv_bfloat16 * key_new,
    const int32_t *token_sparse_index,
    const int *block_tables,
    ...
    const int head_dim_vector8 = head_dim/8;
    const int num_block_pre_batch = indexer_top_k/block_size;
    // const int dim_num_pre_warp = head_dim / 8;
    const int token_num_pre_warp = 32/(head_dim / 8);

    for (int64_t linear_index = global_id; linear_index < (elem_cnt/8); linear_index += step) {

        const int bid = linear_index / 32; 
        const int tid = linear_index % 32; //thread层只保留一个warp,剩下划到block_id上
        const int dim_id = tid / head_dim_vector8;  
        const int v8tid = tid % head_dim_vector8;    //一个warp的thread，分为两个half warp，每16个线程负责一个head_dim
        
        int batch_id      = bid / (indexer_top_k * kv_heads);
        int indexer_kv_id = bid % (indexer_top_k * kv_heads) / kv_heads * token_num_pre_warp; //每个warp读两个token 2*head_dim
        int head_id       = bid % (indexer_top_k * kv_heads) % kv_heads;
...
void GQAFlashRewriteCache(
    ...
    const int kv_heads = token_sparse_index.dims()[0];
    const int indexer_top_k = token_sparse_index.dims()[2];
    const int head_dim = cache_k.dims()[3];
    
    const int elem_cnt = batch * indexer_top_k * kv_heads * head_dim;
    const int block_size = 64;
    int grid_size = batch * indexer_top_k/4 * kv_heads;//indexer_top_k;

    dim3 block_dim(128);
    // if(head_dim%8 !=0 || indexer_top_k%128!=0){
    //     printf("not supported by current kernel launch!\n");
    //     exit(-1);
    // }
    // TODO: solve the static grid launch problem
    
    if (head_dim==64){
        indexer_rewrite_cache<<<grid_size,block_dim,0,stream>>>(
