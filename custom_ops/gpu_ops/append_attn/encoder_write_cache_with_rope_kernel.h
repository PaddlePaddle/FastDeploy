...
void EncoderWriteCacheWithRopeKernel(
  ...
  if (rotary_embs){
    ...
    if(rotary_dim < head_dim){
      if (!use_neox_style || num_heads == kv_num_heads || is_scale_channel_wise){
        PADDLE_THROW(phi::errors::Fatal(
          "partial_rotary_factor < 1.0 only supports use_neox_rotary_style=True, GQA and is_scale_channel_wise=false."));
      }
    }
  }
