...
#pragma unroll
      for (uint32_t reg_id = 0; reg_id < 8; ++reg_id) {
        if constexpr (!IS_SYSTEM) {
          ...
          if (mask_offset) {
            // NOTE (changwenbin): Supports sink + swa functionality under
            // mask_offset.
            if (sliding_window > 0) {
              int swa_part = mask_offset[q_idx * 2 + 1] - sliding_window;
              if (swa_part < 0) swa_part = 0;

              // sink_size is currently controlled by environment variables.
              int sink_part = mask_offset[q_idx * 2] + sink_size;
              out_of_boundary =
