...
std::vector<paddle::Tensor> GQARopeWriteCacheKernel(
  ...
  if (token_num < kv_token_num) {
    AppendCacheKV<data_t, 128, 64>(
        key_cache,
        value_cache,
        cache_k_dequant_scales ? &cache_k_dequant_scales.get() : nullptr,
        cache_v_dequant_scales ? &cache_v_dequant_scales.get() : nullptr,
        cache_k_zp ? &cache_k_zp.get() : nullptr,
        cache_v_zp ? &cache_v_zp.get() : nullptr,
        seq_lens_this_time,
        seq_lens_decoder,
        cu_seqlens_k,
        block_tables,
