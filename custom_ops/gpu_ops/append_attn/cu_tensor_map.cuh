...
struct cu_tensor_map_type_traits {
  static const CUtensorMapDataType type = CUtensorMapDataType::CU_TENSOR_MAP_DATA_TYPE_BFLOAT16;
};
...
struct cu_tensor_map_type_traits<phi::dtype::float16> {
  static const CUtensorMapDataType type = CUtensorMapDataType::CU_TENSOR_MAP_DATA_TYPE_FLOAT16;
};

template <>
struct cu_tensor_map_type_traits<uint8_t> {
  static const CUtensorMapDataType type = CUtensorMapDataType::CU_TENSOR_MAP_DATA_TYPE_UINT8;
};
...
template <typename T>
CUtensorMap makeTensorMapForKVCache(T const* addr, uint32_t block_num, uint32_t kv_num_head,
                                    uint32_t second_size, uint32_t last_size) {
  ...
  uint64_t global_strides[] = {
    last_size_bytes, second_size * last_size_bytes, kv_num_head * second_size * last_size_bytes};
  ...
  printf("start");
  CUresult res = cuTensorMapEncodeTiled(&tensorMap, CU_TENSOR_MAP_DATA_TYPE_UINT8, rank, reinterpret_cast<void *>(const_cast<T*>(addr)), global_dims,
                                 global_strides, box_dims, elem_strides, CUtensorMapInterleave::CU_TENSOR_MAP_INTERLEAVE_NONE,
                                 swizzle, CUtensorMapL2promotion::CU_TENSOR_MAP_L2_PROMOTION_L2_128B,
