...
__global__ void multi_query_append_attention_kernel(
  ...
  const uint32_t const_offset = kv_head_idx * kv_h_stride +
                                ...
                                tid % 8 * num_elems_per_128b<T>();
  // guard: block_id=-1 is an evicted SWA slot; SWA mask zeroes contribution
  const T *cache_k_now =
      cache_k + (block_id >= 0 ? block_id : 0) * kv_n_stride + const_offset;
  const T *cache_v_now =
      cache_v + (block_id >= 0 ? block_id : 0) * kv_n_stride + const_offset;
