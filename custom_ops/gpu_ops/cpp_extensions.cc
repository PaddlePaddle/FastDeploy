...
// Build a zero-copy numpy view that aliases the storage of a CPU (optionally
// pinned) paddle tensor. Faster than the Python implementation that goes
// through ctypes + np.frombuffer + reshape.
py::array numpy_view_of_cpu_tensor(paddle::Tensor& cpu_tensor) {
  PD_CHECK(
      is_cpu_place(cpu_tensor.place()) || is_pinned_place(cpu_tensor.place()),
      "numpy_view_of_cpu_tensor only supports CPU/pinned tensors, got: ",
      cpu_tensor.place());
  ...
  switch (cpu_tensor.dtype()) {
    ...
    case paddle::DataType::BOOL:
      ...
      break;
    default:
      PD_THROW("Unsupported dtype for numpy_view_of_cpu_tensor");
  }

  void* host_ptr = const_cast<void*>(cpu_tensor.data());
  // No-op capsule: the underlying paddle tensor owns the memory; the caller
  // is expected to keep the tensor alive while using the numpy view.
  py::capsule base(host_ptr, [](void*) {});
  return py::array(np_dtype, cpu_tensor.shape(), host_ptr, base);
}
...
PYBIND11_MODULE(fastdeploy_ops, m) {
  ...
  m.def("numpy_view_of_cpu_tensor",
        &numpy_view_of_cpu_tensor,
        "Zero-copy numpy view of a CPU/pinned paddle tensor");
