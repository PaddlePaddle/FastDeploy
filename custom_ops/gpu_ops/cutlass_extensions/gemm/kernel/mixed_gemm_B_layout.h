...
struct LayoutDetailsB<TypeA, TypeB, arch::Sm70> {
  static constexpr int ThreadblockK =
      128 * 8 / cutlass::sizeof_bits<TypeA>::value;
...
// MoE networks.
// TODO - Switch this to column major for weights since gemms should be more
// performant.
...
struct LayoutDetailsB<TypeA, cutlass::float_e4m3_t, arch::Sm89> {
 ...
 public:
  using Layout = layout::ColumnMajor;
  static constexpr int ElementsPerAccess =
      128 / cutlass::sizeof_bits<cutlass::float_e4m3_t>::value;
  using Operator = cutlass::arch::OpMultiplyAdd;
...
template <typename TypeA, typename Arch>
    ...
    uint8_t, Arch,
    typename platform::enable_if<Arch::kMinComputeCapability >= 75 &&
                                 Arch::kMinComputeCapability<90>::type> {
  static constexpr int ThreadblockK =
      128 * 8 / cutlass::sizeof_bits<TypeA>::value;
...
struct LayoutDetailsB<
    ...
    typename platform::enable_if<Arch::kMinComputeCapability >= 75>::type> {
  static constexpr int ThreadblockK =
      128 * 8 / cutlass::sizeof_bits<TypeA>::value;  // 64

 private:
  static constexpr int ElementsPerCacheLine =
      128 * 8 / sizeof_bits<uint2b_t>::value;
  static constexpr int ColumnsInterleaved =
      ElementsPerCacheLine / ThreadblockK;  // 8
 ...
 public:
  ...
  using Layout =
      layout::ColumnMajorTileInterleave<ThreadblockK, ColumnsInterleaved>;
  static constexpr int ElementsPerAccess =
      128 / cutlass::sizeof_bits<uint2b_t>::value;  // 64
  using Operator = cutlass::arch::OpMultiplyAddDequantizeInterleavedBToA;
};
...
struct LayoutDetailsB<
    ...
    uint8_t,
    Arch,
    typename platform::enable_if<Arch::kMinComputeCapability >= 90>::type> {
  static constexpr int ThreadblockK =
      128 * 8 / cutlass::sizeof_bits<TypeA>::value;
  using Layout = layout::ColumnMajor;
  static constexpr int ElementsPerAccess =
