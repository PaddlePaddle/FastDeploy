...
struct CollectiveMainloopFwd {
    ...
    static constexpr uint32_t TmaTransactionBytesScale = static_cast<uint32_t>(size(SmemLayoutScale{}(_, _0{})) * cutlass::sizeof_bits_v<float> / 8);
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        TMA_B tma_load_B = make_tma_copy(
            ...
            select<1, 2>(TileShape_MNK{}),
            size<0>(ClusterShape{}));
        Tensor mScale = make_tensor(make_gmem_ptr(args.weight_scale), args.layout_Scale);
        TMA_Scale tma_load_Scale = make_tma_copy(
            GmemTiledCopy{},
            mScale,
            SmemLayoutScale{}(_, _0{}),
            select<0>(Shape<Int<kBlockM>>{}),
    ...
    load(Params const& mainloop_params,
        ...
        Tensor sScale = make_tensor(make_smem_ptr(shared_storage.smem_scale.data()), SmemLayoutScale{});
        ...
        Tensor mScale = mainloop_params.tma_load_Scale.get_tma_tensor(mainloop_params.layout_Scale.shape());

        Tensor gA = local_tile(mA(_, _, bidb), select<0, 1>(Shape<Int<kBlockM>, Int<kBlockK / 2>>{}), make_coord(bidm, _));
        Tensor gScale = local_tile(mScale(_, bidm, bidb), select<0>(Shape<Int<kBlockM>>{}), make_coord(_));
        ...
        if constexpr (TokenPackSize == 0) {
            ...
            Tensor gB = local_tile(mB_this_expert, select<1, 2>(TileShape_MNK{}), make_coord(bidn, _));
            auto [tBgB, tBsB] = tma_partition(mainloop_params.tma_load_B, _0{}, Layout<ClusterShape>{}, group_modes<0, 2>(sB), group_modes<0, 2>(gB));
    ...
    mma(Params const& mainloop_params,
        ...
        #pragma unroll
        for (int kiter = 0; kiter < kTiles; ++kiter) {
            Tensor tSsA = smem_thr_copy_A.partition_S(sA(_, _, smem_pipe_read.index()));
            consumer_wait(pipeline, smem_pipe_read);
            gemm</*wg_wait=*/0>(tiled_mma, tSrA, tSsA, tSrB(_, _, _, smem_pipe_read.index()), tSrS, smem_tiled_copy_A, smem_thr_copy_A);
    ...
    mma_pipeline(Params const& mainloop_params,
        ...
        Tensor sA = make_tensor(make_smem_ptr(shared_storage.smem_a.data()), SmemLayoutA{});
        Tensor sB = make_tensor(make_smem_ptr(shared_storage.smem_b.data()), SmemLayoutB{});
        float2 *weight_scale = reinterpret_cast<float2*>(shared_storage.smem_scale.data()) + tidx / 4;

        Tensor tSrS1 = make_fragment_like(tSrS);
        Tensor tSrS2 = make_fragment_like(tSrS);

        __half2 * tSrS_data = reinterpret_cast<__half2*>(raw_pointer_cast(tSrS.data()));
        __half2 * tSrS1_data = reinterpret_cast<__half2*>(raw_pointer_cast(tSrS1.data()));
        __half2 * tSrS2_data = reinterpret_cast<__half2*>(raw_pointer_cast(tSrS2.data()));
        ...
        for (int kiter = 0; kiter < kTiles;) {
            Tensor tSsA1 = smem_thr_copy_A.partition_S(sA(_, _, smem_pipe_read.index()));
            consumer_wait(pipeline, smem_pipe_read);
            ...
            if (kiter < kTiles) {
                Tensor tSsA2 = smem_thr_copy_A.partition_S(sA(_, _, smem_pipe_read.index()));
                consumer_wait(pipeline, smem_pipe_read);
                scale_cur_k = *(weight_scale + smem_pipe_read.index() * (kBlockM / 2));
