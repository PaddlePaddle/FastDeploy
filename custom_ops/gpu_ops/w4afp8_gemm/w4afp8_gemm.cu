...
void weight_convert(
    const uint8_t* weight, uint8_t* weight_new, int batch, int M, int K) {
  assert(K % 64 == 0);
...
std::vector<paddle::Tensor> W4AFp8Gemm(
  ...
  if (token_padding_size == 0) {
    ...
    if (is_bfloat16) {
      ...
      DisPatchW4AFp8Gemm(reinterpret_cast<const cutlass::float_e4m3_t*>(
    ...
    if (is_bfloat16) {
      paddle::Tensor out = paddle::empty({batch_size, token_padding_size, M},
...
std::vector<paddle::Tensor> W4AFp8GemmWeightConvert(
  ...
  paddle::Tensor weight_new = paddle::empty(
      {batch_size, M, K / 2}, paddle::DataType::UINT8, weight.place());
  weight_convert(
      weight.data<uint8_t>(), weight_new.data<uint8_t>(), batch_size, M, K);
  return {weight_new};
}
...
PD_BUILD_STATIC_OP(w4afp8_gemm)
    .Inputs({"input", "weight", "tokens", "input_row_sum", "weight_scale"})
    .Outputs({"out"})
