...
void weight_convert(const uint8_t *weight, uint8_t *weight_new, int experts, int M, int K) {
    assert(K % 64 == 0);
...
__global__ void weight_permute_interleave_kernelw4afp8(
    ...
    int8_t* dst = output_data + n_id * original_k / 2 + (k_group_idx * pack_group_size / 2) + k_idx_in_group * 2;
    dst[0] = dst0;
    dst[1] = dst1;
}
