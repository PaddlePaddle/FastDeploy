...
void __global__ __launch_bounds__(Ktraits::kNWarps *cutlass::NumThreadsPerWarp,
  ...
  if (warp_group_idx == 0) {
    ...
    if constexpr (TokenPackSize == 0) {
      ...
      if (mma_tidx < kBlockN) {
        reinterpret_cast<float *>(input_row_sum)[mma_tidx] =
            reinterpret_cast<const float *>(mainloop_params.input_row_sum +
                                            input_sum_idx)[mma_tidx];
      }
    } else {
      const int input_sum_idx = bidb * TokenPackSize + bidn * kBlockN;
      if (mma_tidx < kBlockN / 4) {
        reinterpret_cast<float4 *>(input_row_sum)[mma_tidx] =
            reinterpret_cast<const float4 *>(mainloop_params.input_row_sum +
                                             input_sum_idx)[mma_tidx];
      }
    }
    ...
    if (TAIL_N > 0 && reamin_tokens < kBlockN) {
      Tensor tSrS_tail = partition_fragment_C(
          tiled_mma_tail, select<0, 1>(TileShape_MNK_TAIL{}));
...
void run_gemm(const InputType *A,
  ...
  int smem_size = sizeof(typename Kernel_traits::SharedStorage) +
                  sizeof(float) * Kernel_traits::kBlockN;
