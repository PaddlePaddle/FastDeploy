...
#pragma unroll
  for (int bid = blockIdx.x * blockDim.x + threadIdx.x;
      bid < real_bsz; bid += gridDim.x * blockDim.x) {
    if (seq_lens[bid] == 0) {
      continue;
...
std::vector<paddle::Tensor> SpeculatePreProcess(
  ...
  if (token_num_data == 0) {
    return {ids_remove_padding, batch_id_per_token, cu_seqlens_q, cu_seqlens_k,
  ...
  auto seq_lens_output = paddle::full(
    {bsz}, 0, paddle::DataType::INT32, input_ids.place());
  auto cu_seq_lens_q_output =
    paddle::empty({bsz + 1}, paddle::DataType::INT32, input_ids.place());
  auto batch_id_per_token_output =
    paddle::empty({bsz * max_draft_tokens_per_batch}, paddle::DataType::INT32, input_ids.place());
  auto real_output_token_num = paddle::zeros({1}, paddle::DataType::INT32, input_ids.place());
  ...
  cudaLaunchCooperativeKernel((void*)SpeculatePreProcessKernel,
    dim3(bsz), dim3(blockSize), kernel_args, 0, cu_stream);
