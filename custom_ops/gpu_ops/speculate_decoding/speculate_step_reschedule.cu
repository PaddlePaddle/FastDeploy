...
__device__ __forceinline__ bool in_need_block_list_schedule(
    const int &qid, int *need_block_list, const int &need_block_len) {
  bool res = false;
    ...
    if (stop_flags[tid]) {
      ...
      if (decoder_used_len > 0) {
        ...
        used_list_len[tid] = 0;
               ...
               block_table_now[(seq_lens_decoder[tid] + max_draft_tokens + 1) /
                               block_size] == -1) {
      // 统计需要分配block的位置和总数
#ifdef DEBUG_STEP
...
void SpeculateStepSchedule(
  ...
  auto step_bs_list =
      GetEmptyTensor({bsz}, paddle::DataType::INT32, stop_flags.place());
#ifdef DEBUG_STEP
  printf(
