...
template <typename T,
          ...
          int kBlockMaxN,
          int searchtimes>
__global__ void qk_gate_sort_decoder_kernel(const T* qk_gate_weight,
  ...
  if (decoder_seq_lens[bidb] == 0 ||
      decoder_seq_lens[bidb] < use_moba_seq_limit) {
    return;
  }
  const int seq_len =
      (decoder_seq_lens[bidb] + moba_block_size - 1) / moba_block_size;
  ...
  const int load_offset =
      bidb * head_num * kBlockMaxN + bidh * kBlockMaxN + tidx * kPackSize;
...
#pragma unroll
  ...
  const int store_idx =
      bidb * kv_head_num * kBlockMaxN + bidh_kv * kBlockMaxN + tidx * kPackSize;
...
void qk_gate_sort_decoder(const T* qk_gate_weight,
                          int* qk_gate_topk_idx,
                          const int* decoder_seq_lens,
                          const int head_num,
                          const int kv_head_num,
  ...
  constexpr auto kernel = qk_gate_sort_decoder_kernel<T,
                                                      knthreads,
                                                      moba_block_size,
                                                      kBlockMaxN,
                                                      searchtimes>;

  kernel<<<grid_dims, knthreads, 0, 0>>>(qk_gate_weight,
                                         qk_gate_topk_idx,
                                         decoder_seq_lens,
                                         head_num,
                                         kv_head_num,
...
std::vector<paddle::Tensor> DispatchQkSortDecoder(
  ...
  paddle::Tensor qk_gate_topk_idx =
      paddle::empty({batch_size, kv_head_num, kMaxN},
                    paddle::DataType::INT32,
                    qk_gate_weight.place());

  qk_gate_sort_decoder<kMaxN, kMobaBlockSize, T>(qk_gate_weight.data<T>(),
...
std::vector<paddle::Tensor> QkSortDecoder(const paddle::Tensor& qk_gate_weight,
                                          const paddle::Tensor& seq_len_encoder,
                                          const paddle::Tensor& seq_len_decoder,
                                          const int head_num,
                                          const int kv_head_num,
  ...
  if (qk_gate_weight.dtype() == paddle::DataType::FLOAT16) {
    return std::move(
        DispatchQkSortDecoder<phi::dtype::float16>(qk_gate_weight,
                                                   seq_len_encoder,
                                                   seq_len_decoder,
                                                   head_num,
                                                   kv_head_num,
    ...
    return std::move(
        DispatchQkSortDecoder<phi::dtype::bfloat16>(qk_gate_weight,
                                                    seq_len_encoder,
                                                    seq_len_decoder,
                                                    head_num,
                                                    kv_head_num,
...
PD_BUILD_STATIC_OP(moba_qk_sort_decoder)
    .Inputs({"qk_gate_weight", "seq_len_encoder", "seq_len_decoder"})
    .Attrs({"head_num: int",
            "kv_head_num: int",
            "top_k_left: int",
            "top_k_right: int",
