...
__global__ void moba_mlp_einsum_kernel(const T* src_data,
                                       const T* weight_data,
  ...
  const int src_base_idx = cu_seq_k[bidb] * head_num * kHeadDim +
                           block_idx * moba_block_size * head_num * kHeadDim +
                           bidh * kHeadDim + row_idx * head_num * kHeadDim +
                           col_idx;
  const int weight_base_idx =
      bidh * kHeadDim * moba_block_size + row_idx * kHeadDim + col_idx;
...
#pragma unroll
  for (int i = 0; i < kPackSize; i += 2) {
    *reinterpret_cast<int32_t*>(neighbor.data.elt + i) = __shfl_down_sync(
        0xffffffff, *reinterpret_cast<int32_t*>(sums.data.elt + i), 16);
  }
  ...
  __syncthreads();
  using pack_half =
      std::conditional_t<std::is_same<T, phi::dtype::float16>::value,
                         __half2,
                         nv_bfloat162>;
...
#pragma unroll
  ...
  const int base_store_idx =
      bidb * kMaxN * head_num * kHeadDim +
      (block_idx * (moba_block_size / 128) + store_row_id) * head_num *
          kHeadDim +
      bidh * kHeadDim + store_col_id;
...
void moba_mlp_einsum(const T* src_data,
                     const T* weight_data,
                     const int* seq_lens_encoder,
                     const int* seq_lens_decoder,
                     const int* cu_seq_k,
  ...
  if (moba_block_size == 1024) {
    moba_mlp_einsum_kernel<T, 1024, kHeadDim, kMaxN>
        <<<grid_dims, 128, 0, stream>>>(src_data,
                                        weight_data,
                                        seq_lens_encoder,
                                        seq_lens_decoder,
                                        cu_seq_k,
    ...
    moba_mlp_einsum_kernel<T, 128, kHeadDim, kMaxN>
        <<<grid_dims, 128, 0, stream>>>(src_data,
                                        weight_data,
                                        seq_lens_encoder,
                                        seq_lens_decoder,
    ...
    PADDLE_THROW(phi::errors::Unimplemented(
        "MobaMlpEinsum not implemented for moba_block_size = %d",
        moba_block_size));
  }
}
...
std::vector<paddle::Tensor> MobaMlpEinsum(
  ...
  paddle::Tensor k_gate_weight =
      paddle::zeros({batch_size, kMaxN, kv_head_num, kHeadDim},
                    k_input.dtype(),
                    k_input.place());
...
PD_BUILD_STATIC_OP(moba_mlp_einsum)
    .Inputs({"k_input",
             ...
             "seq_lens_decoder",
             "cu_seq_k"})
    .Attrs({"max_seq_len: int", "kv_head_num: int"})
    .Outputs({"k_gate"})
