...
__global__ void get_kv_from_cache_c16_kernel(T* k_input,
                                             T* v_input,
  ...
  const int physical_block_number =
      block_tables[bidb * max_blocks_per_seq + block_idx];
  ...
  if (bidh < kv_head_num) {
    const int cache_offset =
        ...
        bidh * kBlockSize * kHeadDim + col_idx;
    const int base_store_idx =
        (base_token_idx + cu_seq_k[bidb]) * kv_head_num * kHeadDim +
        bidh * kHeadDim + col_idx;
#pragma unroll
    for (int i = row_idx; i < kBlockSize; i += 128 / (kHeadDim / kPackSize)) {
      if (i < remain_tokens) {
        *reinterpret_cast<float4*>(k_input + base_store_idx +
                                   i * kv_head_num * kHeadDim) =
            *reinterpret_cast<const float4*>(cache_k + cache_offset +
                                             i * kHeadDim);
      }
    }
  } else {
...
#pragma unroll
    for (int i = row_idx; i < kBlockSize; i += 128 / (kHeadDim / kPackSize)) {
      if (i < remain_tokens) {
        *reinterpret_cast<float4*>(v_input + base_store_idx +
                                   i * kv_head_num * kHeadDim) =
            *reinterpret_cast<const float4*>(cache_v + cache_offset +
                                             i * kHeadDim);
      }
    }
  }
...
void get_kv_from_cache(T* k_input,
  ...
  if (cache_quant_type_str == "none") {
    ...
    get_kv_from_cache_c16_kernel<T, kBlockSize, kHeadDim>
...
void GetKVFromCache(
  ...
  if (k_input.dtype() == paddle::DataType::FLOAT16) {
    ...
    get_kv_from_cache<cute_type>(
        ...
        cache_k_dequant_scale ? reinterpret_cast<cute_type*>(const_cast<T*>(
                                    cache_k_dequant_scale.get().data<T>()))
                              : nullptr,
        cache_v_dequant_scale ? reinterpret_cast<cute_type*>(const_cast<T*>(
                                    cache_v_dequant_scale.get().data<T>()))
                              : nullptr,
        cache_k_zero_points ? reinterpret_cast<cute_type*>(const_cast<T*>(
                                  cache_k_zero_points.get().data<T>()))
                            : nullptr,
        cache_v_zero_points ? reinterpret_cast<cute_type*>(const_cast<T*>(
                                  cache_v_zero_points.get().data<T>()))
                            : nullptr,
        kv_head_num,
        head_dim,
        max_seq_k,
...
__global__ void get_cur_cu_seq_len_k_kernel(
  ...
  for (uint32_t bid = 0; bid < bsz; bid++) {
    ...
    cu_seq_q_pack[bid + 1] =
        cu_seq_q_pack[bid] + (q_len + pack_size - 1) / pack_size * pack_size;
  }
