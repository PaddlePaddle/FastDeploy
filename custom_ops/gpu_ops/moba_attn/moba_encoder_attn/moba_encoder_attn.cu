...
__global__ void __launch_bounds__(Ktraits::kNWarps* cutlass::NumThreadsPerWarp,
  ...
  const int* qk_gate_idx_cur_offset =
      data_params.qk_gate_topk_idx +
      data_params.cu_seq_q_pack[bidb] / kBlockM * data_params.head_num * kMaxN +
      (m_block * data_params.head_num + bidh) * kMaxN;

#pragma unroll
  for (int i = threadIdx.x; i < kMaxN / 4;
       i += Ktraits::kNWarps * cutlass::NumThreadsPerWarp) {
    reinterpret_cast<int4*>(qk_gate_topk_idx)[i] =
        reinterpret_cast<const int4*>(qk_gate_idx_cur_offset)[i];
  }

  const int n_block_max = min(
      cute::ceil_div((m_block + 1) * kBlockM + seq_len_k - seq_len_q, kBlockN),
  ...
  if (warp_group_idx == 0) {
    ...
    if (warp_idx_in_warpgroup == 0) {
      ...
      collective_mainloop.load<Ktraits::UseMoba>(mainloop_params,
                                                 pipeline_k,
                                                 pipeline_v,
                                                 smem_pipe_write_k,
                                                 smem_pipe_write_v,
    ...
    collective_mainloop.mma<Ktraits::UseMoba>(mainloop_params,
                                              pipeline_k,
                                              pipeline_v,
                                              smem_pipe_read_k,
                                              smem_pipe_read_v,
...
void run_moba_decoder_attn(moba_encoder_attn_params& params,
                           cudaStream_t stream) {
  ...
  typename CollectiveMainloop::Params mainloop_params =
      CollectiveMainloop::to_underlying_arguments(
          {static_cast<Element const*>(params.q_ptr),
           get_gmem_layout<kHeadDim>(params.max_seq_q * params.batch_size,
                                     params.head_num),
           static_cast<Element const*>(params.k_ptr),
           get_gmem_layout<kHeadDim>(params.max_seq_k * params.batch_size,
                                     params.kv_head_num),
  ...
  int num_blocks_m =
      cutlass::ceil_div(params.max_seq_q, Kernel_traits::kBlockM);
...
void run_moba_encoder_attn_hdim128(moba_encoder_attn_params& params,
                                   cudaStream_t stream) {
  constexpr static int Headdim = 128;
  constexpr static int kNWarps = kBlockM / 16 + 4;
  constexpr static int kStages = 2;

  using Ktraits = moba_encoder_attn_kernel_traits<Headdim,
                                                  kBlockM,
                                                  kBlockN,
                                                  kNWarps,
                                                  kStages,
...
template <typename T>
void DispatchMobaEncoderAttn(const paddle::Tensor& q_input,
  ...
  run_moba_encoder_attn_hdim128<kBlockM, kBlockN, kMaxN, cute_type>(
      params, out.stream());
}

void MobaEncoderAttn(const paddle::Tensor& q_input,
                     const paddle::Tensor& k_input,
                     const paddle::Tensor& v_input,
                     const paddle::Tensor& qk_gate_topk_idx,
                     const paddle::Tensor& cu_seq_q,
  ...
  if (q_input.dtype() == paddle::DataType::FLOAT16) {
    return DispatchMobaEncoderAttn<phi::dtype::float16>(q_input,
                                                        k_input,
                                                        v_input,
                                                        qk_gate_topk_idx,
                                                        cu_seq_q,
    ...
    return DispatchMobaEncoderAttn<phi::dtype::bfloat16>(q_input,
                                                         k_input,
                                                         v_input,
                                                         qk_gate_topk_idx,
                                                         cu_seq_q,
