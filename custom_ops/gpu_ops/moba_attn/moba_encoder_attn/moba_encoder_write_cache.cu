...
__global__ void write_encoder_cachekv_c16(const T* k_input,
                                          const T* v_input,
                                          const int* cu_seq_k,
  ...
  const uint32_t physical_block_number =
      block_table_now[blockIdx.x + seq_len_decoder[bidb] / kBlockSize];

  if (bidh < kv_head_num) {
    T* cache = cache_k +
               physical_block_number * kv_head_num * kBlockSize * kHeadDim +
               bidh * kBlockSize * kHeadDim + col_idx;
    const int base_load_idx =
        (block_idx + cu_seq_k[bidb]) * kv_head_num * kHeadDim +
        bidh * kHeadDim + col_idx;

#pragma unroll
    for (int i = row_idx; i < kBlockSize; i += 128 / (kHeadDim / kPackSize)) {
      if (i < remain_tokens) {
        *reinterpret_cast<float4*>(cache + i * kHeadDim) =
            *reinterpret_cast<const float4*>(k_input + base_load_idx +
...
#pragma unroll
    for (int i = row_idx; i < kBlockSize; i += 128 / (kHeadDim / kPackSize)) {
      if (i < remain_tokens) {
        *reinterpret_cast<float4*>(cache + i * kHeadDim) =
            *reinterpret_cast<const float4*>(v_input + base_load_idx +
                                             i * kv_head_num * kHeadDim);
      }
    }
  }
...
void MobaEncoderAttnWriteCacheKv(
  ...
  if (cache_quant_type_str == "none") {
    ...
    if (k_input.dtype() == paddle::DataType::FLOAT16) {
      ...
      write_encoder_cachekv_c16<T, kBlockSize, kHeadDim>
          <<<grid_dims, kThreads, 0, k_input.stream()>>>(
    ...
    PADDLE_THROW(phi::errors::Unimplemented(
        "Quantized cache not implemented for cache_quant_type = %s",
