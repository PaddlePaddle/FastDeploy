...
std::vector<paddle::Tensor> MobaAttention(
  ...
  paddle::Tensor out = paddle::empty(
      {qkv.dims()[0], head_num * head_dim}, qkv.dtype(), qkv.place());
  if (max_dec_len_this_time > 0) {
    MobaDecoderAttnWriteCacheKv(qkv,
                                q_input,
                                cu_seq_q,
                                cu_seq_k,
                                seq_len_encoder,
    ...
    auto qk_gate_weight = MobaQKGemm(q_input,
                                     k_block_means,
                                     seq_len_encoder,
                                     seq_len_decoder,
                                     cu_seq_q,
                                     ...
                                     kv_head_num,
                                     true,
                                     moba_use_decoder_seq_limit)[0];

    auto qk_gate_topk_idx = QkSortDecoder(qk_gate_weight,
                                          seq_len_encoder,
                                          seq_len_decoder,
                                          head_num,
                                          kv_head_num,
  ...
  if (max_enc_len_this_time > 0) {
    ...
    MobaEncoderAttnWriteCacheKv(k_input,
                                v_input,
                                cu_seq_k,
                                seq_len_encoder,
                                seq_len_decoder,
    ...
    if (moba_use_mlp && attn_gate_weight) {
      paddle::Tensor k_gate_mlp = MobaMlpEinsum(k_input,
    ...
    auto qk_gate_topk_idx = QkSortEncoder(
        ...
        moba_use_mlp && !attn_gate_weight ? max_seq_len
                                          : moba_use_encoder_seq_limit)[0];

    MobaEncoderAttn(q_input,
                    k_input,
                    v_input,
                    qk_gate_topk_idx,
                    cu_seq_q,
