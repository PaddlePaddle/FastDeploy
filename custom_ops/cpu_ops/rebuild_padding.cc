...
std::vector<paddle::Tensor> RebuildPaddingCPU(
    ...
    const paddle::Tensor &cu_seqlens_q,  // 改名：从cum_offsets改为cu_seqlens_q
    ...
    auto cu_seqlens_q_cpu = cu_seqlens_q.copy_to(paddle::CPUPlace(), true);  // 改名
    ...
    int bsz = cu_seqlens_q_cpu.shape()[0] - 1;  // 改为bsz+1的长度，所以bsz = shape[0] - 1
    ...
    const int *cu_seqlens_q_data = cu_seqlens_q_cpu.data<int>();  // 改名
...
std::vector<std::vector<int64_t>> RebuildPaddingInferShape(
    ...
    const std::vector<int64_t> &cu_seqlens_q_shape,  // 改名：从cum_offsets_shape改为cu_seqlens_q_shape
    ...
    if (output_padding_offset_shape) {
        ...
        int64_t bsz = cu_seqlens_q_shape[0] - 1;  // 改为bsz+1的长度，所以bsz = shape[0] - 1
...
std::vector<paddle::DataType> RebuildPaddingInferDtype(
    ...
    const paddle::DataType &cu_seqlens_q_dtype,  // 改名：从cum_offsets_dtype改为cu_seqlens_q_dtype
...
PD_BUILD_STATIC_OP(rebuild_padding_cpu)
    .Inputs({"tmp_out",
             "cu_seqlens_q",  // 改名：从cum_offsets改为cu_seqlens_q
             "seq_len_this_time",
             "seq_lens_decoder",
             "seq_lens_encoder",
