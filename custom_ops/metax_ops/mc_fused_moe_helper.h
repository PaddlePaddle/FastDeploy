...
void mc_grouped_gemm_basic_kernel(const ElementA *ptrA,
  ...
  mctlassExMatrixLayoutCreate(
      &matLayoutA, mctlassExDataType::MCTLASS_EX_BF16, m, k, k);
  ...
  mctlassExMatrixLayoutCreate(
      &matLayoutB, mctlassExDataType::MCTLASS_EX_INT8, k, n, k);
  mctlassExMatrixLayoutSetAttribute(
      matLayoutB,
      mctlassExMatrixLayoutAttribute_t::MCTLASS_EX_MATRIX_LAYOUT_ORDER,
      &majorB,
  ...
  // mat C: (m, n)
  mctlassExMatrixLayoutCreate(
      &matLayoutC, mctlassExDataType::MCTLASS_EX_BF16, m, n, n);
  mctlassExMatrixLayoutSetAttribute(
      matLayoutC,
      mctlassExMatrixLayoutAttribute_t::MCTLASS_EX_MATRIX_LAYOUT_ORDER,
      &majorC,
  ...
  mctlassExEpilogueType epilogue_type =
      mctlassExEpilogueType::MCTLASS_EX_GEMM_DEFAULT;
  if (ptrBias) {
  ...
  mctlassExDescSetAttribute(
      mctlass_desc,
      mctlassExDescAttributes_t::MCTLASS_EX_GEMM_DESC_B_SCALE_POINTER,
      &ptrScale,
      sizeof(ptrScale));
  mctlassExDescSetAttribute(
      mctlass_desc,
      mctlassExDescAttributes_t::MCTLASS_EX_GEMM_DESC_B_SCALE_TYPE,
      &scale_type,
  ...
  if (ptrBias) {
    mctlassExDescSetAttribute(
        mctlass_desc,
        mctlassExDescAttributes_t::MCTLASS_EX_GEMM_DESC_BIAS_POINTER,
        &ptrBias,
        sizeof(ptrBias));
  }
  // set coumpute type
  mctlassExDescSetAttribute(
      mctlass_desc,
      mctlassExDescAttributes_t::MCTLASS_EX_GEMM_DESC_COMPUTE_TYPE,
      &compute_type,
      sizeof(mctlassExDataType));
  // set epilogue type
  mctlassExDescSetAttribute(
      mctlass_desc,
      mctlassExDescAttributes_t::MCTLASS_EX_GEMM_DESC_EPILOGUE_TYPE,
      &epilogue_type,
      sizeof(mctlassExEpilogueType));

  const mctlassExContiguousGroupedGemmAlgo_t algo =
      mctlassExContiguousGroupedGemmAlgo_t::
          MCTLASS_EX_CONTIGUOUS_GROUPED_ALGO_SEGPTR;
  int blocksizeM = mctlassExContiguousGroupedGemmGetBlocksizeM(
      handle, mctlass_desc, matLayoutA, matLayoutB, matLayoutC, &algo);
 ...
 public:
  ...
  void computeFFN(const paddle::Tensor *input,
    ...
    mctlassExOrder_t column_major =
        mctlassExOrder_t::MCTLASS_EX_COLUMNMAJOR_ORDER;
