...
#if 0
void gather_or_scatter(paddle::Tensor& output,
...
void FlashAttnVarlenAndKVCacheForward(
    const paddle::Tensor& q, // prefill
    const paddle::Tensor& k, // prefill
    const paddle::Tensor& v, // prefill
    const paddle::Tensor& k_cache, // decode
    const paddle::Tensor& v_cache, // decode
    const paddle::Tensor& cu_seqlens_q, // prefill
    const paddle::Tensor& cu_seqlens_k, // prefill
    const paddle::Tensor& tokens_index, // decode
    const paddle::optional<paddle::Tensor>& cache_seqlens, // decode
    const paddle::optional<paddle::Tensor>& cache_batch_idx, // decode
    const paddle::optional<paddle::Tensor>& cache_leftpad, // decode
    const paddle::optional<paddle::Tensor>& block_tables, // decode
    const paddle::optional<paddle::Tensor>& alibi_slopes, // prefill, decode
    paddle::Tensor& query, // decode
    paddle::Tensor& out, // prefill, decode
    const int hidden_size, // decode
    const int max_seqlen_q, // prefill
    const int max_seqlen_k, // prefill
    const float dropout_p, // prefill
    const float softmax_scale, // prefill
    const bool causal, // prefill
    const int window_size_left, // prefill
    const int window_size_right, // prefill
    const float softcap, // prefill
    const bool rotary_interleaved, // decode
    const int num_splits, // decode
    const int num_prefills, // prefill
    const int num_decodes, // decode
  ) {
  if (qkv != paddle::DataType::BFLOAT16) {
    PD_THROW("Only support qkv dtype of BF16");
  }
  using data_t = phi::dtype::bfloat16;

  auto main_stream = q.stream();
  static cudaEvent_t main_event;
  static cudaEvent_t decoder_event;
  static cudaStream_t decoder_stream;
  static bool init_flag = false;
  if (num_prefills > 0 && num_decodes > 0 && !init_flag) {
  ...
  if (num_decodes > 0) {
    ...
    if (num_prefills > 0) {
      ...
      gather_or_scatter(decode_query,
                        q,
                        decode_tokens_index,
                        hidden_size,
                        false,
    ...
    decode_output = paddle::empty({decode_query.shape()[0], hidden_size})
