...
namespace xpu3 {
namespace plugin {
__attribute__((global)) void draft_model_preprocess_v2(
            ...
            if (!(base_model_stop_flags[tid] || batch_drop[tid])) {
                ...
                if (seq_lens_encoder[tid] > 0) {
                    ...
                    if(kvcache_scheduler_v1) {
                        ...
                        if (!base_model_is_block_step[tid] &&
                            is_block_step[tid]) {
                    ...
                    if (stop_flags[tid]) {
                        stop_flags[tid] = false;
                        // TODO: check
                        seq_lens_decoder[tid] = base_model_seq_len_decoder -
                                                base_model_seq_len_this_time;
...
static int xpu3_wrapper(api::Context* ctx,
    ...
    xpu3::plugin::draft_model_preprocess_v2<<<1, 64, ctx->xpu_stream>>>(
...
int draft_model_preprocess_v2(api::Context* ctx,
                              int64_t* draft_tokens,
                              int64_t* input_ids,
                              bool* stop_flags,
                              int* seq_lens_this_time,
    ...
    WRAPPER_DUMP_FUNCTION_T1(ctx, "draft_model_preprocess_v2", int64_t);
    ...
    WRAPPER_DUMP_PARAM3(
        ctx, num_model_step, accept_tokens_len, draft_tokens_len);
    WRAPPER_DUMP_PARAM4(ctx,
                        input_ids_len,
                        base_model_draft_tokens_len,
                        pre_ids_len,
    ...
    WRAPPER_CHECK_PTR(ctx,
                      int64_t,
                      bsz * base_model_draft_tokens_len,
