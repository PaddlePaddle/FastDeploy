...
static int cpu_wrapper(Context *ctx, bool *stop_flags, int *seq_lens_this_time,
                       ...
                       int64_t *first_token_ids, int *accept_num, const int bsz,
                       const int block_size, const int block_num_per_seq,
...
static int xpu3_wrapper(Context *ctx, bool *stop_flags, int *seq_lens_this_time,
                        ...
                        int *free_list_len, int64_t *first_token_ids, int *accept_num,
                        const int bsz, const int block_size,
                        const int block_num_per_seq,
                        const int max_decoder_block_num, const int max_draft_tokens) {
    using XPU_INT64 = typename XPUIndexType<int64_t>::type;
    auto speculate_free_and_dispatch_block_kernel = xpu3::plugin::speculate_free_and_dispatch_block;
    speculate_free_and_dispatch_block_kernel<<<ctx->ncluster(), 64, ctx->xpu_stream>>>(
...
int speculate_free_and_dispatch_block(Context *ctx, bool *stop_flags,
    ...
    if (ctx->dev().type() == api::kCPU) {
        return cpu_wrapper(
            ...
            encoder_block_lens, is_block_step, step_block_list, step_len,
            recover_block_list, recover_len, need_block_list, need_block_len,
            used_list_len, free_list, free_list_len, first_token_ids, accept_num, bsz,
            block_size, block_num_per_seq, max_decoder_block_num, max_draft_tokens);
    }
