...
eb_mtp_gather_next_token(TX *src, TY *dst, int *encoder_seqs_lods, int *decoder_seqs_lods,
...
template <typename TX, typename TY>
static int
cpu_wrapper(api::Context *ctx, const TX *x, TY *y, const int *encoder_seqs_lods, const int *decoder_seqs_lods,
  ...
  for (int i=0; i < output_token_num; i++) {
    ...
    if (is_enc) {
      ...
      src = x + (encoder_len_total + decoder_seqs_lods[dec_idx] - (len - i)) * hidden_dim;
    }
    ret = api::cast<TX, TY>(ctx, src,
                            y + i * hidden_dim, hidden_dim);
...
static int xpu3_wrapper(api::Context *ctx, const TX *x, TY *y,
                        ...
                        int en_batch, int de_batch, int64_t hidden_dim) {
  auto eb_mtp_gather_next_token_kernel = xpu3::plugin::eb_mtp_gather_next_token<TX, TY>;
  // NOTE: Don't change 16 to 64, because kernel use gsm
  eb_mtp_gather_next_token_kernel<<<ctx->ncluster(), 16, ctx->xpu_stream>>>(
      const_cast<TX *>(x), y, encoder_seqs_lods.xpu, decoder_seqs_lods.xpu,
      encoder_batch_map.xpu, decoder_batch_map.xpu, en_batch, de_batch, hidden_dim);
  return api::SUCCESS;
}

template <typename TX, typename TY>
int eb_mtp_gather_next_token(api::Context *ctx, const TX *x, TY *y,
                         api::VectorParam<int32_t> &encoder_seqs_lods, // NOLINT
                         api::VectorParam<int32_t> &decoder_seqs_lods, // NOLINT
                         api::VectorParam<int32_t> &encoder_batch_map, // NOLINT
                         api::VectorParam<int32_t> &decoder_batch_map, // NOLINT
  ...
  WRAPPER_DUMP_PARAM6(ctx, x, y, encoder_seqs_lods, decoder_seqs_lods, encoder_batch_map,
                      decoder_batch_map);
  ...
  int m = encoder_seqs_lods.cpu[encoder_batch] + decoder_seqs_lods.cpu[decoder_batch];
  ...
  if (ctx->dev().type() == api::kCPU) {
    return cpu_wrapper<TX, TY>(ctx, x, y, encoder_seqs_lods.cpu, decoder_seqs_lods.cpu,
  ...
  if (ctx->dev().type() == api::kXPU3) {
    ...
    return xpu3_wrapper<TX, TY>(ctx, x, y, encoder_seqs_lods_xpu, decoder_seqs_lods_xpu,
...
#define INSTANTIATION_EB_MTP_GATHER_NEXT_TOKEN(TX, TY)                             \
  template int eb_mtp_gather_next_token<TX, TY>(                                   \
      api::Context *, const TX *, TY *, api::VectorParam<int32_t> &,               \
      api::VectorParam<int32_t> &, api::VectorParam<int32_t> &,                    \
      api::VectorParam<int32_t> &, int64_t);
