...
void lod_to_slot_mapping_nonmtp_decode(
    api::Context* xpu_ctx,
    paddle::Place place,
...
void lod_to_slot_mapping_nonmtp_decode_debug(
  ...
  // where操作
  paddle::Tensor minus_one =
      paddle::full({batch_size}, -1, paddle::DataType::INT32, place);
  auto minus_one_ptr = const_cast<int32_t*>(minus_one.data<int32_t>());
  ret = api::where<TID>(xpu_ctx,
                        mask.data<bool>(),
                        minus_one_ptr,
                        slot_mapping,
...
template <typename TQKV,
          ...
          typename TID,
          typename TSCALE>
void split_rope_kvcache_splice_encoder(
    api::Context* xpu_ctx,
    ...
    const api::VectorParam<int32_t>& pos_emb_offset,
    const api::VectorParam<int32_t>& real_batch,
    int64_t qkv_offset,
  ...
  if (rope_3d) {
    PD_THROW(
        "split_rope_kvcache_splice_encoder does not support rope_3d == true!");
  }
  ...
  auto k_split = paddle::empty(
      {token_num, real_kv_num_heads * head_dim}, qkv.type(), qkv.place());
  ...
  if (std::is_same<TKV_CACHE, int8_t>::value && intx_k_pc_scale &&
    ...
    if (k_cache_scale_inv) {
      k_scale_cache = paddle::empty(
          {real_kv_num_heads}, paddle::DataType::FLOAT32, qkv.place());
...
template <typename TQKV,
          ...
          typename TID,
          typename TSCALE>
void split_rope_kvcache_splice_decoder(
    api::Context* xpu_ctx,
    xftblock::XFTContext& xctx,
    const paddle::Tensor& qkv,
    const paddle::Tensor& rotary_embs,
    ...
    const api::VectorParam<int32_t>& pos_emb_offset,
    const api::VectorParam<int32_t>& real_batch,
    const api::VectorParam<int32_t>& non_mtp_decoder_seq_lod_vp,
    int64_t qkv_offset,
  ...
  if (rope_3d) {
    PD_THROW(
        "split_rope_kvcache_splice_decoder does not support rope_3d == true!");
  }
  if (std::is_same<TKV_CACHE, int8_t>::value &&
      (k_cache_scale_inv == nullptr || v_cache_scale_inv == nullptr)) {
    PD_THROW(
        "split_rope_kvcache_splice_decoder of kv_cache type int8_t does not "
  ...
  if (k_cache_scale_inv) {
    k_scale_cache = paddle::empty(
        {cache_scale_zero_len}, paddle::DataType::FLOAT32, qkv.place());
...
std::vector<paddle::Tensor> SplitEmbeddingKVCache(
  ...
  if (enc_batch > 0) {
    ...
    if (FLAGS_encoder_splice) {
      split_rope_kvcache_splice_encoder<XPU_XType,
                                        float,
                                        XPU_CType,
                                        int,
                                        E_Scale>(
      ...
      if (use_neox_rotary_style) {
        ret = infer_ops::
            split_neox_cache_kv_encoder<XPU_XType, float, XPU_CType, int>(
                ...
                usual_lod_vp,     // seq_lod
                slot_mapping_vp,  // real_batch
  ...
  if (dec_batch > 0) {
    ...
    api::VectorParam<int32_t> decoder_batch_map_vp = {
        ...
        dec_batch,
        const_cast<int32_t*>(decoder_batch_map.data<int32_t>())};  // real batch
    ...
    if (total_dec_len != dec_batch) {
      ...
      if (false && FLAGS_encoder_splice) {
        ...
        if (use_neox_rotary_style) {
          ret = infer_ops::
              split_neox_cache_kv_encoder<XPU_XType, float, XPU_CType, int>(
                  xpu_ctx,
                  reinterpret_cast<const XPU_XType*>(
                      qkv.data<data_t>()),  // qkv
                  reinterpret_cast<const float*>(
                      rotary_embs.data<float>()),  // rotary_pos_emb
                  reinterpret_cast<const int*>(
      ...
      if (FLAGS_decoder_splice) {
        paddle::Tensor seq_lod_xpu = paddle::empty(
            {dec_batch + 1}, paddle::DataType::INT32, qkv.place());
        auto seq_lod_xpu_ptr =
            const_cast<int32_t*>(seq_lod_xpu.data<int32_t>());
        ret = api::arange<int32_t, int64_t>(
            xpu_ctx, seq_lod_xpu_ptr, 0, 1, dec_batch + 1);
        PD_CHECK(ret == api::SUCCESS, "arange failed.");
        api::VectorParam<int32_t> non_mtp_decoder_seq_lod_vp = {
            const_cast<int32_t*>(non_mtp_decoder_seq_lod_cpu.data<int32_t>()),
            dec_batch + 1,
            const_cast<int32_t*>(
                seq_lod_xpu
                    .data<int32_t>())};  // use for split rope enc as lod in MTP
        split_rope_kvcache_splice_decoder<XPU_XType,
                                          float,
                                          XPU_CType,
                                          int,
                                          D_Scale>(
        ...
        if (use_neox_rotary_style) {
          ret = infer_ops::split_neox_cache_kv_decoder<XPU_XType,
              ...
              reinterpret_cast<D_Scale*>(quant_v_zp),     // v_cache_zp
              rope_3d);
          PD_CHECK(ret == api::SUCCESS, "split_neox_cache_kv_decoder failed.");
        } else {
