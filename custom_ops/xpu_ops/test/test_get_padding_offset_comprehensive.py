...
class TestGetPaddingOffset(unittest.TestCase):
    ...
    def test_normal_path_xpu(self):
        ...
        for i in range(bsz):
            ids_len = seq_lens[i]
            input_ids[i, 0:ids_len] = np.random.randint(
                1, 100, seq_lens[i], dtype=np.int64
            )
    ...
    def test_single_batch(self):
        ...
        np.testing.assert_array_equal(
            batch_id_per_token.numpy(), np.array([0, 0, 0, 0, 0], dtype=np.int32)
        )

        # cu_seqlens should be [0, 5]
        np.testing.assert_array_equal(
            cu_seqlens_q.numpy(), np.array([0, 5], dtype=np.int32)
        )
    ...
    def test_mixed_sequence_lengths(self):
        ...
        expected_batch_ids = (
            [1] * 1 + [2] * 10 + [3] * 20
        )  # Skip batch 0 (0 tokens)
        np.testing.assert_array_equal(
            batch_id_per_token.numpy(), np.array(expected_batch_ids, dtype=np.int32)
        )
...
class TestGetPaddingOffsetWithSpeculative(unittest.TestCase):
    """Test cases for get_padding_offset with speculative decoding."""
    ...
    def test_with_draft_tokens(self):
        """
        Test speculative decoding path when draft_tokens is provided.

        This tests the branch at lines 71-86 in get_padding_offset.cc:
        if (draft_tokens) {
        ...
        # --------------------
        # Setup: encoder and draft tokens
        # --------------------
        bsz = 2
        max_seq_len = 10
        seq_lens = np.array([4, 6], dtype=np.int32)  # Total tokens before output

        # Input IDs (used for normal remove padding)
        input_ids = np.random.randint(1, 100, (bsz, max_seq_len), dtype=np.int64)

        # Draft tokens for speculative decoding
        max_draft_tokens = 3
        draft_tokens = np.random.randint(1, 100, (bsz, max_draft_tokens), dtype=np.int64)

        # Encoder sequence lengths (tokens generated by encoder before this step)
        seq_lens_encoder = np.array([2, 3], dtype=np.int32)

        token_num = int(np.sum(seq_lens))

        # --------------------
        # Call function (lines 71-86: speculative path)
        # --------------------
        result = self.get_padding_offset(
            paddle.to_tensor(input_ids),
            paddle.to_tensor(seq_lens),
        ...
        # Verify outputs
        # --------------------
        expected_token_num = int(np.sum(seq_lens))
        self.assertEqual(x_remove_padding.shape[0], expected_token_num)
        self.assertEqual(batch_id_per_token.shape[0], expected_token_num)
        self.assertEqual(cu_seqlens_q.shape[0], bsz + 1)
        self.assertEqual(cu_seqlens_k.shape[0], bsz + 1)

    def test_draft_tokens_single_batch(self):
        """
        Test speculative decoding with single batch.

        Tests: boundary value bsz == 1 with draft tokens
        """
        try:
            paddle.set_device("xpu")
        except Exception:
        ...
        # All tokens should belong to batch 0
        np.testing.assert_array_equal(
            cu_seqlens_q.numpy(), np.array([0, 4], dtype=np.int32)
        )

    def test_draft_tokens_multiple_batches(self):
        """
        Test speculative decoding with multiple batches.

        Tests: bsz > 1 with varying draft token lengths
        """
        try:
            paddle.set_device("xpu")
        except Exception:
