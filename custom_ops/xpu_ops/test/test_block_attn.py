...
# block_attn_fused is deprecated and should be deleted in the future
...
def run_block_attn(
    ...
    rotary_embs = paddle.uniform(shape=[2, 1, 8192, 1, head_dim], dtype="float32", min=-1.0, max=1.0, seed=seed)
...
def run_compare_block_attn(
    ...
    for mode in [0, 1]:
        ...
        for idx, seqlen in enumerate(seq_len_list):
            ...
            print(
                f"runnning block attention of mode {mode_name[mode]} ({branch_name}), is_prefix_cache: {hit_prefix_len > 0}, kvcache type {kvcache_dtype}, has_zp={has_zp}"
            )
