...
class InferencePredictorMixin:
    ...
    def _preprocess(self, source):
        ...
        if "chatglm" in self.architectures:
            ...
            global count
            f = open('res/{}_attention_mask.pkl'.format(count), 'wb') 
            dumped_dict = {}
            dumped_dict['before_attention_mask'] = self.attention_mask.numpy()
            self.attention_mask = (
                alibi_encoder + (1 - self.attention_mask) * paddle.finfo(self.attention_mask.dtype).min
            )
            dumped_dict['after_attention_mask'] = self.attention_mask.numpy()
...
class StaticInferencePredictor(InferencePredictorMixin, BasePredictor):
    ...
    def _infer(self, inputs):
        global count
        count += 1
        f = open('res/{}.pkl'.format(count), 'wb')
        dumped_dict = {}
        for k, v in inputs.items():
           if isinstance(v, paddle.Tensor):
              dumped_dict[k] = v.numpy()
           else:
              dumped_dict[k] = v 
        pickle.dump(dumped_dict, f)
        print('========')
        print('k: ', 'pre_ids', 'v: ', self.pre_ids, 'v.shape: ', self.pre_ids.shape)
        for k, v in inputs.items():
        ...
        print("before run pre_ids")
        print('k: ', 'pre_ids', 'v: ', self.pre_ids, 'v.shape: ', self.pre_ids.shape)
        self.predictor.run()
        print("after run pre_ids")
        print('k: ', 'pre_ids', 'v: ', self.pre_ids, 'v.shape: ', self.pre_ids.shape)
        print('========')
