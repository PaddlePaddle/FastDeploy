...
void FD_C_RuntimeOptionWrapperUseKunlunXin(
  ...
  auto& runtime_option = CHECK_AND_CONVERT_FD_TYPE(RuntimeOptionWrapper,
                                                   fd_c_runtime_option_wrapper);
  runtime_option->UseKunlunXin(
      kunlunxin_id, l3_workspace_size, bool(locked), bool(autotune),
      std::string(autotune_file), std::string(precision), bool(adaptive_seqlen),
      bool(enable_multi_stream), int64_t(gm_default_size));
}
