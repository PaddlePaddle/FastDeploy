...
class GCUModelRunner(ModelRunnerBase):
    ...
    def __init__(
        ...
        os.environ["INFERENCE_MSG_QUEUE_ID"] = str(
            self.local_rank + int(self.scheduler_config.engine_worker_queue_port)
        )
    ...
    def _dummy_prefill_inputs(self, num_tokens: int, batch_size: int, expected_decode_len: int):
        ...
        full_length = min(num_tokens // batch_size, self.scheduler_config.max_model_len - max_dec_len)
    ...
    def _init_share_inputs(self, max_num_seqs: int):
        ...
        self.share_inputs["input_ids"] = paddle.full(
            [max_num_seqs, self.scheduler_config.max_model_len], self.model_config.pad_token_id, dtype="int64"
        )
        ...
        tmp_position_ids = paddle.arange(self.scheduler_config.max_model_len).reshape((1, -1))
        ...
        # Set block tables
        pre_max_block_num = (
            self.scheduler_config.max_model_len + self.cache_config.block_size - 1
        ) // self.cache_config.block_size + self.cache_config.enc_dec_block_num
    ...
    def _dummy_run(
        ...
        while True:
            ...
            hiddden_states = rebuild_padding(
                ...
                if self.speculative_decoding
                else None,  # speculative decoding requires
                self.scheduler_config.max_model_len,
            )
            ...
            if not self.speculative_decoding:
                ...
                self.sampler(logits, self.sampling_metadata, self.scheduler_config.max_model_len, self.share_inputs)
                sampler_output = None
                if self.parallel_config.tensor_parallel_size > 1:
    ...
    def capture_model(self) -> None:
        ...
        for batch_size in sorted(capture_sizes, reverse=True):
            self._dummy_run(
                num_tokens=self.scheduler_config.max_model_len,
                batch_size=batch_size,
                in_capturing=True,
                expected_decode_len=expected_decode_len,
            )
    ...
    def update_share_input_block_num(self, num_gpu_blocks: int) -> None:
        ...
        # Reset free list
        free_list = list(
            range(self.num_gcu_blocks - 1, int(self.num_gcu_blocks * self.cache_config.kv_cache_ratio) - 1, -1)
        )
        self.free_list_len = len(free_list)
