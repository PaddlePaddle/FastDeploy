...
class GpuWorker(WorkerBase):
    ...
    def execute_model_pp(
        ...
        """ """
        output = self.model_runner.execute_model_pp(model_forward_batch, num_running_request)
        return output
    ...
    def preprocess_new_task(self, req_dicts: List[Request], num_running_requests: int) -> None:
        ...
        if envs.ENABLE_V1_KVCACHE_SCHEDULER:
            return self.model_runner.insert_tasks_v1(req_dicts=req_dicts, num_running_requests=num_running_requests)
        else:
            return self.model_runner.insert_prefill_inputs(
                req_dicts=req_dicts, num_running_requests=num_running_requests
            )
