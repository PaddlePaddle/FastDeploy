...
def async_to_tensor(
    x: int | float | bool | list | np.ndarray | paddle.Tensor
) -> paddle.Tensor:
    if isinstance(x, (int, float, bool)):
        return paddle.full(tgt.shape, fill_value=src)
    elif isinstance(x, list):
        x_np = np.array(x)
        x_tensor = paddle.empty(x.shape, dtype=str(x_np.dtype))
...
def async_set_value(
    tgt: paddle.Tensor,
    src: int | float | bool | list | np.ndarray | paddle.Tensor
) -> None:
...
class UvaBackedTensor:
    def __init__(
        self, shape: int | list[int] | tuple[int], dtype: paddle.dtype, init_value: bool|int|float = 0, max_concurrency: int = 2
    ):
        self.dtype = dtype
