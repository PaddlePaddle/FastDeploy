...
from fastdeploy.worker.gpu.buffer_utils import (
    async_to_tensor,
    UvaBackedTensor,
    StagedWriteTensor
)
...
class BlockTable:
    def __init__(
        ...
        self.block_table = StagedWriteTensor(
            (self.max_num_seqs, self.max_num_blocks),
            dtype=paddle.int32,
            init_value=-1
        )
        ...
        self.input_block_table = paddle.full(
            self.block_table.shape, -1, dtype=paddle.int32
        )
        self.slot_mappings = paddle.full(
            self.max_num_batched_tokens, 0, dtype=paddle.int32,
        )
