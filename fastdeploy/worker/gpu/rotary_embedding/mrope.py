...
def compute_cos_sin_cache(
    ...
    """
    inv_freq = 1.0 / (
        base ** (paddle.arange(0, rotary_dim, 2, dtype="float32") / rotary_dim)
    )
...
class MRotaryEmbedding:
    ...
    def _apply_interleaved_rope(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        """
        x_t = x[0].clone()
        x_t[..., 1 : self.mrope_section[1] * 3 : 3] = x[
            1, ..., 1 : self.mrope_section[1] * 3 : 3
        ]
        x_t[..., 2 : self.mrope_section[2] * 3 : 3] = x[
            2, ..., 2 : self.mrope_section[2] * 3 : 3
        ]
        return x_t
