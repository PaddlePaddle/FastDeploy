...
from fastdeploy.worker.gpu.buffer_utils import StagedWriteTensor
...
class SamplingState:
    def __init__(
        ...
        self.bad_word_token_ids = StagedWriteTensor(
            (max_num_seqs, bad_words_max_len * max_bad_words_num), dtype=paddle.int32
        )
        self.bad_word_offsets = StagedWriteTensor((max_num_seqs, max_bad_words_num + 1), dtype=paddle.int32)
        ...
        self.stop_token_ids = StagedWriteTensor(
            (max_num_seqs, max_stop_seqs_num * stop_seqs_max_len), dtype=paddle.int32
        )
    ...
    def add_request(self, req_idx: int, request) -> None:
        ...
        self.use_penalty[req_idx] = repetition_penalty != 1.0 or frequency_penalty != 0.0 or presence_penalty != 0.0
