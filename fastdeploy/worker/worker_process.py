...
class PaddleDisWorkerProc:
    ...
    def __init__(self, fd_config: FDConfig, ranks: int = 1, local_rank: int = 0) -> None:
        ...
        task_address = (
            self.scheduler_config.master_ip,
        ...
        while True:
            ...
            if self.local_rank % mp_num_per_node == 0:
                if self.task_queue.num_tasks() > 0:
                    ...
                    if not self.fd_config.multi_modal_config.enable_mm or not self.worker.prefill_finished():
                        if self.nnode > 1:
...
def initialize_fd_config(args, ranks: int = 1, local_rank: int = 0) -> FDConfig:
    ...
    # Set VL tag
    multi_modal_config.enable_mm = args.enable_mm
