...
class NPUModelRunner(ModelRunnerBase):
    ...
    def __init__(
        ...
        self.device_id = device_id  # FIXME
        ...
        # Cuda Graph
        # self.use_cuda_grpah = False  # remove
    ...
    def insert_prefill_inputs(self, req_dicts: List[Request], num_running_requests: int = None):
        ...
        if (
            req_dicts[-1].disaggregate_info is not None
            and req_dicts[-1].disaggregate_info["role"] == "prefill"
        ):
        ...
        for i in range(req_len):
            ...
            # Is Decode Node
            if (
                req_dicts[i].disaggregate_info is not None
                and req_dicts[i].disaggregate_info["role"] == "decode"
            ):
                self.share_inputs["pre_ids"][idx : idx + 1] = request.prompt_token_ids[
                    -1
                ]
                self.share_inputs["input_ids"][idx : idx + 1, 0] = (
                ...
                if self.cache_config.enable_chunked_prefill:
                    ...
                    self.share_inputs["seq_lens_decoder"][idx : idx + 1] = request.get(
                        "seq_lens_decoder", 0
                    )
                    self.share_inputs["step_seq_lens_decoder"][idx : idx + 1] = (
            ...
            self.share_inputs["eos_token_id"][:] = np.array(
                request.eos_token_ids, dtype="int64"
            ).reshape(-1, 1)
            ...
            self.share_inputs["first_token_ids"][idx : idx + 1] = self.share_inputs[
                "input_ids"
            ][idx : idx + 1, :1]
            ...
            self.share_inputs["block_tables"][idx : idx + 1, :encoder_block_num] = (
                np.array(request.block_tables, dtype="int32")
            )

            if (
                request.get("stop_token_ids") is not None
                and request.get("stop_seqs_len") is not None
            ):
                ...
                self.share_inputs["stop_seqs"][
                    :stop_seqs_num, : len(request.get("stop_token_ids")[0])
    ...
    def _init_share_inputs(self, max_num_seqs: int):
        ...
        self.share_inputs["top_p"] = paddle.full(
            [max_num_seqs, 1], self.model_config.top_p, dtype="float32"
        )
        self.share_inputs["top_k"] = paddle.full(
            [max_num_seqs, 1], 0, dtype="int64"
        )
        self.share_inputs["min_p"] = paddle.full(
            [max_num_seqs, 1], 0.0, dtype="float32"
        )
        ...
        self.share_inputs["system_lens"] = paddle.full(
            [max_num_seqs, 1], 0, dtype="int32"
        )
        self.share_inputs["system_ids"] = paddle.full(
            [max_num_seqs, 1], -1, dtype="int32"
        )

        # Initialize rotary position embedding
        tmp_position_ids = paddle.arange(self.parallel_config.max_model_len).reshape(
            (1, -1)
        )
        ...
        self.share_inputs["free_list_len"] = paddle.full(
            [1], self.free_list_len, dtype="int32"
        )
        ...
        self.share_inputs["cum_offsets"] = paddle.full(
        ...
        self.share_inputs["cu_seqlens_q"] = paddle.full(
            [self.parallel_config.max_num_seqs + 1], 
            0, 
            dtype="int32"
        )
        self.share_inputs["cu_seqlens_k"] = paddle.full(
            [self.parallel_config.max_num_seqs + 1], 
            0, 
            dtype="int32"
        )
        ...
        self.share_inputs["max_len_tensor_cpu"] = paddle.full(
            [1], 
    ...
    def initialize_kv_cache(self,
                            kv_cache_config = None) -> None:   # FIXME
        """
        Initialize kv cache
        Args:
            kv_cache_config:
    ...
    def execute_model(
        ...
        model_output = self.model(
            self.share_inputs["ids_remove_padding"], self.forward_meta
        )
    ...
    def profile_run(self) -> None:
        ...
        logger.warn("NPU not support profile currently")
    ...
    def update_share_input_block_num(self, num_gpu_blocks: int) -> None:
        ...
        # if not (self.parallel_config.enable_prefix_caching \  # remove
        #         or self.parallel_config.splitwise_role != "mixed"):
        #     self.initialize_kv_cache()
        self.initialize_kv_cache()  # FIXME
        ...
        # self.parallel_config.do_profile = False  # remove
