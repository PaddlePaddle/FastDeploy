...
"""
Lower-triangular matrix inversion Triton Kernel.

Porting notes:
  - torch.empty(..., device=A.device, dtype=torch.float) → paddle.empty([...], dtype=paddle.float32)
  - torch.float → paddle.float32
  - Triton kernel code is unchanged (all three kernels retained as-is)
"""
...
def solve_tril(
    ...
    """
    Compute the inverse of a strictly lower-triangular matrix: (I + A)^{-1}.

    Args:
        A: [B, T, H, K], where K ∈ {16, 32, 64}
        cu_seqlens: cumulative sequence lengths [N+1] for varlen mode
        output_dtype: output dtype (defaults to float32)

    Returns:
        (I + A)^{-1}, same shape as A
    """
    assert A.shape[-1] in [16, 32, 64], f"BT must be 16/32/64, got {A.shape[-1]}"

    B, T, H, BT = A.shape
    out_dtype = output_dtype if output_dtype is not None else paddle.float32
    # Ad: 16x16 block inverses (intermediate result)
    Ad_dtype = paddle.float32 if BT != 16 else out_dtype
    Ad = paddle.empty([B, T, H, 16], dtype=Ad_dtype)  # 16x16 block inverses (intermediate)
    ...
    Ai = paddle.empty([B, T, H, BT], dtype=out_dtype)
