# Adapt from https://github.com/fla-org/flash-linear-attention/blob/main/fla/ops/common/chunk_o.py
...
"""
GDN chunk output computation Triton Kernel.

Porting notes:
  - torch.zeros_like(v) → paddle.zeros_like(v)
...
def chunk_fwd_o(
    ...
    """
    Chunk output computation (last step of Prefill).

    Args:
        q: [B, T, Hg, K]
        k: [B, T, Hg, K]
        v: [B, T, H, V] — updated value vectors (from chunk_gated_delta_rule_fwd_h)
        h: [B, NT, H, K, V] — initial state at each chunk
