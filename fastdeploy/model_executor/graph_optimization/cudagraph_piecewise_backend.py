...
class CudaGraphPiecewiseBackend:
    ...
    def __init__(
        ...
        #
        # NOTE: Paddle PR #78652 fixed c10::cuda::getCurrentCUDAStream() to read
        # directly from Paddle's GPUContext, so paddle.device.stream_guard() is
        # sufficient -- no manual c10 TLS synchronization needed.
    ...
    def run_static_model(self, entry: ConcreteSizeEntry, **kwargs):
        ...
        if not entry.captured:
            ...
            with paddle.device.stream_guard(self._capture_stream):
                # Warmup the model
        ...
        # Replay (same approach as SGLang: bare replay, no stream guard).
        # CUDAGraph::Replay() uses the stream saved during capture (stream_)
        # internally via cudaGraphLaunch(), so the graph is always launched on
        # the capture stream regardless of the current stream.
        # NOTE: do NOT wrap replay with stream_guard -- doing so would switch
        # Paddle's GPUContext to the capture stream, causing subsequent
        # non-graph operations (sampler, post-processing) to run on the wrong
        # stream and produce incorrect results.
    ...
    def __call__(self, **kwargs) -> List[paddle.Tensor] | paddle.Tensor:
        ...
        # Replay (same approach as SGLang: bare replay, no stream guard).
        # CUDAGraph::Replay() internally uses cudaGraphLaunch() on the stream
        # saved during capture. No stream_guard needed -- wrapping replay
        # with stream_guard would switch Paddle's GPUContext to the capture
        # stream, causing subsequent non-graph operations to run on the wrong
        # stream and produce incorrect (repetitive) output.
