...
class SD3JointTransformerBlock(nn.Layer):
    ...
    Unlike Flux's double-stream which shares QK norm, SD3 uses a simpler design:
...
class SD3Transformer2DModel(nn.Layer):
    ...
    def _get_positional_encoding(self, h: int, w: int) -> paddle.Tensor:
        """Crop positional encoding for the given spatial size.
        ...
        pos = pos[:, :h, :w, :].reshape([1, h * w, self.inner_dim])
        return pos
