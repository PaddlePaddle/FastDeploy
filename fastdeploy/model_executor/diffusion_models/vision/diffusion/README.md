...
FastDeploy Diffusion Models 是 PaddlePaddle 生态中的高性能扩散模型部署框架，提供 Stable Diffusion、Stable Diffusion 3 和 Flux 模型的完整推理支持。该框架基于 PaddlePaddle 的推理系统，集成了 CINN 优化、TensorRT 加速和混合精度等先进技术。
...
- **Stable Diffusion 3**：基于 MMDiT 的先进架构
...
#### Stable Diffusion 3 模型推理

```python
from fastdeploy.model_executor.diffusion_models.vision.diffusion import DiffusionConfig, SD3Pipeline
...
image.save("sd3_image.png")
```

#### Flux 模型推理

```python
from fastdeploy.model_executor.diffusion_models.vision.diffusion import DiffusionConfig, FluxPipeline
...
Stable Diffusion 3 推理 pipeline，基于 MMDiT 架构。
...
#### SD3 特性
- **MMDiT 架构**：多模态 Diffusion Transformer
- **双编码器**：CLIP-L/14 + T5-XXL 文本编码
- **高质量生成**：更好的图像质量和细节
- **高效推理**：优化的计算图和内存使用
...
### Stable Diffusion 3 优化

1. **MMDiT 融合** (`SD3MMDiTFusePass`)
   - 融合多模态注意力机制
   - 优化 CLIP 和 T5 条件注入
   - 加速 MMDiT 计算图

2. **双编码器优化** (`SD3DualEncoderFusePass`)
   - 优化 CLIP-L/14 和 T5-XXL 编码器
   - 融合多模态特征表示
   - 加速文本条件处理

3. **Flow 匹配优化** (`SD3FlowMatchFusePass`)
   - 优化 rectified flow 采样
   - 加速时间步调度
   - 改进生成质量
...
| 特性 | Stable Diffusion | Stable Diffusion 3 | Flux |
|------|------------------|-------------------|------|
| 架构 | U-Net | MMDiT | DiT |
| 文本编码器 | CLIP | CLIP-L/14 + T5-XXL | T5-XXL |
| 采样方法 | DDPM | Flow Matching | Rectified Flow |
| 参数量 | ~1B | ~8B | ~12B |
| 图像质量 | 良好 | 优秀 | 卓越 |
| 推理速度 | 最快 | 中等 | 最慢 |
| 内存使用 | 最低 | 中等 | 最高 |
...
### Stable Diffusion 3 使用指南

#### 模型准备
```bash
# 下载 SD3 模型权重
# 需要 CLIP-L/14、T5-XXL 和 MMDiT 三个组件
wget https://huggingface.co/stabilityai/stable-diffusion-3-medium/resolve/main/sd3_medium.safetensors
wget https://huggingface.co/stabilityai/stable-diffusion-3-medium/resolve/main/clip_l.safetensors
wget https://huggingface.co/stabilityai/stable-diffusion-3-medium/resolve/main/t5_xxl.safetensors
```
...
config = DiffusionConfig(
    ...
    height=1024,
    width=1024,
    num_inference_steps=28,  # SD3 推荐步数
    guidance_scale=7.0,     # SD3 推荐引导尺度
...
#### 提示词技巧
- **正面提示**：使用详细的描述，如 "A photorealistic portrait of a cat wearing sunglasses on a beach, golden hour lighting"
- **负面提示**：避免不需要的元素，如 "blurry, low quality, distorted"
- **引导尺度**：推荐 5.0-9.0 范围
- **推理步数**：推荐 20-50 步
...
```bash
# 下载 Flux 模型权重
# 需要 T5-XXL 和 DiT 两个组件
wget https://huggingface.co/blackforestlabs/FLUX.1-dev/resolve/main/flux-dev.safetensors
wget https://huggingface.co/blackforestlabs/FLUX.1-dev/resolve/main/ae.safetensors
```
...
config = DiffusionConfig(
    ...
    height=1024,
    width=1024,
    num_inference_steps=28,  # Flux 推荐步数
    guidance_scale=3.5,     # Flux 推荐引导尺度
    enable_memory_optimization=True
)
...
#### 提示词技巧
- **正面提示**：Flux 对提示词非常敏感，使用自然语言描述
- **负面提示**：通常不需要，Flux 有很好的默认质量
- **引导尺度**：推荐 2.5-4.0 范围（比 SD 更低）
- **推理步数**：推荐 20-50 步
