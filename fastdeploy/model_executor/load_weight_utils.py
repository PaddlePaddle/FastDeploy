...
def load_ep_checkpoint(model_path: str, config: ModelConfig, return_numpy: bool = False):
    """
    load ep checkpoint
    """
    ...
    def get_expert_ranges(config):
        ...
        if isinstance(config.moe_num_experts, list):
            return chain(
                base_range,
                range(
                    base_range.start + config.moe_num_experts[0],
                    base_range.stop + config.moe_num_experts[0],
                ),
            )
        return base_range
...
def load_composite_checkpoint(
    ...
    if fd_config.parallel_config.use_ep and fd_config.speculative_config.model_type != "mtp":
        state_dict = load_ep_checkpoint(model_path, fd_config.model_config, return_numpy=True)
    else:
        rank_dirs = [
