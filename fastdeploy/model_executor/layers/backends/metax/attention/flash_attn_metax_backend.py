...
class MetaxFlashAttentionBackend(AttentionBackend):
    ...
    enable_ids_reorder: bool = True if FA_SPLIT_ENABLED else envs.FD_PD_REORDER
    ...
    def __init__(
        ...
        self.fa_split_enabled: bool = FA_SPLIT_ENABLED
    ...
    def init_attention_metadata(self, forward_meta: MetaxForwardMeta):
        ...
        if self.fa_split_enabled:
            self._init_attention_metadata_split(metadata, forward_meta)
        else:
            self._init_attention_metadata_mix(metadata, forward_meta)
    ...
    def forward_mixed(
        ...
        if self.fa_split_enabled:
            return self._forward_split(q, k, v, qkv, compressed_kv, k_pe, layer, forward_meta)
        else:
            return self._forward_mix(qkv, layer, forward_meta)
