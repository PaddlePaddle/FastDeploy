...
class ModelOptNvFp4FusedMoECuteDSL(QuantMethodBase):
    def __init__(self, quant_config: ModelOptNvFp4Config):
    ...
    def apply(
        ...
        """
        NVFP4 CuteDSL MoE forward (no-dispatch version for testing).

        Flow:
          1. gate + topk selection
          2. Manual token permutation → [num_local_experts, max_m, hidden] + masked_m
          3. flashinfer_cutedsl_moe_masked (FP4 quantize → GEMM1 → SiLU → GEMM2)
          4. Manual combine → scatter back with routing weights

        Note: Production path should replace steps 2&4 with DeepEP low-latency dispatch/combine.
        """
        ...
        flat_topk_ids = topk_ids.reshape([-1])  # [bs * top_k]

        # Step 2: compute masked_m (token count per expert) and permute tokens
        masked_m = paddle.zeros([num_local_experts], dtype=paddle.int32)
        for i in range(num_local_experts):
            masked_m[i] = int((flat_topk_ids == i).sum())

        max_m = int(masked_m.max())
        hidden_states_3d = paddle.zeros([num_local_experts, max_m, hidden_dim], dtype=x.dtype)
        for i in range(num_local_experts):
            count = int(masked_m[i])
            if count > 0:
                mask = flat_topk_ids == i
                hidden_states_3d[i, :count, :] = x[mask]

        # Step 3: CuteDSL masked GEMM
        # hidden_states[1] is None → scaled_fp4_grouped_quantize is called inside the kernel
        ...
        # ffn_out: [num_local_experts, max_m, hidden_dim]

        # Step 4: scatter output back and apply routing weights
        # Use a loop to handle duplicate batch_indices correctly (same sample → multiple experts)
        bs = x.shape[0]
        output = paddle.zeros([bs, hidden_dim], dtype=ffn_out.dtype)
        flat_weights = topk_weights.reshape([-1])  # [bs * top_k]
        for i in range(num_local_experts):
            count = int(masked_m[i])
            if count == 0:
                continue
            token_indices = paddle.nonzero(flat_topk_ids == i).squeeze(-1)  # [count]
            batch_indices = (token_indices // layer.top_k).tolist()
            weights_list = flat_weights[token_indices].cast(ffn_out.dtype)  # [count]
            expert_out = ffn_out[i, :count, :]  # [count, hidden_dim]
