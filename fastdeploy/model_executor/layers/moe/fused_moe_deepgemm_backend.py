...
from fastdeploy.model_executor.layers.quantization.fp8_utils import TDU, deep_gemm
...
def m_grouped_fp8_gemm_nt_contiguous_custom_python_op(
    ...
    # swiglu
    if fused_ops_spaq:
        ffn_in_x, ffn_in_x_scale_tensor = TDU.fuse_weighted_swiglu_fp8_quant(
...
class DeepGemmFusedMoeMethod(MoEMethodBase):
    ...
    def apply_tp(
        ...
        ffn_out = m_grouped_fp8_gemm_nt_contiguous_custom_python_op(
            ...
            self.quant_config.weight_block_size[0],
            disable_ue8m0_cast=not self.quant_config.deepgemm_scale_ue8m0,
            fused_ops_spaq=self.quant_config.fused_ops_spaq,
