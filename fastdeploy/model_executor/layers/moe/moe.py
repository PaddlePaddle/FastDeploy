...
class FusedMoE(nn.Layer):
    ...
    def forward_split_allgather(self, x: paddle.Tensor, gate: nn.Layer, topk_ids_hookfunc: Callable = None, shared_experts: nn.Layer = None):
        """
        ...
        out = self.quant_method.apply(self, part_x, gate, topk_ids_hookfunc=topk_ids_hookfunc, shared_experts=shared_experts)
    ...
    def forward(self, x: paddle.Tensor, gate: nn.Layer, forward_meta: ForwardMeta = None, shared_experts: nn.Layer = None):
        """
        ...
        if current_platform.is_intel_hpu():
            out = self.forward_normal(x, gate, forward_meta, topk_ids_hookfunc=topk_ids_hookfunc, shared_experts=shared_experts)
        ...
        if (
            ...
            out = self.forward_split_allgather(x, gate, topk_ids_hookfunc=topk_ids_hookfunc, shared_experts=shared_experts)
    ...
    def forward_chunked_moe(
        self, x: paddle.Tensor, gate: nn.Layer, forward_meta: ForwardMeta, topk_ids_hookfunc: Callable = None, shared_experts: nn.Layer = None
    ):
        """
        ...
        if token_num > chunk_size:  # chunked moe
            ...
            for i in range(forward_meta.max_moe_num_chunk):
                if i < forward_meta.moe_num_chunk:
                    ...
                    self.quant_method.apply(self, fake_x, gate, topk_ids_hookfunc=topk_ids_hookfunc, shared_experts=shared_experts)

            out = paddle.concat(out_split_list, axis=0)
        else:
            # when only one chunk, just need to use real data to infer once.
            out = self.quant_method.apply(self, x, gate, topk_ids_hookfunc=topk_ids_hookfunc, shared_experts=shared_experts)
