...
from .indexer import TSAIndexer
...
class TokenSparseAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        #page
        self.key_new = paddle.zeros(shape=[(fd_config.scheduler_config.max_num_seqs+10)*((self.tsa_index_topk)//self.block_size), self.kv_num_heads, self.block_size, self.head_dim], dtype="bfloat16")
        self.value_new = paddle.zeros(shape=[(fd_config.scheduler_config.max_num_seqs+10)*((self.tsa_index_topk)//self.block_size), self.kv_num_heads, self.block_size, self.head_dim], dtype="bfloat16")
    ...
    def indexer_extend_forward(self, forward_meta: ForwardMeta, query_states, key_states):
        ...
        # #==============================
        k_token_num = key_states.shape[0]
        k_div = k_token_num // k_share_tokens
        k_extra = k_token_num - k_div*k_share_tokens
        k_pack_seq = (k_token_num+k_share_tokens-1) // k_share_tokens
        # k = paddle.zeros([k_pack_seq,self.kv_num_heads,self.head_dim], dtype=key_states.dtype)
        # k[:k_div] = paddle.mean(
        #     key_states[:k_div*k_share_tokens].reshape(-1,k_share_tokens,self.kv_num_heads,self.head_dim),
        #     axis=1,
        # ).reshape(-1,self.kv_num_heads,self.head_dim)
        # if k_extra > 0:
        #     k[-1:] = paddle.mean(key_states[k_div*k_share_tokens:],axis=0,keepdim=True)
        # #==============================

        k = paddle.nn.functional.avg_pool1d(
            key_states.transpose([1, 2, 0]).contiguous(), 
            kernel_size=k_share_tokens, 
            stride=k_share_tokens, 
            ceil_mode=True
        )#.transpose([2, 0, 1])
        ...
        # score[..., -1] = 1e10
        # score[..., :64//k_share_tokens] = 1e10
        
        # #==============================
        q_token_num = query_states.shape[0]
        q_div = q_token_num // q_share_tokens
        q_extra = q_token_num - q_div*q_share_tokens
        # score_qzip = paddle.zeros([self.kv_num_heads,(q_token_num+q_share_tokens-1) // q_share_tokens,k_pack_seq])

        # score_qzip[:,:q_div,:] = paddle.max(
        #     score[:,:q_div*q_share_tokens,:].reshape(self.kv_num_heads,-1,q_share_tokens,k_pack_seq),
        #         axis=2,
        #     ).reshape(self.kv_num_heads,-1,k_pack_seq)
        # if q_extra > 0:
        #     score_qzip[:,-1:,:] = paddle.max(score[:, q_div*q_share_tokens:, :],axis=1,keepdim=True)
        # score_qzip = score_qzip.transpose([1,0,2])#.reshape([-1,k_pack_seq])
        # #==============================
        ...
        score_qzip = paddle.nn.functional.max_pool1d(
            ...
            kernel_size=q_share_tokens, 
            stride=q_share_tokens, 
            ceil_mode=True
        ).transpose([2, 0, 1]).reshape([-1,k_pack_seq])


        # paddle.max(score.transpose([0, 2, 1]).contiguous()[:,:,:-1].reshape(4,2149,-1,8),axis=-1)
        # score_qzip1 = paddle.nn.functional.max_pool1d(score.transpose([0, 2, 1]).contiguous(), kernel_size=q_share_tokens, stride=q_share_tokens, ceil_mode=True).transpose([2, 0, 1]).reshape([-1,k_pack_seq])


        breakpoint()

        token_sparse_index_packk = paddle.full([score_qzip.shape[0], self.tsa_index_topk//k_share_tokens], -1,dtype='int32')

        from fastdeploy.model_executor.ops.gpu import radix_topk_ragged_transform
        ks,ke = forward_meta.attn_mask_offsets[::2],forward_meta.attn_mask_offsets[1::2]
        
        mask = paddle.zeros([score_qzip.shape[0]//self.kv_num_heads],dtype="int32")
        mask[:q_div] = (ke[:q_div*q_share_tokens].reshape([-1,q_share_tokens])[:,-1] +k_share_tokens-1)//k_share_tokens
        mask[-1] = (ke[-1]+k_share_tokens-1)//k_share_tokens
        # mask = mask.reshape(-1,1).tile([4]).reshape(-1)


        radix_topk_ragged_transform(
            score_qzip, 
            token_sparse_index_packk, 
            ks.contiguous(),#self.offsets,
            mask.contiguous(),#self.lengths,
            None,#forward_meta.seq_lens_decoder,
            None,#forward_meta.batch_id_per_token,
            None,#self.buffer
            self.tsa_index_topk//k_share_tokens
        )
    
        # offsets = paddle.arange(k_share_tokens,dtype="int32")
        token_sparse_index = (
            token_sparse_index_packk * k_share_tokens
        ).unsqueeze(-1) +  paddle.arange(k_share_tokens,dtype="int32")

        # breakpoint()
        # tmp = paddle.topk(score_qzip, self.tsa_index_topk//k_share_tokens,axis =-1)[1]
    ...
    def forward_mixed(
        ...
        if forward_meta.max_len_tensor_cpu[1] > 0:
            ...
            # breakpoint()
            # q = q.reshape(-1,self.num_heads//4,4,self.head_dim)
            # k = k.reshape(8,-1,self.num_key_value_heads,self.head_dim)
            # breakpoint()
            # q = paddle.mean(q, axis=1).reshape(-1,4,self.head_dim)
            # k = paddle.max(k,axis=0).reshape(-1,4,self.head_dim)
            # breakpoint()
