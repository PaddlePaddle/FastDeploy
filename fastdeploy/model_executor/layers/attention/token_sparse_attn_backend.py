...
class TokenSparseAttnBackend(AttentionBackend):
    ...
    def indexer_extend_forward(self, forward_meta: ForwardMeta, query_states, key_states):
        ...
        with nvtx.annotate("gemm_q*k", color="green"):
            # breakpoint()
            score = paddle.matmul(q.transpose([1,0,2]),k)
            # score = paddle.einsum("shd,hdm->hsm", q, k)
            score[..., -1] = 1e10
            score[..., :min(score.shape[-1],64//self.k_share_tokens)] = 1e10
        ...
        with nvtx.annotate("trick_-1", color="blue"):
            ...
            token_sparse_index_packk[token_sparse_index_packk == -1 ] = 1e5
    ...
    def indexer_decode_forward(self, forward_meta: ForwardMeta, query_states, key_states):
        if True:
        # with nvtx.annotate("pack_q_head", color="blue"):
            q = query_states.reshape(-1,self.kv_num_heads,self.num_heads//4,self.head_dim)
            q = paddle.mean(q, axis=2).reshape(-1,self.kv_num_heads,self.head_dim)
        ...
        # with nvtx.annotate("pack_k_token", color="yellow"):
            # breakpoint()
            k = paddle.nn.functional.avg_pool1d(
                key_states[:max(1,(forward_meta.seq_lens_decoder+1+63)//64),:,:,:].transpose([1, 3, 0, 2]).reshape([self.kv_num_heads,self.head_dim, -1])[:,:,:forward_meta.seq_lens_decoder+1].contiguous(), 
                kernel_size=self.k_share_tokens, 
                stride=self.k_share_tokens, 
                ceil_mode=True
            )#.transpose([2, 0, 1])
        ...
        # with nvtx.annotate("gemm_q*k", color="green"):
            ...
            score[..., -1] = 1e10
            score[..., :min(score.shape[-1],64//self.k_share_tokens)] = 1e10
            # from .mha_page_logits import indexer_mha_page_logits
            # index_score_fast = indexer_mha_page_logits(
            #     q.contiguous(),
            #     self.indexer_cache,
            #     weights,
        ...
        # with nvtx.annotate("topk", color="yellow"):
            token_sparse_index_packk = paddle.full([score.shape[0], self.tsa_index_topk//self.k_share_tokens], -1,dtype='int32')

            from fastdeploy.model_executor.ops.gpu import radix_topk_ragged_transform
            ks,ke = forward_meta.attn_mask_offsets[::2].contiguous(),forward_meta.attn_mask_offsets[1::2].contiguous()

            radix_topk_ragged_transform(
                score.transpose([1,0,2]).reshape(-1,score.shape[-1]).contiguous(), 
                token_sparse_index_packk,
                self.offsets,
                self.lengths,
                ((forward_meta.seq_lens_decoder + self.k_share_tokens) // self.k_share_tokens),
        ...
        # token_sparse_index = (token_sparse_index_packk * self.k_share_tokens).unsqueeze(-1) + paddle.arange(self.k_share_tokens,dtype="int32")
        # tmp = paddle.topk(score, self.tsa_index_topk//self.k_share_tokens,axis =-1)[1]
        # paddle.max(paddle.sort(token_sparse_index_packk,axis=-1)-paddle.sort(paddle.topk(score, self.tsa_index_topk//self.k_share_tokens,axis =-1)[1],axis=-1).reshape([-1,512]).cast("int32"))
        # paddle.set_printoptions(precision=4, threshold=160, edgeitems=40, sci_mode=None, linewidth=80)
        # token_sparse_index = paddle.repeat_interleave(token_sparse_index,[1,4])
    ...
    def forward_mixed(
        ...
        if forward_meta.max_len_tensor_cpu[1] > 0:
            ...
            if (os.getenv("FD_ATTENTION_BACKEND", False)) == 'TSA_ATTN' and layer.layer_id > 0:
                # if True:
        ...
        if  forward_meta.max_len_tensor_cpu[2]>0: 
            if (os.getenv("FD_ATTENTION_BACKEND", False)) == 'TSA_ATTN' and layer.layer_id > 0:
                with nvtx.annotate("rope_norm_cache", color="green"):
                    gqa_decoder_rope_norm_with_write_cache(
                        qkv,
                        forward_meta.caches[3 * layer.layer_id],
                        forward_meta.caches[3 * layer.layer_id + 1],
                ...
                token_sparse_index = self.indexer_decode_forward(forward_meta,qkv[-1,:self.num_heads*self.head_dim],forward_meta.caches[3 * layer.layer_id])
                with nvtx.annotate("write_cache", color="yellow"):
                ...
                with nvtx.annotate("full attention", color="red"):
                    res_decoder = append_attention(
                        qkv,
                        forward_meta.caches[3 * layer.layer_id],
                        forward_meta.caches[3 * layer.layer_id + 1],
