...
def prefill_qk_varlen(
    ...
    Args:
        ...
        q_share_tokens: Number of shared tokens in Q dimension (default: 8)
    
    Returns:
        Output tensor of shape 
            [total_tokens, num_heads, max_seqlen_k] in q_share_tokens is None or not set
            [(total_tokens+q_share_tokens-1)//q_share_tokens, num_heads, max_seqlen_k ] in q_share_tokens is set
    ...
    if q_share_tokens > 0:
        out = paddle.zeros(
            ((total_q+q_share_tokens-1)//q_share_tokens, num_heads, max_seqlen_k),
            device=q.device,
            dtype=q.dtype
        )
    else:
    ...
    _prefill_qk_kernel[grid](
        ...
        BLOCK_M=block_m,
        BLOCK_N=block_n,
        Q_SHARED_TOKENS=q_share_tokens,
        num_warps=num_warps,
        num_stages=2,
    )
