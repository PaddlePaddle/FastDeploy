...
class TritonMLAAttentionBackend(AttentionBackend):
    ...
    def init_attention_metadata(self, forward_meta: ForwardMeta):
        ...
        # max_seqlen_k must include cached tokens for chunked prefill / prefix caching.
        metadata.max_seqlen_k = max(
            int(metadata.max_kv_len_this_time.item()),
            int(metadata.max_enc_len_this_time.item()),
        )
        ...
        #
        # IMPORTANT: q for decode is built by extract_decoder_token_from_q which produces
        # a tensor of shape [max_num_seqs, hidden_dim] indexed by ORIGINAL batch_id (with
        # garbage for non-decode batches). Therefore kv_indptr/kv_indices MUST follow the
        # same full max_num_seqs layout (not mask-compressed) so that batch i in q is
        # paired with batch i's KV cache indices. For non-decode batches we leave a
        # zero-length range so the kernel produces a zeroed/no-op output for those slots.
        ...
        if decode_bs > 0:
            # Full-layout per-batch KV lengths: real length for decode batches, 0 otherwise.
            full_seq_lens = paddle.where(
                decode_mask,
                seq_lens_decoder + seq_lens_this_time,
                paddle.zeros_like(seq_lens_decoder),
            ).cast("int32")
            total_kv_len = int(paddle.sum(full_seq_lens).item())
            ...
            # num_kv_splits in full layout: real value for decode batches, 1 otherwise
            # (must be >= 1 to avoid division by zero in stage1 kernel).
            compute_num_kv_splits(full_seq_lens, max_num_seqs, self.max_kv_splits, out_buf=self._num_kv_splits_buf)
            # Force padded/non-decode entries to 1 (compute_num_kv_splits may return 0 for len=0)
            self._num_kv_splits_buf[:] = paddle.where(
                self._num_kv_splits_buf > 0,
                self._num_kv_splits_buf,
                paddle.ones_like(self._num_kv_splits_buf),
            )
        else:
    ...
    def forward_mixed(
        ...
        # Decode branch within mixed mode. q is in full max_num_seqs layout (built by
        # extract_decoder_token_from_q upstream). kv_indptr/kv_indices are built in the
        # same full layout, so non-decode batches naturally get 0-length KV ranges and
        # produce no-op output, which insert_decoder_result_back then ignores.
        metadata = self.attention_metadata
        latent_cache = forward_meta.caches[layer.layer_id] if hasattr(forward_meta, "caches") else None
