...
class FlashInferAttentionBackend(AttentionBackend):
    def __init__(
        ...
        self._workspace_buffer = paddle.empty(
            [394 * 1024 * 1024],
            dtype=paddle.uint8
        )
        ...
        max_num_blocks_per_req = cdiv(
            fd_config.model_config.max_model_len, fd_config.cache_config.block_size
        )
        self.max_num_blocks = self.max_num_seqs * max_num_blocks_per_req
        self.paged_kv_indptr = self._make_buffer(self.max_num_seqs + 1)
        self.paged_kv_indptr_cpu_buffer = paddle.zeros_like(
    ...
    def init_attention_metadata(self, forward_meta: ForwardMeta):
        ...
        seq_lens = forward_meta.input_batch.seq_lens
        seq_lens_np = forward_meta.input_batch.seq_lens_np
        qo_indptr = forward_meta.input_batch.query_start_loc
    ...
    def _make_buffer(
        self, *size: int, dtype: paddle.dtype = paddle.int32
    ) -> CpuGpuBuffer:
        return CpuGpuBuffer(
            *size,
            dtype=dtype,
    ...
    def _compute_flashinfer_kv_metadata(
        ...
        self.paged_kv_indptr_cpu_buffer[: num_seqs + 1] = self.paged_kv_indptr.cpu[
            : num_seqs + 1
        ]
        paged_kv_indptr = self.paged_kv_indptr.gpu[: num_seqs + 1]
        paged_kv_indptr.copy_(
            self.paged_kv_indptr_cpu_buffer[: num_seqs + 1], non_blocking=True
        )
        ...
        self.paged_kv_last_page_len.gpu[:num_seqs].copy_(
            self.paged_kv_last_page_len.cpu[:num_seqs], non_blocking=True
        )
