...
def append_attention_with_output(
    ...
    linear_smooth: Optional[paddle.Tensor] = None,
    kv_signal_data: Optional[paddle.Tensor] = None,
    fmha_out: Optional[paddle.Tensor] = None,
    compute_type: str = "bf16",
    cache_quant_type: str = "none",
    use_neox_rotary_style: bool = False,
    rope_3d: bool = False,
    ...
    append_attention
    """
    ## fmha_output can't be None in `append_attention_with_output`
    assert fmha_out is not None, ValueError(
        "fmha_out must not be None when append_attention_with_output is selected.")
