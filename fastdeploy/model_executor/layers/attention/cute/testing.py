...
class IndexFirstAxis(paddle.autograd.Function):
    @staticmethod
...
class IndexPutFirstAxis(paddle.autograd.Function):
    @staticmethod
    def forward(ctx, values, indices, first_axis_dim):
        ctx.save_for_backward(indices)
        assert indices.ndim == 1
        assert values.ndim >= 2
        output = paddle.zeros(first_axis_dim, *values.shape[1:], device=values.device, dtype=values.dtype)
...
def unpad_input(hidden_states, attention_mask, unused_mask=None):
    ...
    seqlens_in_batch = all_masks.sum(dim=-1, dtype=paddle.int32)
    used_seqlens_in_batch = attention_mask.sum(dim=-1, dtype=paddle.int32)
...
def generate_random_padding_mask(max_seqlen, batch_size, device, mode="random", zero_lengths=False):
    ...
    if mode == "full":
        lengths = paddle.full((batch_size, 1), max_seqlen, device=device, dtype=paddle.int32)
    elif mode == "random":
    ...
    if zero_lengths:
        for i in range(batch_size):
            if i % 5 == 0:
                lengths[i] = 0
        lengths[-1] = 0
    padding_mask = repeat(paddle.arange(max_seqlen, device=device), "s -> b s", b=batch_size) < lengths
    return padding_mask
...
def generate_qkv(
    ...
    if query_padding_mask is not None:
        ...
        cu_seqlens_q = paddle.arange(
            0, (batch_size + 1) * seqlen_q, step=seqlen_q, dtype=paddle.int32, device=q_unpad.device
        )
        seqused_q = None
    ...
    if key_padding_mask is not None:
        ...
        cu_seqlens_k = paddle.arange(
            0, (batch_size + 1) * seqlen_k, step=seqlen_k, dtype=paddle.int32, device=k_unpad.device
        )
        seqused_k = None
    ...
    if qkvpacked:
        ...
        qkv_unpad = paddle.stack([q_unpad, k_unpad, v_unpad], dim=1)
        ...
        kv_unpad = paddle.stack([k_unpad, v_unpad], dim=1)
        kv = paddle.stack([k, v], dim=2)
...
def construct_local_mask(
    ...
    row_idx = rearrange(paddle.arange(seqlen_q, device=device, dtype=paddle.long), "s -> s 1")
    col_idx = paddle.arange(seqlen_k, device=device, dtype=paddle.long)
    ...
    if window_size[0] is None:
        ...
        if window_size[1] is None:
            ...
            local_mask_left = col_idx > paddle.minimum(row_idx + sk - sq + window_size[1], sk)
...
def construct_chunk_mask(
    ...
    return paddle.logical_or(col_idx < col_limit_left_chunk, col_idx >= col_limit_left_chunk + attention_chunk)
...
def attention_ref(
    ...
    if learnable_sink is None:
        attention = paddle.softmax(scores, dim=-1).to(v.dtype)
    else:
        scores_fp32 = scores.to(paddle.float32)
        logits_max = paddle.amax(scores_fp32, dim=-1, keepdim=True)
        ...
        normalizer = unnormalized_scores.sum(dim=-1, keepdim=True) + paddle.exp(learnable_sink - logits_or_sinks_max)
