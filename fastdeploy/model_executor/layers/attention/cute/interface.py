...
def _validate_tensor(t, name, expected_shape, expected_dtype, expected_device=None):
...
def _flash_attn_fwd(
    ...
    for t in [cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k]:
        if t is not None:
            assert t.dtype == paddle.int32, "cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k must be int32"
            assert t.strides[0] == 1, "cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k must be contiguous"
    if learnable_sink is not None:
        assert learnable_sink.shape == (num_head,)
        assert learnable_sink.dtype == paddle.bfloat16, "learnable_sink must be bfloat16"

    assert all(
        t is None or t.device.type == "cuda"
        for t in (
            q,
            k,
            v,
    ...
    if num_splits < 1:
        ...
        num_splits = num_splits_heuristic(
            total_mblocks,
            paddle.cuda.get_device_properties(device).multi_processor_count,
            num_n_blocks,
            128,
        )
    ...
    if is_split_kv:
        out_partial = paddle.empty(
            num_splits, *q_batch_seqlen_shape, num_head, head_dim_v, dtype=paddle.float32, device=device
        )
        lse_partial = paddle.empty(num_splits, *lse_shape, dtype=paddle.float32, device=device)
    ...
    if compile_key not in _flash_attn_fwd.compile_cache:
        ...
        page_table_tensor = (
            to_cute_tensor(page_table, assumed_align=4, leading_dim=1) if page_table is not None else None
        )
        ...
        if compute_capability == 9:
            ...
            raise ValueError(f"Unsupported compute capability: {compute_capability}. Supported: 9.x, 10.x, 11.x")
...
def _flash_attn_bwd(
    ...
    assert all(
        t is None or t.device.type == "cuda" for t in (q, k, v, out, dout, lse, cu_seqlens_q, cu_seqlens_k)
    ), "inputs must be on CUDA device"
    assert num_head % num_head_kv == 0, "num_head must be divisible by num_head_kv"
    assert head_dim <= 256, "head_dim must be less than or equal to 256"
    alignment = 16 // q.element_size()
    ...
    if cu_seqlens_q is None:
        ...
        dpsum = paddle.empty(batch_size, num_head, seqlen_q_rounded, dtype=paddle.float32, device=device)
        lse_log2 = paddle.empty(batch_size, num_head, seqlen_q_rounded, dtype=paddle.float32, device=device)
    else:
        total_q_rounded_padded = (total_q + cu_seqlens_q.shape[0] * m_block_size - 1) // m_block_size * m_block_size
        dq_accum = paddle.empty(
            num_head, total_q_rounded_padded * head_dim_rounded, dtype=paddle.float32, device=device
        )
        dpsum = paddle.empty(num_head, total_q_rounded_padded, dtype=paddle.float32, device=device)
        lse_log2 = paddle.empty(num_head, total_q_rounded_padded, dtype=paddle.float32, device=device)
    ...
    if deterministic:
        dQ_semaphore = paddle.zeros(
            batch_size, num_head, seqlen_q_rounded // m_block_size, 1, dtype=paddle.int32, device="cuda"
        )
    else:
        dQ_semaphore = None
    ...
    if deterministic and qhead_per_kvhead > 1:
        dK_semaphore = paddle.zeros(
            batch_size, num_head_kv, seqlen_k_rounded // n_block_size, 2, dtype=paddle.int32, device="cuda"
        )
    ...
    if compile_key_pre not in _flash_attn_bwd.compile_cache_pre:
        ...
        cu_seqlens_q_tensor, seqused_q_tensor = [
            to_cute_tensor(t, assumed_align=4) if t is not None else None for t in (cu_seqlens_q, seqused_q)
        ]
    ...
    if dKV_postprocess:
        ...
        if compile_key_post not in _flash_attn_bwd.compile_cache_post:
            ...
            cu_seqlens_k_tensor, seqused_k_tensor = [
                to_cute_tensor(t, assumed_align=4) if t is not None else None for t in (cu_seqlens_k, seqused_k)
            ]
            arch = compute_capability * 10
            fa_bwd_post = FlashAttentionBackwardPostprocess(
...
class FlashAttnFunc(paddle.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q: paddle.Tensor,
...
class FlashAttnVarlenFunc(paddle.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q: paddle.Tensor,
...
def _flash_attn_fwd_combine(
    ...
    if compile_key not in _flash_attn_fwd_combine.compile_cache:
        ...
        lse_partial_tensor = to_cute_tensor(lse_partial, assumed_align=4, leading_dim=lse_partial.ndim - 2)
        out_tensor = to_cute_tensor(out, leading_dim=3 if not is_varlen else 2)
        lse_tensor = to_cute_tensor(lse, assumed_align=4, leading_dim=lse.ndim - 2) if lse is not None else None
...
def flash_attn_combine(
    ...
    if out is None:
        if is_varlen:
            out = paddle.empty(total_q, num_heads, head_size, dtype=out_dtype, device=device)
        else:
            out = paddle.empty(batch_size, seqlen, num_heads, head_size, dtype=out_dtype, device=device)

    # Create lse output only if requested
    if return_lse:
        if is_varlen:
            lse = paddle.empty(num_heads, total_q, dtype=paddle.float32, device=device).transpose(0, 1)
        else:
            lse = paddle.empty(batch_size, num_heads, seqlen, dtype=paddle.float32, device=device).transpose(1, 2)
    else:
        lse = None
