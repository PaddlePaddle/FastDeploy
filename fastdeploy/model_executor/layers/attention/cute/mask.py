...
class AttentionMask:
    ...
    def apply_mask(
        ...
        t0ScS_mn = utils.make_acc_tensor_mn_view(thr_mma.get_slice(0).partition_C(cS), transpose=self.swap_AB)
            ...
            if const_expr(mask_causal):
                ...
                if const_expr(not r2p):
                    ...
                    for i in cutlass.range(cute.size(tScS_t2r.shape), unroll_full=True):
                        ...
                        acc_S[i] = -Float32.inf if col_idx >= col_limit_right or col_idx < col_limit_left else acc_S[i]
                else:
    ...
    def apply_mask_sm100_transposed(
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is not None):
            ...
            if const_expr(mask_causal):
                ...
                if const_expr(not r2p):
                    for i in cutlass.range(cute.size(acc_S.shape), unroll_full=True):
                        acc_S[i] = -cutlass.Float32.inf if t0ScS_t2r[i][ROW] < row_limit_top else acc_S[i]
                else:
