...
import fastdeploy.model_executor.layers.attention.cute.blackwell_helpers as sm100_utils
...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        ...
        self.tmem_o_offset = [
            self.tmem_s_offset[-1] + self.n_block_size + i * self.head_dim_v_padded for i in range(self.q_stage)
        ]  # e.g., 256, 384
    ...
    def _setup_attributes(self):
        ...
        self.uneven_kv_smem_offset = (
            self.m_block_size * (self.head_dim_padded - self.head_dim_v_padded) // 2 if self.uneven_kv_smem else 0
        )
    ...
    def __call__(
        ...
        if const_expr(not self.same_hdim_kv_padded):
            ...
            stage_stride = const_expr(
                max(stride_sK, stride_sV) if not self.uneven_kv_smem else (stride_sK + stride_sV) // 2
            )
        ...
        if const_expr(mCuSeqlensQ is not None or mSeqUsedQ is not None):
            ...
            if const_expr(self.is_causal or self.is_local):
                ...
                TileScheduler = (
                    SingleTileScheduler if const_expr(not self.is_persistent) else StaticPersistentTileScheduler
                )
    ...
    def kernel(
        ...
        tStSs = tuple(
            cute.make_tensor(tStS.iterator + self.tmem_s_offset[stage], tStS.layout) for stage in range(self.q_stage)
        )
        tOtOs = tuple(
            cute.make_tensor(tOtO.iterator + self.tmem_o_offset[stage], tOtO.layout) for stage in range(self.q_stage)
        )
    ...
    def correction_loop(
        ...
        while work_tile.is_valid_tile:
            ...
            stats = [
                (0.0, -Float32.inf if const_expr(mLSE is not None or learnable_sink is not None) else None, True)
            ] * self.q_stage
            ...
            if const_expr(mLSE is not None):
                ...
                for stage in cutlass.range_constexpr(self.q_stage):
                    ...
                    seqlen_q = (
                        seqlen.seqlen_q if const_expr(not self.pack_gqa) else seqlen.seqlen_q * self.qhead_per_kvhead
                    )
        ...
        if const_expr(self.use_correction_warps_for_epi):
            ...
            if const_expr(not self.pack_gqa):
                for rest_m in cutlass.range_constexpr(cute.size(tOrO.shape[1])):
                    if (
                        t0OcO[0, rest_m, 0][0]
                        < seqlen_q - (self.q_stage * m_block + stage) * self.m_block_size - tOcO[0][0]
                    ):
                        cute.copy(
                            gmem_tiled_copy_O,
