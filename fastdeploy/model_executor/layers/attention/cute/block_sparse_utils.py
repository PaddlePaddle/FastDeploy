...
def get_total_block_count(
    ...
    if const_expr(full_block_cnt is not None):
        return (
            mask_block_cnt[batch_idx, head_idx, m_block_sparse] + full_block_cnt[batch_idx, head_idx, m_block_sparse]
        )
    else:
...
def handle_block_sparse_empty_tile_correction_sm100(
    ...
    * satisfy the usual barrier protocol so downstream warps continue to make progress.
    ...
    for stage in cutlass.range_constexpr(q_stage):
        ...
        row_max_value = -Float32.inf if const_expr(mLSE is not None or learnable_sink is not None) else None
        if const_expr(learnable_sink is not None):
            ...
            if sink_val != -Float32.inf and (const_expr(not is_split_kv) or split_idx == 0):
                if row_max_value == -Float32.inf:
                    ...
                    row_sum_value = row_sum_value + utils.exp2f(sink_val * LOG2_E - row_max_value * softmax_scale_log2)
        ...
        if curr_full_block_cnt > 0:
            ...
            if curr_mask_block_cnt == 0:
                (
                    ...
                    full_n_block,
                    is_first=True,
                    mask_fn=partial(mask_fn_none, mask_seqlen=True, check_q_boundary=check_m_boundary),
                )
            else:
                (
                    mma_si_consumer_phase,
                    ...
                    full_n_block,
                    is_first=False,
                    mask_fn=partial(mask_fn_none, mask_seqlen=False, check_q_boundary=check_m_boundary),
                )
            for i in cutlass.range(1, curr_full_block_cnt):
                full_n_block = curr_full_block_idx[curr_full_block_cnt - 1 - i]
                (
