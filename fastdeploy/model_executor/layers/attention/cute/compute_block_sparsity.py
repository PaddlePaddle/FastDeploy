...
class BlockSparsityKernel:
    ...
    def kernel(
        ...
        @cute.struct
        class SharedStorage:
            reduction_buffer_smem: cute.struct.Align[cute.struct.MemRange[cutlass.Int8, 2 * self.num_warps], 1024]

        smem = cutlass.utils.SmemAllocator()
        storage = smem.allocate(SharedStorage, 16)

        reduction_buffer = storage.reduction_buffer_smem.get_tensor(cute.make_layout((self.num_warps, 2)))
        ...
        for n_block in cutlass.range(num_n_blocks, unroll_full=True):
            ...
            if const_expr(self.use_fast_sampling):
                ...
                warp_has_unmasked_mask = cute.arch.vote_any_sync(thread_has_unmasked & thread_is_valid)
...
def compute_block_sparsity(
    ...
    mask_block_cnt = paddle.zeros((batch_size, num_heads, num_m_blocks), device=device, dtype=paddle.int32)
    mask_block_idx = paddle.zeros(
        (batch_size, num_heads, num_m_blocks, num_n_blocks), device=device, dtype=paddle.int32
    )
    full_block_cnt = (
        paddle.zeros((batch_size, num_heads, num_m_blocks), device=device, dtype=paddle.int32)
        if compute_full_blocks
        else None
    )
    full_block_idx = (
        paddle.zeros((batch_size, num_heads, num_m_blocks, num_n_blocks), device=device, dtype=paddle.int32)
        if compute_full_blocks
        else None
    )
