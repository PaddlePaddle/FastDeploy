...
import fastdeploy.model_executor.layers.attention.cute.barrier as barrier
import fastdeploy.model_executor.layers.attention.cute.copy_utils as copy_utils
import fastdeploy.model_executor.layers.attention.cute.pipeline as pipeline
import fastdeploy.model_executor.layers.attention.cute.utils as utils

from .blackwell_helpers import gemm_ptx_w_idx, gemm_w_idx  # noqa
...
from .tile_scheduler import SingleTileLPTBwdScheduler  # noqa
...
class FlashAttentionBackwardSm100:
    ...
    def __init__(
        ...
        assert self.tile_hdim == self.tile_hdimv, "tile_hdim and tile_hdimv must be the same for now"
    ...
    def __call__(
        ...
        mLSE, mdPsum, mdQaccum = [utils.select(t, mode=LSE_dPsum_dQaccum_transpose) for t in (mLSE, mdPsum, mdQaccum)]
    ...
    def kernel(
        ...
        if const_expr(not self.dKV_postprocess):
            sdV = storage.sdO.get_tensor(sdKV_layout.outer, swizzle=sdKV_layout.inner, dtype=self.dv_dtype)
            sdK = storage.sQ.get_tensor(sdKV_layout.outer, swizzle=sdKV_layout.inner, dtype=self.dk_dtype)
        else:
    ...
    def load(
        ...
        while work_tile.is_valid_tile:
            ...
            load_K, _, _ = copy_utils.tma_get_copy_fn(tma_atom_K, 0, cute.make_layout(1), tSgK, sK, single_stage=True)
            ...
            if process_tile:
                if const_expr(self.use_block_sparsity):
                    ...
                    if const_expr(should_load_Q):
                        pipeline_Q.producer_acquire(producer_state_Q_LSE, extra_tx_count=self.tma_copy_bytes["K"])
                    ...
                    if const_expr(should_load_dO):
                        pipeline_dO.producer_acquire(producer_state_dO_dPsum, extra_tx_count=self.tma_copy_bytes["V"])
                ...
                if const_expr(should_load_Q):
                    pipeline_Q.producer_tail(producer_state_Q_LSE.clone())  # will hang if we don't clone
    ...
    def mma(
        ...
        mma_qk_fn = partial(gemm_ptx_w_idx, tiled_mma_S, tStS, tSrK, tSrQ, sA=sK, sB=sQ, zero_init=True)
        ...
        while work_tile.is_valid_tile:
            ...
            if process_tile:
                ...
                main_loop_iters = (
                    block_iter_count - 1 if const_expr(self.use_block_sparsity) else m_block_max - m_block_min - 1
                )
    ...
    def dQacc_reduce(
        ...
        assert cute.size(tdQrdQ_t2r_shape, mode=[1]) == self.dQaccum_reduce_stage, "dQaccum reduce stage mismatch"
        ...
        while work_tile.is_valid_tile:
            ...
            for iter_idx in cutlass.range(loop_count, unroll=1):
                ...
                for stage in cutlass.range_constexpr(cute.size(tdQrdQ_t2r, mode=[1])):  # 4
                    ...
                    tdQrdQ_r2s = cute.make_tensor(tdQrdQ_t2r[None, stage, None, None].iterator, tdQsdQ_r2s.shape)
    ...
    def epilogue_dK_or_dV_tma(
        ...
        if const_expr(not self.dKV_postprocess):
            ...
            gdKV_p = cute.local_tile(mdKV_cur, (self.tile_n, self.tile_hdim), (n_block, 0))  # (tile_n, hdim)
            ...
            if const_expr(not seqlen.has_cu_seqlens_k):
                ...
                mdKV_cur = cute.domain_offset((seqlen.padded_offset_k * self.tile_hdim,), mdKV[None, head_idx_kv])
            gdKV_p = cute.local_tile(mdKV_cur, (self.tile_n * self.tile_hdim,), (n_block,))  # (tile_n * hdim)
            gdKV = cute.logical_divide(gdKV_p, (self.tile_n * self.tile_hdim // num_wg,))[
                ((None, wg_idx),)
            ]  # (tile_n * hdim / 2)
            gdKV_epi = cute.flat_divide(gdKV, (self.sdKV_flat_epi_tile,))  # (tile_n * hdim / 2 / epi_stage, epi_stage)
        ...
        for epi_stage in cutlass.range_constexpr(num_epi_stages):
            ...
            assert (
                cute.size(tdKVrdKV_t2r) == cute.size(tdKVtdKV_t2r) // cute.arch.WARP_SIZE
