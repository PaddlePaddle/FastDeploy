...
import fastdeploy.model_executor.layers.attention.cute.ampere_helpers as sm80_utils
...
from .block_sparse_utils import consume_block_sparse_loads, produce_block_sparse_loads
...
class FlashAttentionForwardSm80(FlashAttentionForwardBase):
    ...
    def __call__(
        ...
        mQ, mK, mV, mO = [
            cute.make_tensor(t.iterator, cute.make_layout(t.shape, stride=new_stride(t))) for t in (mQ, mK, mV, mO)
        ]
        mQ, mK, mV, mO = [
            cute.make_tensor(t.iterator, cute.select(t.layout, mode=[1, 3, 2, 0])) for t in (mQ, mK, mV, mO)
        ]
    ...
    def kernel(
        ...
        load_K = partial(self.load_K, gmem_tiled_copy_K, tKgK, tKsK, tKcK, t0KcK, tKpK, seqlen=seqlen.seqlen_k)
        load_V = partial(self.load_V, gmem_tiled_copy_V, tVgV, tVsV, tVcV, t0VcV, tVpV, seqlen=seqlen.seqlen_k)
        ...
        if const_expr(self.is_causal or self.is_local):
            n_block_min_causal_local_mask = block_info.get_n_block_min_causal_local_mask(seqlen, m_block, n_block_min)
...
class FlashAttentionForwardSm90(FlashAttentionForwardBase):
    ...
    def __call__(
        ...
        self.num_mma_regs = 256 if self.num_mma_warp_groups == 1 else (240 if self.num_mma_warp_groups == 2 else 160)
        self.num_producer_regs = 56 if self.num_mma_warp_groups == 1 else (24 if self.num_mma_warp_groups == 2 else 32)
        ...
        if const_expr(not self.mma_pv_is_rs):
            self.sP_layout = sm90_utils.make_smem_layout(mV.dtype, LayoutEnum.ROW_MAJOR, (self.tile_m, self.tile_n))
