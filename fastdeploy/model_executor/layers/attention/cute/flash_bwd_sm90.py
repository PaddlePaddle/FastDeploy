...
import fastdeploy.model_executor.layers.attention.cute.hopper_helpers as sm90_utils
...
from .hopper_helpers import gemm_w_idx, gemm_zero_init
from .mask import AttentionMask
from .named_barrier import NamedBarrierBwd, NamedBarrierFwd
from .seqlen_info import SeqlenInfoQK
from .softmax import apply_score_mod_bwd_inner, apply_score_mod_inner
from .tile_scheduler import ParamsBase, SingleTileScheduler, TileSchedulerArguments
...
class FlashAttentionBackwardSm90:
    ...
    def __init__(
        ...
        self.mma_dkv_is_rs = (
            AtomLayoutMSdP == 1 and AtomLayoutNdKV == self.num_mma_warp_groups and SdP_swapAB and not dKV_swapAB
        )
    ...
    def _get_tiled_mma(self):
        ...
        tiled_mma_SdP = sm90_utils_basic.make_trivial_tiled_mma(
            ...
            warpgroup.OperandMajorMode.K,
            Float32,
            atom_layout_mnk=(atom_layout_SdP if not self.SdP_swapAB else atom_layout_SdP[::-1]) + (1,),
        ...
        tiled_mma_dK, tiled_mma_dV = [
            sm90_utils_basic.make_trivial_tiled_mma(
                ...
                Float32,
                atom_layout_mnk=(atom_layout_dKV if not self.dKV_swapAB else atom_layout_dKV[::-1]) + (1,),
    ...
    def __call__(
        ...
        new_stride = lambda t: (
            *(cute.assume(s, divby=128 // t.element_type.width) if s != 0 else s for s in t.stride[:-1]),
            t.stride[-1],
        )
        ...
        mLSE, mdPsum, mdQaccum = [utils.select(t, LSE_dPsum_dQaccum_transpose) for t in (mLSE, mdPsum, mdQaccum)]
        ...
        self.tma_copy_bytes["dQ"] = self.tile_m * self.tile_hdim * Float32.width // 8 // self.num_mma_warp_groups
    ...
    def mma(
        ...
        smem_copy_atom_PdS = utils.get_smem_store_atom(self.arch, self.dtype, transpose=self.SdP_swapAB)
    ...
    def mma_one_m_block(
        ...
        consumer_state_dO_cur = consumer_state_dO if const_expr(self.Q_stage == self.dO_stage) else consumer_state_Q
    ...
    def epilogue_dKV(
        ...
        if const_expr(self.qhead_per_kvhead == 1):
            ...
            gdKaccum = cute.flat_divide(gdKaccum_, (self.tile_n * self.tile_hdim // self.num_mma_warp_groups,))

            mdVaccum_cur = mdV[None, head_idx_kv, batch_idx]
            gdVaccum_ = cute.local_tile(mdVaccum_cur, (self.tile_n * self.tile_hdimv,), (n_block,))
            gdVaccum = cute.flat_divide(gdVaccum_, (self.tile_n * self.tile_hdimv // self.num_mma_warp_groups,))
            ...
            tdKrdKaccum_flat = cute.make_tensor(acc_dK.iterator, cute.make_layout(tdKsdKVaccum.shape))
            ...
            tdVrdVaccum_flat = cute.make_tensor(acc_dV.iterator, cute.make_layout(tdKsdKVaccum.shape))
    ...
    def dQaccum_store(
        ...
        while work_tile.is_valid_tile:
            ...
            # (M * K / WG, WG, _)
            gdQaccum = cute.flat_divide(gdQaccum_, (self.tile_m * self.tile_hdim // self.num_mma_warp_groups,))
            m_block_min, m_block_max = block_info.get_m_block_min_max(seqlen, n_block)
            if const_expr(not self.use_block_sparsity):
                process_tile = const_expr(not self.is_local) or m_block_min < m_block_max
