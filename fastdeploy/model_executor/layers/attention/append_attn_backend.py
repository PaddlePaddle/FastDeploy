...
def allocate_launch_related_buffer(
    ...
    decode_max_tile_size = 1024 * max_num_seqs * np.ceil((decoder_step_token_num * group_size) / decoder_block_shape_q)
    encode_max_tile_size = max_num_seqs * np.ceil((max_model_len * group_size) / encoder_block_shape_q)
    kv_max_tile_size = max_num_seqs * np.ceil(max_model_len / block_size)
    res = {}
