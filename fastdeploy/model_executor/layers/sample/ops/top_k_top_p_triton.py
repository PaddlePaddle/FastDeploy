...
def _topk_topp_kernel(
    ...
    for row_id in tl.range(pid, BATCH_SIZE, num_programs):
                ...
                if num_outliers > k:
                    ...
                    while found_pivot == 0:
                        ...
                        for i in range(0, NUM_TILES):
                            ...
                            finite_blk2 = tl.where(logits_blk2 > -float("inf"), logits_blk2, float("inf"))
        ...
        if not (final_pivot < max_logit):
            ...
            if WRITE_MASK:
                MASK_ROW = MASK_OUT + row_id * VOCAB_SIZE
            for i in range(0, NUM_TILES):
                offs_n = i * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
                mask_n = offs_n < VOCAB_SIZE
                logits_blk = tl.load(LOGITS_ROW + offs_n, mask=mask_n, other=-float("inf"))
                ...
                if WRITE_MASK:
                    tl.store(MASK_ROW + offs_n, keep_mask, mask=mask_n)

        # When no masking was applied (final_pivot == -inf), all tokens are kept.
        if WRITE_MASK and final_pivot == -float("inf"):
            MASK_ROW = MASK_OUT + row_id * VOCAB_SIZE
            for i in range(0, NUM_TILES):
                offs_n = i * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
                mask_n = offs_n < VOCAB_SIZE
                tl.store(MASK_ROW + offs_n, mask_n, mask=mask_n)


def apply_top_k_top_p_triton(
    logits: paddle.Tensor,
    k: paddle.Tensor | None,
    p: paddle.Tensor | None,
    ...
    Args:
        ...
        return_mask: If True, also return a bool mask [batch_size, vocab_size]
            where True = retained token. The mask is computed inside the kernel
            with zero extra memory bandwidth cost.
    ...
    # Allocate mask output if requested.
    write_mask = return_mask
    if write_mask:
        mask_out = paddle.empty(logits.shape, dtype=paddle.int8)
    else:
        mask_out = logits  # Dummy pointer (won't be written)
