...
def xpu_pre_process(
    ...
    if not use_speculate_method:
        ...
        share_inputs["ids_remove_padding"] = ids_remove_padding
        xpu_forward_meta.ids_remove_padding = ids_remove_padding
...
def xpu_post_process(
    ...
    # save_each_rank do not support in xpu, set defalut False
    if speculative_decoding:
        xpu_post_process_specualate(model_output, False, skip_save_output)
    else:
        xpu_post_process_normal(
            sampler_output.sampled_token_ids, model_output, share_inputs, False, block_size, skip_save_output, think_end_id, line_break_id
        )
