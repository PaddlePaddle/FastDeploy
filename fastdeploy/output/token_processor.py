    ...
    def _process_batch_output(self):
        ...
        for i in range(batch):
            ...
            if self.tokens_counter[task_id] == 0:
                ...
                # tracing.trace_slice_start("prefill", rid, int(task.inference_start_time * 1e9))
                # tracing.trace_slice_end("prefill", rid, thread_finish_flag=True)
            ...
            for batch_token_index in range(len(token_ids)):
                ...
                if token_id in task.eos_token_ids or is_prefill or recovery_stop:
                    ...
                    tracing.trace_report_span(
                        name=tracing.TraceSpanName.DECODE_LOOP,
                        rid=rid,
                        start_time_ns=int(task.inference_start_time * 1e9),
                        end_time_ns=int(time.time() * 1e9),
                        thread_finish_flag=True,
                    )
                    # tracing.trace_slice_end("decode_loop", rid, thread_finish_flag=True)
                    if recovery_stop:
                        result.error_msg = "Recover is not supported, the result is incomplete!"
                    llm_logger.info(
