 ...
 *  @note       None
 ***************************************************************/
    bool RKNPU2Backend::SetCoreMask(rknpu2_core_mask &core_mask) {
 ...
 ***************************************************************/
    bool RKNPU2Backend::LoadModel(void *model) {
        int ret = RKNN_SUCC;
        ret = rknn_init(&ctx, model, 0, 0, NULL);
        if (ret != RKNN_SUCC) {
 ...
 ***************************************************************/
    bool RKNPU2Backend::GetModelInputOutputInfos() {
        ...
        for (uint32_t i = 0; i < io_num.n_input; i++) {
            ...
            std::vector<int> temp_shape = {(int)input_attrs[i].dims[0], (int)input_attrs[i].dims[1], 
 ...
 ***************************************************************/
    ...
    std::vector<TensorInfo> RKNPU2Backend::GetOutputInfos() { return outputs_desc_; }

    bool RKNPU2Backend::Infer(std::vector<FDTensor>& inputs, std::vector<FDTensor>* outputs) {
        int ret = 0;
        ...
        if (width == stride) {
            memcpy(input_mems[0]->virt_addr, inputs[0].Data(), width * input_attrs[0].dims[1] * input_attrs[0].dims[3]);
        } else {
        ...
        for (size_t i = 0; i < outputs_desc_.size(); ++i) {
            ...
            memcpy((*outputs)[i].MutableData(), output_mems[i]->virt_addr, (*outputs)[i].Nbytes());
        }
