...
void PIPNet::GenerateLandmarks(std::vector<FDTensor>& infer_result, 
  ...
  for (unsigned int i = 0; i < num_landmarks_; ++i)
  {
    ...
    {
      const unsigned int max_id = max_ids.at(i);
      const float *offset_nb_x_ptr = outputs_nb_x_ptr + (i * num_nb_ + j) * grid_length;
      const float *offset_nb_y_ptr = outputs_nb_y_ptr + (i * num_nb_ + j) * grid_length;
  ...
  for (unsigned int i = 0; i < num_landmarks_; ++i)
  {
    ...
    lms_pred_x[i] = ((cx + output_x_select[i]) * (float) net_stride_) / (float) input_w;
    lms_pred_y[i] = ((cy + output_y_select[i]) * (float) net_stride_) / (float) input_h;
    for (unsigned int j = 0; j < num_nb_; ++j)
    {
      lms_pred_nb_x[i][j] = ((cx + output_nb_x_select[i][j]) * (float) net_stride_) / (float) input_w;
      lms_pred_nb_y[i][j] = ((cy + output_nb_y_select[i][j]) * (float) net_stride_) / (float) input_h;
    }
  }
  ...
  for (unsigned int i = 0; i < num_landmarks_; ++i)
  {
    ...
    {
      unsigned int ri = reverse_index1_map_[num_landmarks_][i * max_len_map_[num_landmarks_] + j];
      unsigned int rj = reverse_index2_map_[num_landmarks_][i * max_len_map_[num_landmarks_] + j];
...
bool PIPNet::Postprocess(std::vector<FDTensor>& infer_result, FaceAlignmentResult* result,
