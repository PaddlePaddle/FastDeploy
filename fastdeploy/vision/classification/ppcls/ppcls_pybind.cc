...
void BindPaddleClas(pybind11::module& m) {
  pybind11::class_<vision::classification::PaddleClasPreprocessor>(
           ...
           [](vision::classification::PaddleClasPreprocessor& self,
             ...
             if (!self.Run(&images, &outputs)) {
               pybind11::eval("raise Exception('Failed to preprocess the input "
                              "data in PaddleClasPreprocessor.')");
             }
             if (!self.WithGpu()) {
      ...
      .def("use_gpu", [](vision::classification::PaddleClasPreprocessor& self,
                         int gpu_id = -1) { self.UseGpu(gpu_id); });
      ...
      .def("run",
           [](vision::classification::PaddleClasPostprocessor& self,
             ...
             if (!self.Run(inputs, &results)) {
               pybind11::eval("raise Exception('Failed to postprocess the "
                              "runtime result in PaddleClasPostprocessor.')");
             }
             return results;
           })
      .def("run",
