...
bool StructureV2LayoutPostprocessor::Run(
  ...
  FDASSERT(tensor_size == 8, "StructureV2Layout should has 8 output tensors,"
                             "but got %d now!", tensor_size)
  ...
  for (int batch_idx = 0; batch_idx < batch; ++batch_idx) {
    ...
    SingleBatchPostprocessor(single_batch_tensors, batch_layout_img_info[batch_idx], 
...
void StructureV2LayoutPostprocessor::SetSingleBatchExternalData(
  const std::vector<FDTensor>& tensors, std::vector<FDTensor>& single_batch_tensors, 
  size_t batch_idx) {
  ...
  for (int j = 0; j < tensors.size(); ++j) {
    ...
    size_t j_step = accumulate(j_shape.begin(), j_shape.end(), 1, 
                               std::multiplies<int>());
    ...
    single_batch_tensors[j].SetExternalData(
      j_shape, tensors[j].Dtype(), const_cast<void*>(
        reinterpret_cast<const void*>(j_start_ptr)),
  ...
  for (int i = 0; i < fpn_stride_.size(); ++i) {
    ...
    for (int idx = 0; idx < feature_h * feature_w; ++idx) {
      ...
      if (score > score_threshold_) {
        ...
        bbox_results[label].boxes.push_back(
          DisPred2Bbox(bbox_pred, col, row, fpn_stride_[i], in_w, in_h, reg_max_));                             
...
std::array<float, 4> StructureV2LayoutPostprocessor::DisPred2Bbox(
  const std::vector<float>& bbox_pred, int x, int y, int stride, 
  int resize_w, int resize_h, int reg_max) {
