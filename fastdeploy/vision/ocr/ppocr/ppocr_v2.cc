...
bool PPOCRv2::BatchPredict(
  ...
  //        image_list[i_box] = vision::ocr::GetRotateCropImage(img,
  //        boxes[i_box]);
  //      }
  //    }
  ...
  //        size_t end_index = std::min(start_index + cls_batch_size_,
  //        image_list.size()); if (!classifier_->BatchPredict(image_list,
  //        cls_labels_ptr, cls_scores_ptr, start_index, end_index)) {
  ...
  //      size_t end_index = std::min(start_index + rec_batch_size_,
  //      image_list.size()); if (!recognizer_->BatchPredict(image_list,
  //      text_ptr, rec_scores_ptr, start_index, end_index, indices)) {
  //        FDERROR << "There's error while recognizing image in PPOCR." <<
  //        std::endl; return false;
  //      }
  //    }
