...
inline void FuseNormalizeCast(
  ...
  auto processor = dynamic_cast<Cast*>((*processors)[cast_index].get());
  FDINFO << "Cast dtype: " << processor->GetDtype() << std::endl;
  if (processor->GetDtype() != "float") {
    return;
  }
...
inline void FuseNormalizeColorConvert(
  ...
  static std::unordered_set<std::string>middle_processors(
      {"Resize", "ResizeByShort", "ResizeByLong", "CenterCrop"});
