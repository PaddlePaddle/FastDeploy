...
void PaddleMultiClassNMSRotated::RotatedNMS(const float* boxes, const float* scores,
                                            const int& num_boxes,
...
int PaddleMultiClassNMSRotated::NMSForEachSample(
  ...
  for (int i = 0; i < num_classes; ++i) {
    ...
    RotatedNMS(boxes, score_for_class_i, num_boxes, &((*keep_indices)[i]));
  }
  int num_det = 0;
...
void PaddleMultiClassNMSRotated::Compute(const float* boxes_data, const float* scores_data,
