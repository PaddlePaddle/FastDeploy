...
bool YOLOv5SegPostprocessor::Run(const std::vector<FDTensor>& tensors, std::vector<DetectionResult>* results,
  ...
  for (size_t bs = 0; bs < batch; ++bs) {
    ...
    for (size_t i = 0; i < tensors[0].shape[1]; ++i) {
      ...
      if (multi_label_) {
        for (size_t j = 5; j < tensors[0].shape[2] - mask_nums_; ++j) {
          ...
          for (size_t k = 0; k < mask_proposal.size(); ++i) {
    ...
    for (size_t i = 0; i < (*results)[bs].yolo_masks.size(); ++i) {
      mask_proposals.push_back(cv::Mat((*results)[bs].yolo_masks[i]));
    }
    cv::Mat matmul_result = mask_proposals * mask_proto;
    cv::Mat masks = matmul_result.reshape((*results)[bs].boxes.size(), { tensors[1].shape[2], tensors[1].shape[3] });
    ...
    for (size_t i = 0; i < (*results)[bs].boxes.size(); ++i) {
      ...
      cv::Rect roi_src(x1_src, y1_src, x2_src, y2_src);
      mask = mask(roi_src);
      ...
      int32_t* keep_mask_ptr =
          reinterpret_cast<int32_t*>((*results)[bs].masks[i].Data());
      std::memcpy(keep_mask_ptr, mask.ptr(), keep_mask_numel * sizeof(int32_t));
    }
  }
  return true;
}
