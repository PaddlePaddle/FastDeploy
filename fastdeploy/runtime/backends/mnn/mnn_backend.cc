...
FDDataType MNNDataTypeToFD(const halide_type_t& dtype) {
  ...
  if (code == halide_type_code_t::halide_type_int) {
    ...
    if (bits == 8) { 
      ...
      FDASSERT(false, "FastDeploy only support UINT8 for unsigned integers now" 
                      "but got %d", bits);
    }
    ...
    if (bits == 16) {
      ...
      FDASSERT(false, "FastDeploy only support FP16/FP32/FP64 for float now" 
                      "but got %d", bits);
    }
  }
...
void MNNBackend::BuildOption(const MNNBackendOption& option) {
  ...
  if (option_.device == Device::CPU) {
    ...
    FDERROR << "Backend::MNN only support CPU now, but got" 
            << option_.device << ", fallback to CPU now."
            << std::endl;
  }
  // power mode
  if (option_.power_mode != MNNPowerMode::MNN_POWER_NORMAL) {
    backend_config_.power = static_cast<MNN::BackendConfig::PowerMode>(
      option_.power_mode);
  }
  // fp32/fp16/int8
    ...
    for (const auto& it: custom_orders) {
      if (it.second == i) {
        ...
        if (iter == tensors_info.end()) {
          FDERROR << "Cannot find name:" << it.first
...
bool MNNBackend::Init(const RuntimeOption& runtime_option) {
  ...
  if (runtime_option.model_format != ModelFormat::MNN_MODEL) {
    FDERROR
        << "Backend::MNN only supports model format MNN, but now it's "
  ...
  if (!option_.custom_input_orders.empty()) {
    FDASSERT(option_.custom_input_orders.size() == inputs_info.size(),
             "The size of custom input orders must be equal the size of"
             " inputs info, but got %d != %d now!",
             option_.custom_input_orders.size(), 
             inputs_info.size())
  }
  if (!option_.custom_output_orders.empty()) {
    FDASSERT(option_.custom_output_orders.size() == outputs_info.size(),
             "The size of custom output orders must be equal the size of"
             " outputs info, but got %d != %d now!",
             option_.custom_output_orders.size(), 
             outputs_info.size())
  }
  // Check the name in custom inputs/outputs orders and reorder
  if (!option_.custom_input_orders.empty()) {
    FDASSERT(SetTensorInfoByCustomOrder(option_.custom_input_orders, 
                                        inputs_info, &inputs_desc_, 
                                        &inputs_order_),
  ...
  if (!option_.custom_output_orders.empty()) {
    FDASSERT(SetTensorInfoByCustomOrder(option_.custom_output_orders, 
                                        outputs_info, &outputs_desc_,
                                        &outputs_order_),
...
bool MNNBackend::Infer(std::vector<FDTensor>& inputs,
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    ...
    // Get input tensor by name in MNN, only support NCHW
    auto input_shape = GetMNNShape(inputs[i].shape);
    auto input_tensor = interpreter_->getSessionInput(
      session_, inputs[i].name.c_str());

#define MNN_COPY_NCHW_DATA_FROM_HOST(__type__)                                    \
    {                                                                             \
      auto host_nchw_tensor = MNN::Tensor::create<__type__>(                      \
        input_shape, const_cast<void*>(inputs[i].CpuData()), MNN::Tensor::CAFFE); \
    ...
    switch (inputs[i].dtype) {
      ...
      default:
        FDASSERT(
          false, "Unexpected data type of %s.", Str(inputs[i].dtype).c_str())
        break;
    }
#undef MNN_COPY_NCHW_DATA_FROM_HOST
  ...
  for (size_t i = 0; i < outputs_desc_.size(); ++i) {
    ...
    auto device_tensor = interpreter_->getSessionOutput(
      session_, outputs_desc_[i].name.c_str());
    FDASSERT(device_tensor, 
            "Backend::MNN failed to get output tensors: %s", 
    ...
    // from host -> FDTensor
    auto fd_tensor_shape = GetFDShape(host_tensor.shape());
    (*outputs)[i].Resize(fd_tensor_shape, outputs_desc_[i].dtype,
