...
func RegisterInstanceCore(ctx context.Context, rawInstance *InstanceInfo) error {
	...
	default:
		logger.Warn(nil, "Instance %s role is unknown", id)
	}
...
func RegisterInstance(c *gin.Context) {
	...
	if err := RegisterInstanceCore(c.Request.Context(), &rawInstance); err != nil {
		logger.Error(nil, "Failed to register instance: %v", err)
...
func RegisterInstancesFromConfig(yamlPath string) {
	...
	if err != nil {
		logger.Error(nil, "Failed to read YAML file %s: %v", yamlPath, err)
		return
	}

	var config RegisterConfig
	if err := yaml.Unmarshal(data, &config); err != nil {
		logger.Error(nil, "Failed to unmarshal YAML file %s: %v", yamlPath, err)
		return
	}

	if len(config.Instances) == 0 {
		logger.Info(nil, "No instances found in config file %s", yamlPath)
		return
	}

	for i, instanceConfig := range config.Instances {
		if err := RegisterInstanceCore(context.Background(), &instanceConfig); err != nil {
			logger.Error(nil, "Failed to register instance from index %d: %v", i, err)
		} else {
			logger.Info(nil, "Successfully registered instance from index %d", i)
		}
	}
}
