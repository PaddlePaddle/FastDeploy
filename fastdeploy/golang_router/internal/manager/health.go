...
func CheckServiceHealth(ctx context.Context, baseURL string, timeout ...time.Duration) bool {
	...
	if baseURL == "" {
		logger.Error(nil, "empty baseURL provided")
		return false
	}
	...
	if err != nil {
		logger.Error(nil, "failed to create request: %v", err)
		return false
	}
	...
	if err != nil {
		logger.Error(nil, "failed to send request to %s with error: %v", url, err)
		return false
	}
	...
	if err != nil {
		logger.Error(nil, "failed to read response body: %v", err)
		return false
	}
...
func HealthGenerate(c *gin.Context) {
	...
	for res := range results {
		// Process each result
		if !res.isHealthy {
			logger.Warn(nil, "Server %s is not healthy", res.url)
		} else {
			logger.Info(nil, "Server %s is healthy", res.url)
		}
	}
...
func RemoveServers(ctx context.Context, prefillToRemove []string, decodeToRemove []string, mixedToRemove []string) {
	...
	for _, id := range prefillToRemove {
		if worker, exists := DefaultManager.prefillWorkerMap[id]; exists {
			...
			logger.Info(nil, "Removed unhealthy prefill instance: %s", worker.Url)
		}
	}
	for _, id := range decodeToRemove {
		if worker, exists := DefaultManager.decodeWorkerMap[id]; exists {
			...
			logger.Info(nil, "Removed unhealthy decode instance: %s", worker.Url)
		}
	}
	for _, id := range mixedToRemove {
		if worker, exists := DefaultManager.mixedWorkerMap[id]; exists {
			...
			logger.Info(nil, "Removed unhealthy mixed instance: %s", worker.Url)
		}
	}
}
...
func ReadServers(ctx context.Context) (prefillInstances, decodeInstances, mixedInstances []string) {
	if DefaultManager == nil {
		logger.Debug(nil, "Healthy instances: prefill=[], decode=[], mixed=[] (DefaultManager is nil)")
