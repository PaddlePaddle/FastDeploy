...
class DeepSeekToolParser(ToolParser):
    ...
    def __init__(self, tokenizer, model_name=None):
        ...
        if self.tool_calls_begin_token_id is None or self.tool_call_begin_token_id is None:
            raise RuntimeError(
                "DeepSeek Tool parser could not locate tool call tokens in the tokenizer!"
            )
    ...
    def extract_tool_calls(
        ...
        try:
            ...
            if self.is_v31:
                ...
                pattern = f"{begin_escaped}(?P<function_name>[^<]+?){sep_escaped}(?P<function_arguments>.*?){end_escaped}"
            else:
                    ...
                    if sep_pos != -1:
                        ...
                        if function_name:
                            ...
                            if not self.is_v31 and tool_type_or_name == "function":
                                ...
                                processed_end = begin_pos + len(self.tool_call_begin_token) + sep_pos + len(self.tool_sep_token) + newline_pos + 1
                            else:
                                # V3.1：移除到分隔符之后
                                processed_end = begin_pos + len(self.tool_call_begin_token) + sep_pos + len(self.tool_sep_token)
