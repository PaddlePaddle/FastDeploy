...
class SplitwiseConnector:
    ...
    def send_splitwise_tasks(self, tasks, current_id):
        ...
        for task in tasks:
            ...
            if task.disaggregate_info["transfer_protocol"] == "ipc":
                ...
                task.disaggregate_info["cache_info"] = self.cfg.scheduler_config.disaggregate_info["cache_info"]
    ...
    def send_splitwise_tasks_innode(self, tasks, port):
        ...
        if port not in self.connect_innode_instances:
            self.create_connection(port)
        for task in tasks:
            task.disaggregate_info["cache_info"]["ipc"]["port"] = self.cfg.scheduler_config.engine_worker_queue_port
            ...
            if tasks[i].disaggregate_info["role"] == "decode":
                if tasks[i].disaggregate_info["transfer_protocol"] == "ipc":
                    ...
                    cache_info = {
                        "request_id": tasks[i].request_id,
                        "device_ids": self.cfg.device_config.device_ids.split(","),
                        "ip": self.cfg.scheduler_config.host_ip,
                        "rdma_ports": self.cfg.scheduler_config.disaggregate_info["cache_info"]["rdma"]["rdma_port"],
