...
class RedundantExpertManager:
    ...
    def get_unique_name(self, name):
        return f"{envs.get_unique_name(name)}_dprank{self.rank}"
    ...
    def listen_rearrange_expert_signal(self):
        ...
        if self.rank == 0:
            ...
            shm_rearrange_experts_ips_size = shared_memory.SharedMemory(
                ...
                name=self.get_unique_name("rearrange_experts_ips_size", dp_rank=self.rank),
            )
            ...
            shm_rearrange_experts_ips_list = shared_memory.SharedMemory(
                create=False,
                size=1024,
                name=self.get_unique_name("rearrange_experts_ips_list", dp_rank=self.rank),
            )
        ...
        shm_signal_update_weight_from_disk = shared_memory.SharedMemory(
            ...
            name=self.get_unique_name("signal_update_weight_from_disk", dp_rank=self.rank),
        )
        ...
        shm_all_experts_token_stats = shared_memory.SharedMemory(
            ...
            name=self.get_unique_name("all_experts_token_stats", dp_rank=self.rank),
        )
    ...
    def update_weight_from_disk(self):
        ...
        shm_result_update_weight_from_disk = shared_memory.SharedMemory(
            ...
            name=self.get_unique_name("result_update_weight_from_disk", dp_rank=self.rank),
        )
