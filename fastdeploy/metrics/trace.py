...
def trace_get_proc_propagate_context(rid, remote_propagate=False) -> Optional[Dict[str, Any]]:
    if not tracing_enabled:
        return None
...
def trace_get_remote_propagate_context(bootstrap_room_list: List[str]):
    ...
    for bootstrap_room in bootstrap_room_list:
        ...
        _context = trace_get_proc_propagate_context(bootstrap_room, remote_propagate=True)
...
def trace_req_start(
    ...
    if active_span_context.is_valid and active_span_context.trace_id != 0:
        ...
        # Idempotency Check: If 'rid' is already in reqs_context, it means duplicate call, return immediately
        ...
        reqs_context[rid] = TraceReqContext(
            ...
            start_time_ns=ts,
            threads_context={},
            root_span_context=context.get_current(),  # Use the entire current context
            is_copy=True,  # Mark as copied/inherited context
        )
        # Thread context is necessary so that trace_slice_start can find the tracer
        if pid not in reqs_context[rid].threads_context:
            reqs_context[rid].threads_context[pid] = __create_thread_context(
                pid,
                context.get_current(),  # Pass the current context to the thread Span
                ts,
            )
    ...
    if str(bootstrap_room) not in remote_trace_contexts:
        ...
        bootstrap_room_span_context = remote_trace_contexts[str(bootstrap_room)].root_span_context

    orig_rid = rid.split("_")[0]
    role = "" if role == "null" else role
    attrs = {"rid": orig_rid}
    root_span = tracer.start_span(
        name=f"{role} Req {orig_rid}",
        start_time=ts,
    ...
    if str(bootstrap_room) in remote_trace_contexts:
        reqs_context[rid].threads_context[pid].last_span_context = remote_trace_contexts[
            str(bootstrap_room)
        ].prev_span_context
...
def trace_req_finish(rid: str, ts: Optional[int] = None, attrs: Optional[Dict[str, Any]] = None):
    ...
    # Only end the bootstrap_room_span if it was manually created
...
class TraceSpanName(str, Enum):
    ...
    SUBMIT_TO_INFER_ENGINE = "submit_to_infer_engine"

    SCHEDULE = "schedule"
    SCHEDULE_QUEUE_WAIT = "schedule_queue_wait"
    SCHEDULE_DISPATCH = "schedule_dispatch"
    SCHEDULER_ALLOCATE_RESOURCE = "scheduler_allocate_resource"
