...
def trace_req_start(
    ...
    if active_span_context.is_valid and active_span_context.trace_id != 0:
        ...
        reqs_context[rid] = TraceReqContext(
            ...
            start_time_ns=ts,
            threads_context={},
            root_span=active_span,  # 记录 active span 对象
    ...
    # --- Fallback to Manual Span Creation (Case 2 & 3) ---

    parent_context = None

    # 判断是否使用 Upstream Context
    use_upstream = False
    if upstream_context:
        # 验证 upstream context 是否包含有效的 trace_id
    ...
    if use_upstream:
        ...
        # 初始化 reqs_context (由于未走 Bootstrap Room 逻辑，部分字段可省略)
        reqs_context[rid] = TraceReqContext(
            rid=rid,
            start_time_ns=ts,
            threads_context={},
            is_copy=True,  # 标记为继承自外部 Context
        )

    else:
        # 初始化 reqs_context (必须在创建 bootstrap span 之前)
        reqs_context[rid] = TraceReqContext(
            rid=rid,
            start_time_ns=ts,
            threads_context={},
    ...
    root_span = tracer.start_span(
        ...
        start_time=ts,
        context=parent_context,  # 使用确定的父级上下文
        kind=trace.SpanKind.SERVER,  # 推荐加上 SpanKind
        attributes=attrs,
    )
