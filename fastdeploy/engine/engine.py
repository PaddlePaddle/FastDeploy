...
class LLMEngine(object):
    ...
    def __init__(self, cfg):
        ...
        if envs.ENABLE_V1_KVCACHE_SCHEDULER:
            self.resource_manager = ResourceManagerV1(cfg.max_num_seqs, cfg,
                                                    cfg.tensor_parallel_size,
                                                    cfg.splitwise_role)
            if cfg.splitwise_role != 'mixed':
                raise NotImplementedError('Currently ENABLE_V1_KVCACHE_SCHEDULER=1 only supported in mixed sampling now.')
    ...
    def start(self, api_server_pid=None):
        ...
        if envs.ENABLE_V1_KVCACHE_SCHEDULER:
            self.insert_task_to_worker_thread = threading.Thread(
                target=self._scheduler_task_to_worker_v1, daemon=True)
        else:
            self.insert_task_to_worker_thread = threading.Thread(
