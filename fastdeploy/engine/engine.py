...
class LLMEngine:
    ...
    def __init__(self, cfg):
        ...
        self.input_processor = InputPreprocessor(
            cfg.model_config.tokenizer,
            cfg.decoding_config.reasoning_parser,
            cfg.multi_modal_config.limit_mm_per_prompt,
            cfg.multi_modal_config.mm_processor_kwargs,
            cfg.multi_modal_config.enable_mm,
        )
        ...
        os.environ["INFERENCE_MSG_QUEUE_ID"] = str(self.cfg.scheduler_config.engine_worker_queue_port)
        ...
        self.partial_chunked_tokens = [0] * (self.cfg.scheduler_config.max_num_partial_prefills + 1)
        ...
        if self.cfg.decoding_config.guided_decoding_backend != "off":
            self.guided_decoding_checker = schema_checker(
                self.cfg.decoding_config.guided_decoding_backend,
    ...
    def start(self, api_server_pid=None):
        ...
        if self.do_profile == 0 and (
            ...
            self.cache_manager_processes = self.resource_manager.cache_manager.launch_cache_manager(
                ...
                pod_ip=self.cfg.scheduler_config.master_ip,
        ...
        if self.cfg.scheduler_config.use_warmup:
        ...
        if self.cfg.scheduler_config.splitwise_role != "mixed":
            ...
            if self.cfg.parallel_config.enable_expert_parallel and self.cfg.parallel_config.data_parallel_size > 1:
                ...
                for i in range(
                    1,
                    self.cfg.parallel_config.data_parallel_size // self.cfg.scheduler_config.nnodes,
                ):
                    time.sleep(1)
                    self.dp_processed.append(
                        multiprocessing.Process(
                            ...
                            args=(
                                self.cfg,
                                i + self.cfg.scheduler_config.node_rank * self.cfg.parallel_config.worker_num_per_node,
        ...
        while self.running:
            try:
                ...
                num_prefill_batch = min(
                    int(self.resource_manager.available_batch()), self.cfg.parallel_config.max_prefill_batch
                )
    ...
    def add_requests(self, task, sampling_params=None, **kwargs):
        ...
        request = self.data_processor.process_request(
            request, self.cfg.scheduler_config.max_model_len, enable_thinking=enable_thinking
        )
        request.prompt_token_ids_len = len(request.prompt_token_ids)
        input_ids_len = request.prompt_token_ids_len
        request.set(
            "max_tokens", min(self.cfg.scheduler_config.max_model_len - input_ids_len, request.get("max_tokens"))
        )
        ...
        if input_ids_len + min_tokens >= self.cfg.scheduler_config.max_model_len:
            error_msg = (
        ...
        if input_ids_len > self.cfg.scheduler_config.max_model_len:
            error_msg = f"Length of input token({input_ids_len}) exceeds the limit max_model_len({self.cfg.scheduler_config.max_model_len})."
            llm_logger.error(error_msg)
            raise EngineError(error_msg, error_code=400)
    ...
    def update_requests_chunk_size(self, requests):
        ...
        if not self.cfg.scheduler_config.enable_chunked_prefill or len(requests) == 0:
            return
    ...
    def _worker_processes_ready(self):
        ...
        """
        if np.sum(self.worker_ready_signal.value) == self.cfg.parallel_config.worker_num_per_node:
            return True
        return False
    ...
    def _init_worker_signals(self):
        ...
        worker_ready_signal_data = np.zeros(shape=[self.cfg.parallel_config.worker_num_per_node], dtype=np.int32)
        ...
        worker_healthy_live_recorded_time_array = np.zeros(
            shape=[self.cfg.parallel_config.worker_num_per_node], dtype=np.int32
        )
        ...
        if self.do_profile:
            get_profile_block_num = np.zeros([self.cfg.parallel_config.worker_num_per_node], dtype=np.int32)
    ...
    def _setting_environ_variables(self):
        ...
        if self.cfg.multi_modal_config.enable_mm:
    ...
    def _start_worker_service(self):
        ...
        arguments = (
            ...
            f" --engine_worker_queue_port {self.cfg.scheduler_config.engine_worker_queue_port!s}"
            f" --master_ip {self.cfg.scheduler_config.master_ip}"
        ...
        worker_append_flag = {
            ...
            "enable_chunked_prefill": self.cfg.scheduler_config.enable_chunked_prefill,
            ...
            "enable_mm": self.cfg.multi_modal_config.enable_mm,
        }
        for worker_flag, value in worker_append_flag.items():
            if value:
                arguments = arguments + f" --{worker_flag}"
        if self.cfg.scheduler_config.nnodes > 1:
            pd_cmd = pd_cmd + (
                f" --master {self.cfg.scheduler_config.dist_init_addr}"
                f" --nnodes {self.cfg.scheduler_config.nnode!s}"
                f" --rank {self.cfg.scheduler_config.node_rank!s}"
            )
    ...
    def _format_and_add_data(self, prompts: dict):
        ...
        if "max_tokens" not in prompts:
            prompts["max_tokens"] = self.cfg.scheduler_config.max_model_len
    ...
    def _stop_profile(self):
        ...
        self.do_profile = 0
        num_gpu_blocks = -1
        for i in range(self.cfg.parallel_config.tensor_parallel_size):
        ...
        def detect_thread():
            for line in self.worker_proc.stdout:
                ...
                if self.worker_init_status.get("finished", False):
                    break
                if match := re.search(r"Loading (?:fastsafetensors |safetensors )?checkpoint shards:\s*(\d+)", line):
    ...
    def start_queue_service(self):
        ...
        """
        address = (self.cfg.scheduler_config.master_ip, self.cfg.scheduler_config.engine_worker_queue_port)
        if (
            self.cfg.scheduler_config.host_ip == self.cfg.scheduler_config.master_ip
            or self.cfg.scheduler_config.master_ip == "0.0.0.0"
        ):
            ...
            if self.cfg.cache_config.enable_prefix_caching or self.cfg.scheduler_config.splitwise_role != "mixed":
                self.cache_task_queue = EngineCacheQueue(
                    address=(
                        self.cfg.scheduler_config.master_ip,
        ...
        self.engine_worker_queue = EngineWorkerQueue(
            ...
            local_data_parallel_id=min(
                self.cfg.parallel_config.worker_num_per_node * self.cfg.scheduler_config.node_rank,
