...
class Ernie4_5Processor(BaseDataProcessor):
    ...
    def process_response_obj_streaming(self, response_obj, **kwargs):
        ...
        response_obj.outputs.skipped = False
        response_obj.outputs.tool_calls = None
        response_obj.outputs.reasoning_content = ""
        if self.reasoning_parser:
            ...
            if reasoning_delta_message:
                ...
                response_obj.outputs.reasoning_token_num = len(reasoning_tokens)
                response_obj.outputs.reasoning_content = reasoning_content or ""
                response_obj.outputs.text = reasoning_delta_message.content or ""
            else:
                if not is_end:
                    response_obj.outputs.skipped = True
        else:
            response_obj.outputs.text = delta_text
        if self.tool_parser_obj:
            ...
            if tool_call_delta_message:
                if tool_call_delta_message.tool_calls:
                    response_obj.outputs.text = tool_call_delta_message.content
                    response_obj.outputs.tool_calls = tool_call_delta_message.tool_calls
                    response_obj.outputs.skipped = False
            else:
                if not is_end:
                    response_obj.outputs.skipped = True
