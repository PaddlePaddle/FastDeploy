...
class MultiDataProcessor(DataProcessor):
    """The processor class for  MultiModal models."""
    ...
    def __init__(
        ...
        self.image_processor = self.get_image_processor(
            self.tokenizer, model_name_or_path, enable_processor_cache, processor_kwargs, architecture
        )
    ...
    def process_request_dict(self, request, max_model_len=None):
        ...
        if request.get("prompt_token_ids"):
            ...
            outputs = self.image_processor.prompt_token_ids2outputs(request)
            ...
            outputs = self.image_processor.text2ids(request["prompt"], images, videos)
            ...
            outputs = self.image_processor.request2ids(request)
        else:
            raise ValueError(f"Request must contain 'prompt', or 'messages': {request}")

        if request.get("completion_token_ids"):
            self.image_processor.append_completion_tokens(outputs, request["completion_token_ids"])

        outputs = self.image_processor.pack_outputs(outputs)
        ...
        data_processor_logger.debug(f"Processed request {request}")
