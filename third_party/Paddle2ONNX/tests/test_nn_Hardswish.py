...
class Net(paddle.nn.Layer):
    ...
    def __init__(self):
        ...
        self._hard_swish = paddle.nn.Hardswish()

    def forward(self, inputs):
        """
        forward
        """
        x = self._hard_swish(inputs)
        return x
...
def test_Hardswish_11():
    ...
    op.eval()
    # net, name, ver_list, delta=1e-6, rtol=1e-5
    obj = APIOnnx(op, 'nn_Hardswish', [11])
    obj.set_input_data(
        "input_data",
        paddle.to_tensor(
            randtool("float", -1, 1, [3, 1, 10, 10]).astype('float32')))
...
def test_Hardswish_12():
    ...
    op.eval()
    # net, name, ver_list, delta=1e-6, rtol=1e-5
    obj = APIOnnx(op, 'nn_Hardswish', [12])
    obj.set_input_data(
        "input_data",
        paddle.to_tensor(
            randtool("float", -1, 1, [3, 1, 10, 10]).astype('float32')))
