...
static void test_simple_padding(const Eigen::SyclDevice& sycl_device) {
  ...
  array<IndexType, 4> padedtensorRange = {
      {padedSizeDim1, padedSizeDim2, padedSizeDim3, padedSizeDim4}};
  ...
  DataType* gpu_data2 = static_cast<DataType*>(
      sycl_device.allocate(padded.size() * sizeof(DataType)));
  ...
  sycl_device.memcpyDeviceToHost(padded.data(), gpu_data2,
                                 (padded.size()) * sizeof(DataType));
...
static void test_padded_expr(const Eigen::SyclDevice& sycl_device) {
  ...
  DataType* gpu_data1 = static_cast<DataType*>(
      sycl_device.allocate(tensor.size() * sizeof(DataType)));
  DataType* gpu_data2 = static_cast<DataType*>(
      sycl_device.allocate(result.size() * sizeof(DataType)));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu1(gpu_data1,
                                                             tensorRange);
  ...
  sycl_device.memcpyDeviceToHost(result.data(), gpu_data2,
                                 (result.size()) * sizeof(DataType));
