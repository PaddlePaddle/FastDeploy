...
void test_device_memory(const Eigen::SyclDevice& sycl_device) {
  ...
  sycl_device.memcpyDeviceToHost(in.data(), gpu_in_data,
...
void test_device_exceptions(const Eigen::SyclDevice& sycl_device) {
  ...
  DataType* gpu_data =
      static_cast<DataType*>(sycl_device.allocate(sizeDim1 * sizeof(DataType)));
  sycl_device.memset(gpu_data, 1, sizeDim1 * sizeof(DataType));

  TensorMap<Tensor<DataType, 1, DataLayout, IndexType>> in(gpu_data,
                                                           tensorDims);
  TensorMap<Tensor<DataType, 1, DataLayout, IndexType>> out(gpu_data,
                                                            tensorDims);
