...
static void test_1D_sycl(const Eigen::SyclDevice& sycl_device) {
  ...
  DataType* gpu_data_vec =
      static_cast<DataType*>(sycl_device.allocate(tensorBuffSize));
  DataType* gpu_data_result =
      static_cast<DataType*>(sycl_device.allocate(tensorBuffSize));

  TensorMap<Tensor<DataType, 1, DataLayout, IndexType>> gpu_vec(gpu_data_vec,
                                                                tensorRange);
  TensorMap<Tensor<DataType, 1, DataLayout, IndexType>> gpu_result(
      gpu_data_result, tensorRange);
  ...
  sycl_device.memcpyDeviceToHost(result.data(), gpu_data_result,
                                 tensorBuffSize);
...
static void test_2D_sycl(const Eigen::SyclDevice& sycl_device) {
  ...
  DataType* gpu_data_matrix =
      static_cast<DataType*>(sycl_device.allocate(tensorBuffSize));
  DataType* gpu_data_result =
      static_cast<DataType*>(sycl_device.allocate(tensorBuffSize));

  TensorMap<Tensor<DataType, 2, DataLayout, IndexType>> gpu_matrix(
      gpu_data_matrix, tensorRange);
  TensorMap<Tensor<DataType, 2, DataLayout, IndexType>> gpu_result(
      gpu_data_result, tensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_matrix, matrix.data(),
                                 tensorBuffSize);
...
static void test_gaussian_sycl(const Eigen::SyclDevice& sycl_device) {
  ...
  std_devs[0] = 3.14f;
  std_devs[1] = 2.7f;
  internal::GaussianGenerator<DataType, Eigen::DenseIndex, 2> gaussian_gen(
      means, std_devs);
