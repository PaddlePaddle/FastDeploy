...
// See std::numeric_limits<mpfr::mpreal> at the end of the file for more
// information.
...
// Everything should be thread-safe and support passing ownership over thread
// boundary.
...
// types
// This is needed for smooth integration with libraries based on expression
// templates, like Eigen.
...
inline bool isnan EIGEN_NOT_A_MACRO(const mpreal& op) {
...
inline const mpreal hypot(const mpreal& a, const mpreal& b, const mpreal& c) {
  if (isnan EIGEN_NOT_A_MACRO(a) || isnan EIGEN_NOT_A_MACRO(b) ||
      isnan EIGEN_NOT_A_MACRO(c))
...
inline const mpreal hypot(const mpreal& a, const mpreal& b, const mpreal& c,
                          const mpreal& d) {
  if (isnan EIGEN_NOT_A_MACRO(a) || isnan EIGEN_NOT_A_MACRO(b) ||
      isnan EIGEN_NOT_A_MACRO(c) || isnan EIGEN_NOT_A_MACRO(d))
    return mpreal().setNan();
  else {
...
#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0) && \
     MPFR_VERSION < MPFR_VERSION_NUM(4, 0, 0))
...
class numeric_limits<mpfr::mpreal> {
 public:
  ...
  // specialization.
  // But it is incompatible with standard std::numeric_limits and might not work
  // with other libraries, e.g. boost.
