...
// use #error to make
// it error out. For this reason, we use the pragma to make sure G++ aborts at
// the first error
// it sees. Unfortunately, that is still not our #error directive, but at least
// the output is
// short enough the user has a chance to see that the compiler version is not
// sufficient for
 ...
 * On the other hand, visual studio still doesn't claim to support C++11
 * although it's
 ...
 * since
 * g++ can only match templates called with parameter packs if the number of
 * template
 * arguments is not a fixed size (so inside the first specialization,
 * referencing
 * X<Foo<Sn...>> will fail in g++). On the other hand, g++ will accept the
 * following:
 *    template<typename S...> struct X<Foo<S...>> { ::: }:
 * as an additional (!) specialization, which will then only match the empty
 * case.
 * But Intel's compiler 13.0 won't accept that, it will only accept the empty
 * syntax,
