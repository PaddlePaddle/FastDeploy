 ...
 * specifically related to Eigen.
  * This file expands upon Core/util/Meta.h and adds support for C++11 specific
 * features.
  */
 ...
 *     typename gen_numeric_list_reversed<int, 5>::type
 * numeric_list<int, 4,3,2,1,0>
 *     typename gen_numeric_list_swapped_pair<int, 5,1,2>::type
 * numeric_list<int, 0,2,1,3,4>
 *     typename gen_numeric_list_repeated<int, 0, 5>::type
 * numeric_list<int, 0,0,0,0,0>
 */
...
struct h_apply_op {
  ...
  constexpr static
      typename h_apply_op_helper<from_left, op, additional_param,
                                 values...>::type helper(type_list<values...>) {
...
// have to hack it
// together in front... (13.0 doesn't work with array_prod/array_reduce/...
// anyway, but 13.1
// does...
template <typename... Ts>
EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE decltype(
    reduce<product_op, Ts...>::run((*((Ts*)0))...)) arg_prod(Ts... ts) {
  return reduce<product_op, Ts...>::run(ts...);
}

template <typename... Ts>
constexpr EIGEN_STRONG_INLINE decltype(
    reduce<sum_op, Ts...>::run((*((Ts*)0))...)) arg_sum(Ts... ts) {
