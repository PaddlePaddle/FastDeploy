...
/** \internal
  ...
  * \ingroup CXX11_TensorSymmetry_Module
  *
  * \brief Recursive template that adds powers of the first generator to the
...
/** \internal
  ...
  *
  * It accepts a boolean parameter that determines if the generator is
 * redundant,
...
struct dimino_add_generator {
   ...
   */
  typedef typename apply_op_from_right<Multiply, current_generator,
                                       elements>::type multiplied_elements;
 ...
 * elements
  *                       with each other.
  * \tparam Equality      The equality check operation that checks if two group
 * elements
