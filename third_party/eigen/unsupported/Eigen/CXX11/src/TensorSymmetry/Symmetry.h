 ...
 * at
  * compile time. It is useful if one wants to support arbitrary run-time
 * defineable
  * symmetries for tensors, but it is also instantiated if a symmetry group is
 * defined
  * at compile time that would be either too large for the compiler to
 * reasonably
  * generate (using templates to calculate this at compile time is very
 * inefficient)
  * or that the compiler could generate the group but that it wouldn't make
 * sense to
...
/** \internal
  ...
  * </dl>
  *
  * Note: Example compile time performance with g++-4.6 on an Intenl Core
 * i5-3470
  ...
  * the memory and CPU requirements become unreasonable. Thus we only
 * instantiate
...
#endif
    m_symmetry.template apply<internal::tensor_symmetry_assign_value<Tensor_>,
