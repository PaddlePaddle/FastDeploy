...
struct TensorEvaluator<const TensorAssignOp<LeftArgType, RightArgType>,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(
    ...
    // m_leftImpl.data() returns a non
    // null value), attempt to evaluate the rhs expression in place. Returns
    // true iff in place
    // evaluation isn't supported and the caller still needs to manually assign
