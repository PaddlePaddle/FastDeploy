...
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 1,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TgtPacket packet(Index index) const {
    ...
    // data. This
    // ensures that we don't compute the subexpression twice. We may however
    // load some
    // coefficients twice, but in practice this doesn't negatively impact
    // performance.
  ...
  packet(Index index) const {
    ...
    const bool Vectorizable =
        IsSameType
            ? TensorEvaluator<ArgType, Device>::PacketAccess
            : TensorEvaluator<ArgType, Device>::PacketAccess &
                  internal::type_casting_traits<SrcType,
                                                TargetType>::VectorizedCast;
