 ...
 public:
  ...
  EIGEN_STRONG_INLINE const Scalar& operator()(Index i0, Index i1,
    ...
    if (Options & RowMajor) {
      const Index index =
          i2 +
          m_storage.dimensions()[2] * (i1 + m_storage.dimensions()[1] * i0);
      return m_storage.data()[index];
    } else {
      const Index index =
          i0 +
          m_storage.dimensions()[0] * (i1 + m_storage.dimensions()[1] * i2);
      return m_storage.data()[index];
    }
  }
  EIGEN_DEVICE_FUNC
