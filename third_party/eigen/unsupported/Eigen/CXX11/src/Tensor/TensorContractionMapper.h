...
/// the device. For other platform the default make pointer is used which
...
class SimpleTensorContractionMapper {
 public:
  ...
  EIGEN_STRONG_INLINE Index computeIndex(Index row, Index col) const {
    ...
    EIGEN_UNUSED_VARIABLE(left);  // annoying bug in g++8.1:
...
class BaseTensorContractionMapper
 ...
 public:
  ...
  load(Index i, Index j) const {
    ...
    // On the right hand side we need to check if the contracting dimensions may
    // have
    // been shuffled first.
...
class TensorContractionSubMapper {
 public:
  ...
  EIGEN_DEVICE_FUNC TensorContractionSubMapper(const ParentMapper& base_mapper,
    ...
    // possible. This avoids the need to recompute
