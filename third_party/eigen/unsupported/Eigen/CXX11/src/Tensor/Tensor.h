 ...
 * have to worry about them.
  * \tparam Options_  A combination of either \b #RowMajor or \b #ColMajor, and
 * of either
 ...
 * required
  *                 for vectorization. It defaults to aligning tensors. Note
 * that tensors currently do not support any operations that profit from
 * vectorization.
  *                 Support for such operations (i.e. adding two tensors etc.)
 * is planned.
  *
 ...
 * hierarchy as Eigen uses for matrices, so that
  * taking blocks or using tensors in expressions is easily possible, including
 * an interface with the vector/matrix code
  * by providing .asMatrix() and .asVector() (or similar) methods for rank 2 and
 * 1 tensors. However, currently, the %Tensor
  * class does not provide any of these features and is only available as a
 * stand-alone class that just allows for
  * coefficient access. Also, when fixed-size tensors are implemented, the
 * number of template arguments is likely to
