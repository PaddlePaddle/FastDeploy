...
/// It is added due to the fact that for our device compiler `T*` is not
/// allowed.
/// If we wanted to use the same Evaluator functions we have to convert that
/// type to our pointer `T`.
/// This is done through our `MakePointer_` class. By default the Type in the
/// `MakePointer_<T>` is `T*` .
/// Therefore, by adding the default value, we managed to convert the type and
...
class TensorMap
 ...
 public:
  ...
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr,
                                Index firstDimension)
      : m_data(dataPtr), m_dimensions(firstDimension) {
    // The number of dimensions used to construct a tensor must be equal to the
    // rank of the tensor.
  ...
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr, Index dim1,
                                Index dim2)
      : m_data(dataPtr), m_dimensions(dim1, dim2) {
    EIGEN_STATIC_ASSERT(2 == NumIndices || NumIndices == Dynamic,
                        YOU_MADE_A_PROGRAMMING_MISTAKE)
  }
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr, Index dim1,
                                Index dim2, Index dim3)
      : m_data(dataPtr), m_dimensions(dim1, dim2, dim3) {
    EIGEN_STATIC_ASSERT(3 == NumIndices || NumIndices == Dynamic,
                        YOU_MADE_A_PROGRAMMING_MISTAKE)
  }
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr, Index dim1,
                                Index dim2, Index dim3, Index dim4)
      : m_data(dataPtr), m_dimensions(dim1, dim2, dim3, dim4) {
    EIGEN_STATIC_ASSERT(4 == NumIndices || NumIndices == Dynamic,
                        YOU_MADE_A_PROGRAMMING_MISTAKE)
  }
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr, Index dim1,
                                Index dim2, Index dim3, Index dim4, Index dim5)
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorMap(
      StoragePointerType dataPtr, const array<Index, NumIndices>& dimensions)
