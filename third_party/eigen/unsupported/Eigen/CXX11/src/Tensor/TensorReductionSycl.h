 ...
 *approach
 * is used since the GPU does not have Global Synchronization for global memory
 *among
 * different work-group/thread block. To solve the problem, we need to create
 *two kernels
 * to reduce the data, where the first kernel reduce the data locally and each
 *local
 * workgroup/thread-block save the input data into global memory. In the second
 *phase (global reduction)
 * one work-group uses one work-group/thread-block to reduces the intermediate
 *data into one single element.
 * Here is an NVIDIA presentation explaining the optimized two phase reduction
 *algorithm on GPU:
...
struct GenericReducer<Self, Op, Eigen::SyclDevice> {
  ...
  static bool run(const Self &self, const Op &reducer,
    ...
    dev.template unary_kernel_launcher<
        ...
        self, output, cl::sycl::nd_range<1>(cl::sycl::range<1>(GRange),
