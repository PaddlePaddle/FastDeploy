...
class UniformRandomGenerator {
 public:
  ...
  UniformRandomGenerator(uint64_t seed = 0) {
    ...
    // GPU).
    // Thus for CUDA (((CLOCK  + global_thread_id)* 6364136223846793005ULL) +
    // 0xda3e39cb94b95bdbULL) is passed to each thread
    // but for SYCL ((CLOCK * 6364136223846793005ULL) + 0xda3e39cb94b95bdbULL)
    // is passed to each thread and each thread adds
    // the  (global_thread_id* 6364136223846793005ULL) for itself only once, in
    // order to complete the construction
    // similar to CUDA Therefore, the thread Id injection is not available at
    // this stage.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T operator()(Index i) const {
    ...
    if (!m_exec_once) {
      ...
      // The (i * 6364136223846793005ULL) is the remaining part of the
      // PCG_XSH_RS_state on the GPU side
...
class NormalRandomGenerator {
 public:
  ...
  NormalRandomGenerator(uint64_t seed = 0) {
    ...
    // and only  available on the Operator() function (which is called on the
    // GPU).
    // Therefore, the thread Id injection is not available at this stage.
