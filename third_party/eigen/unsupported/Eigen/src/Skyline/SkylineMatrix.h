 ...
 public:
  ...
  inline void finalize() {
    if (IsRowMajor) {
      ...
      //                nbLowerElts << std::endl;
      //                memcpy(newArray + dataIdx, m_data.m_lower +
      //                m_rowStartIndex[row], nbLowerElts * sizeof (Scalar));
      //                m_rowStartIndex[row] = dataIdx;
      //                dataIdx += nbLowerElts;
      //
      //                const Index nbUpperElts = m_colStartIndex[row + 1] -
      //                m_colStartIndex[row];
      //                memcpy(newArray + dataIdx, m_data.m_upper +
      //                m_colStartIndex[row], nbUpperElts * sizeof (Scalar));
      ...
      //            accessor from SkylineMatrix
      //            m_rowStartIndex[rows()] = m_rowStartIndex[rows()-1] +
      //            m_data.lowerProfile(rows()-1);
...
class SkylineMatrix<Scalar, _Options>::InnerUpperIterator {
 public:
  ...
  inline Index index() const {
    return IsRowMajor
               ? m_outer - m_matrix.m_data.upperProfile(m_outer) +
                     (m_id - m_start)
...
class SkylineMatrix<Scalar, _Options>::InnerLowerIterator {
 public:
  ...
  inline Index index() const {
    return IsRowMajor
               ? m_outer - m_matrix.m_data.lowerProfile(m_outer) +
                     (m_id - m_start)
               : m_outer + (m_id - m_start) + 1;
    ;
  }
