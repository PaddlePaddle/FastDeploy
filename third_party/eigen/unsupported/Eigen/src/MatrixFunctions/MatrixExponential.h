...
struct matrix_exp_computeUV {
  /** \brief Compute Pad&eacute; approximant to the exponential.
    *
    * Computes \c U, \c V and \c squarings such that \f$ (V+U)(V-U)^{-1} \f$ is
   * a Pad&eacute;
    * approximant of \f$ \exp(2^{-\mbox{squarings}}M) \f$ around \f$ M = 0 \f$,
   * where \f$ M \f$
    * denotes the matrix \c arg. The degree of the Pad&eacute; approximant and
   * the value of squarings
    * are chosen such that the approximation error is no more than the round-off
   * error.
    */
 ...
 * or evaluated for
  * some other reason (so the argument should not be changed in the meantime).
