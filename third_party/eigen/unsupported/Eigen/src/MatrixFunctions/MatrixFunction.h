...
/** \brief Partition eigenvalues in clusters of ei'vals close to each other
  ...
  * # Any eigenvalue in a certain cluster is at most
 * matrix_function_separation() away from another eigenvalue
  *   in the same cluster.
  * # The distance between two eigenvalues in different clusters is more than
 ...
 * of \p T (which should be
  * upper triangular), with the blocking given by \p blockStart and \p
 * clusterSize. The matrix function of
  * each diagonal block is computed by \p atomic. The off-diagonal parts of \p
 * fT are set to zero.
  */
 ...
 * (otherwise the Sylvester equation
  * does not have a unique solution). In that case, these equations can be
 ...
 * applied to the triangular
  * matrix \p T. It assumes that the block diagonal part of \p fT has already
 * been computed. The part below
 ...
 * functions. The spectrum of the
  * matrix is divided in clustered of eigenvalues that lies close together. This
 * class delegates the
  * computation of the matrix function on every block corresponding to these
 * clusters to an object of type
  * \p AtomicType and uses these results to compute the matrix function of the
 * whole matrix. The class
  * \p AtomicType should have a \p compute() member function for computing the
 * matrix function of a block.
  *
 ...
 * function of that matrix, and then
 ...
 * evaluated for some other
  * reason (so the argument should not be changed in the meantime). It is the
 * return type of
  * matrixBase::matrixFunction() and related functions and most of the time this
