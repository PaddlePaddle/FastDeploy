 ...
 * value semantics.  Either bounding_box(_Object) must
 *                 be defined and return an AlignedBox<_Scalar, _Dim> or
 * bounding boxes must be provided to the tree initializer.
 *
 *  This class provides a simple (as opposed to optimized) implementation of a
 * bounding volume hierarchy analogous to a Kd-tree.
 *  Given a sequence of objects, it computes their bounding boxes, constructs a
 * Kd-tree of their centers
 *  and builds a BVH with the structure of that Kd-tree.  When the elements of
 * the tree are too expensive to be copied around,
...
class KdBVH {
 public:
  ...
  EIGEN_STRONG_INLINE void getChildren(
      Index index, VolumeIterator &outVBegin, VolumeIterator &outVEnd,
      ObjectIterator &outOBegin,
      ObjectIterator &outOEnd) const {  // inlining this function should open
                                        // lots of optimization opportunities to
                                        // the compiler
    if (index < 0) {
      outVBegin = outVEnd;
 ...
 private:
  ...
  // including objects[to]).
  // This routine partitions the objCenters in [from, to) along the dimension
  // dim, recursively constructs
  ...
  std::vector<int> children;  // children of x are children[2x] and
                              // children[2x+1], indices bigger than
