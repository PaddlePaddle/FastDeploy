 ...
 * represented
  *  by Euler angles, but there is no single representation (e.g. unlike
 * rotation matrices).
  * Therefore, you can convert from Eigen rotation and to them
  *  (including rotation matrices, which is not called "rotations" by Eigen
 * design).
  *
 ...
 * representation
  ...
  *  but it's not the only general reason for not having a single
 * representation.
  *
  * When converting rotation to EulerAngles, this class convert it to specific
 * ranges
  * When converting some rotation to EulerAngles, the rules for ranges are as
 * follow:
...
class EulerAngles : public RotationBase<EulerAngles<_Scalar, _System>, 3> {
 ...
 public:
   ...
   * [-PI/2, PI/2].
    *   - otherwise:
    *     - If the beta axis is positive, the beta angle will be in the range
   * [0, PI]
    *     - If the beta axis is negative, the beta angle will be in the range
   * [-PI, 0]
   */
  ...
  /** Constructs and initialize Euler angles from a rotation \p rot.
    *
    * \note If \p rot is an EulerAngles (even when it represented as
   * RotationBase explicitly),
  ...
  */
  ...
  /** Set \c *this from a rotation.
    *
    * See EulerAngles(const RotationBase<Derived, 3>&) for more information
   * about
    *  angles ranges output.
  */
  template <typename Derived>
