...
/** \ingroup SparseCore_Module
  ...
  *
  * \brief A versatile sparse matrix representation where each element is a
 * block
  *
 ...
 * blocks
  * in a row (resp. a column) should have the same number of rows (resp. of
 * columns).
  * It is thus required in this case to describe the layout of the matrix by
 * calling
  * setBlockLayout(rowBlocks, colBlocks).
  *
  * In any of the previous case, the matrix can be filled by calling
 * setFromTriplets().
  * A regular sparse matrix can be converted to a block sparse matrix and vice
 * versa.
  * It is obviously required to describe the block layout beforehand by calling
 * either
  * setBlockSize() for fixed-size blocks or setBlockLayout for variable-size
 * blocks.
  *
...
class BlockSparseMatrix
 ...
 protected:
  ...
  Scalar* m_values;  // Values stored block column after block column (size
                     // m_nonzeros)
  StorageIndex* m_blockPtr;  // Pointer to the beginning of each block in
                             // m_values, size m_nonzeroblocks ... null for
                             // fixed-size blocks
