...
class ArpackGeneralizedSelfAdjointEigenSolver {
 public:
   ...
   * \param[in] eigs_sigma String containing either "LM", "SM", "LA", or "SA",
   * with
   ...
   * #EigenvaluesOnly.
   * \param[in] tol What tolerance to find the eigenvalues to. Default is 0,
   * which
   *    means machine precision.
   *
   * This constructor calls compute(const MatrixType&, const MatrixType&, Index,
   * string, int, RealScalar)
   * to compute the eigenvalues of the matrix \p A with respect to \p B. The
   * eigenvectors are computed if
   ...
   * RealScalar)
   * to compute the eigenvalues of the matrix \p A. The eigenvectors are
   * computed if
   * \p options equals #ComputeEigenvectors.
   *
   */
   ...
   * \p B using ARPACK.  The eigenvalues()
...
ArpackGeneralizedSelfAdjointEigenSolver<MatrixType, MatrixSolver,
  ...
  // initial
  // residual vector is used, any other values means resid contains the initial
  // residual
