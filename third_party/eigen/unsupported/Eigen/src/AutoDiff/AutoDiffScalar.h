 ...
 * base scalar type
  *                 as well as the number of derivatives to compute are
 * determined from this type.
  *                 Typical choices include, e.g., \c Vector4f for 4
 * derivatives, or \c VectorXf
  *                 if the number of derivatives is not known at compile time,
 * and/or, the number
  *                 of derivatives is large.
  *                 Note that _DerType can also be a reference (e.g., \c
 * VectorXf&) to wrap a
  *                 existing vector into an AutoDiffScalar.
  *                 Finally, _DerType can also be any Eigen compatible
 * expression.
  *
  * This class represents a scalar value while tracking its respective
 * derivatives using Eigen's expression
  * template mechanism.
  *
 ...
 * However,
  * in that case, the expression template mechanism only occurs at the top
 * Matrix level,
      ...
      _DerType, !internal::is_same<
                    ...
                    typename NumTraits<
                        typename internal::traits<typename internal::remove_all<
                            _DerType>::type>::Scalar>::Real>::value>
      Base;
