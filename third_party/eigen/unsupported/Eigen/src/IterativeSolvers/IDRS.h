...
/**     \internal Low-level Induced Dimension Reduction algoritm
   ...
   of performed iterations.
        \param relres On input the tolerance error, on output an estimation of
   the relative error.
        \param S On input Number of the dimension of the shadow space.
                \param smoothing switches residual smoothing on.
                \param angle small omega lead to faster convergence at the
   expense of numerical stability
                \param replacement switches on a residual replacement strategy
   to increase accuracy of residual at the expense of more Mat*vec products
        \return false in the case of numerical issue, for example a break down
   of IDRS.
*/
...
bool idrs(const MatrixType& A, const Rhs& b, Dest& x,
  ...
  // of a
  // finite precision number that is so large that the absolute round-off error
  // in
  ...
  // mp is epsilon/C
  // 10^3 * eps is very conservative, so normally no residual replacements will
  // take place.
  // It only happens if things go very wrong. Too many restarts may ruin the
  // convergence.
...
/** \ingroup IterativeLinearSolvers_Module
  * \brief The Induced Dimension Reduction method (IDR(s)) is a
 * short-recurrences Krylov method for sparse square problems.
  *
  * This class allows to solve for A.x = b sparse linear problems. The vectors x
 * and b can be either dense or sparse.
  * he Induced Dimension Reduction method, IDR(), is a robust and efficient
 * short-recurrence Krylov subspace method for
 ...
 * Additionally, IDR(S) can handle matrices
 ...
 * larger values of s). And if both methods
  * converge the convergence for IDR(s) is typically much faster for difficult
 * systems (for example indefinite problems).
  *
  * IDR(s) is a limited memory finite termination method. In exact arithmetic it
 * converges in at most N+N/s iterations,
  * with N the system size.  It uses a fixed number of 4+3s vector. In
 * comparison, BiCGSTAB terminates in 2N iterations
  * and uses 7 vectors. GMRES terminates in at most N iterations, and uses I+3
 * vectors, with I the number of iterations.
  * Restarting GMRES limits the memory consumption, but destroys the finite
