...
int main(int argc, char *argv[]) {
  ...
  m4 = m4 *
       (m4 +
        m4);  // here Eigen chooses to first evaluate m4 + m4 into a temporary.
  // indeed, here it is an optimization to cache this intermediate result.
  m3 =
      m3 *
      m4.block<3, 3>(
          1, 1);  // here Eigen chooses NOT to evaluate block() into a temporary
  // because accessing coefficients of that block expression is not more costly
  // than accessing
