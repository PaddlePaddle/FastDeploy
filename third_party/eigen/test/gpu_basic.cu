...
struct complex_operators {
  ...
  void operator()(int i, const typename T::Scalar* in,
    ...
    int out_idx = i * (num_scalar_operators +
                       num_vector_operators * T::MaxSizeAtCompileTime);
    ...
    // block_idx += size;
    // res.segment(block_idx, size) = (x1 == x2.real() ? true_vector :
    // false_vector);
    // block_idx += size;
    // res.segment(block_idx, size) = (x1.real() == x2 ? true_vector :
    // false_vector);
    // block_idx += size;
    // res.segment(block_idx, size) = (x1 != x2 ? true_vector : false_vector);
    // block_idx += size;
...
bool verifyIsApproxWithInfsNans(
...
void test_with_infs_nans(const Kernel& ker, int n, const Input& in,
                         Output& out) {
...
EIGEN_DECLARE_TEST(gpu_basic) {
  ...
  //           base class 'internal::dense_xpr_base<Replicate<Array<float, 4, 1,
  //           0, 4, 1>, -1, -1> >::type'
  //           (aka 'ArrayBase<Eigen::Replicate<Eigen::Array<float, 4, 1, 0, 4,
  //           1>, -1, -1> >') has protected default constructor
  ...
  CALL_SUBTEST(run_and_compare_to_gpu(prod_test<Matrix3f, Matrix3f>(), nthreads,
                                      in, out));
  CALL_SUBTEST(run_and_compare_to_gpu(prod_test<Matrix4f, Vector4f>(), nthreads,
                                      in, out));

  CALL_SUBTEST(run_and_compare_to_gpu(diagonal<Matrix3f, Vector3f>(), nthreads,
                                      in, out));
  CALL_SUBTEST(run_and_compare_to_gpu(diagonal<Matrix4f, Vector4f>(), nthreads,
                                      in, out));
  ...
  CALL_SUBTEST(run_and_compare_to_gpu(eigenvalues_direct<Matrix3f>(), nthreads,
                                      in, out));
  CALL_SUBTEST(run_and_compare_to_gpu(eigenvalues_direct<Matrix2f>(), nthreads,
                                      in, out));

  // Test std::complex.
  CALL_SUBTEST(run_and_compare_to_gpu(complex_operators<Vector3cf>(), nthreads,
                                      cfin, cfout));
  CALL_SUBTEST(
