...
struct pcast_array<SrcPacket, TgtPacket, 2, 1> {
  static void cast(
      const typename internal::unpacket_traits<SrcPacket>::type* src,
...
struct test_cast_runner<SrcPacket, TgtScalar, TgtPacket, true, true> {
  static void run() {
    ...
    test_cast_runner<SrcPacket, TgtScalar, typename internal::unpacket_traits<
                                               TgtPacket>::half>::run();
  }
};
...
// the scalar path
// (for some compilers) compute the bitwise and with 0x1 of the results to keep
// the value in [0,1].
template <>
...
void packetmath() {
  ...
  if (PacketSize == 8 &&
      internal::unpacket_traits<
          typename internal::unpacket_traits<Packet>::half>::size ==
...
void packetmath_real() {
  ...
  if (PacketTraits::HasExp) {
    ...
    for (int j = 0; j < 3; ++j) {
      data1[0] =
          Scalar(std::ldexp(1, std::numeric_limits<Scalar>::min_exponent - j));
      CHECK_CWISE1_BYREF1_IF(PacketTraits::HasExp, REF_FREXP, internal::pfrexp);
      data1[0] = -data1[0];
      CHECK_CWISE1_BYREF1_IF(PacketTraits::HasExp, REF_FREXP, internal::pfrexp);
    }
  ...
  if (PacketTraits::HasExp) {
    ...
    data1[0] = Scalar(
        std::ldexp(Scalar(1.0), std::numeric_limits<Scalar>::min_exponent - 1));
    ...
    data1[0] = Scalar(
        std::ldexp(Scalar(1.0), std::numeric_limits<Scalar>::max_exponent - 1));
    data1[PacketSize] = Scalar(+std::numeric_limits<Scalar>::min_exponent -
                               std::numeric_limits<Scalar>::max_exponent);
    CHECK_CWISE2_IF(PacketTraits::HasExp, REF_LDEXP, internal::pldexp);
  }
...
// Note: 32-bit arm always flushes denorms to zero.
    ...
    if (PacketTraits::HasCos && !internal::is_same<Scalar, half>::value &&
      ...
      for (Scalar k = Scalar(1);
           k < Scalar(10000) / std::numeric_limits<Scalar>::epsilon();
           k *= Scalar(2)) {
        for (int k1 = 0; k1 <= 1; ++k1) {
          ...
          VERIFY_IS_APPROX(
              numext::abs2(data2[0]) + numext::abs2(data2[PacketSize + 0]),
              Scalar(1));
          VERIFY_IS_APPROX(
              numext::abs2(data2[1]) + numext::abs2(data2[PacketSize + 1]),
              Scalar(1));
        }
      }
...
void packetmath_notcomplex() {
...

  {
    ...
    // 0xff;
    // VERIFY(internal::predux_all(internal::pload<Packet>(data1)) &&
    // "internal::predux_all(1111)");
    // for(int k=0; k<PacketSize; ++k)
    // {
    //   for (unsigned int i=0; i<sizeof(Scalar); ++i)
    //   data1_bits[k*sizeof(Scalar)+i] = 0x0;
    //   VERIFY( (!internal::predux_all(internal::pload<Packet>(data1))) &&
    //   "internal::predux_all(0101)");
    //   for (unsigned int i=0; i<sizeof(Scalar); ++i)
  ...
  if (!NumTraits<Scalar>::IsInteger) {
    ...
    for (int i = 0; i < PacketSize; ++i) {
      data1[i] = internal::random<bool>()
                     ? std::numeric_limits<Scalar>::quiet_NaN()
                     : Scalar(0);
      data1[i + PacketSize] = internal::random<bool>()
                                  ? std::numeric_limits<Scalar>::quiet_NaN()
                                  : Scalar(0);
    }
