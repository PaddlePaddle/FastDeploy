...
void quaternion(void) {
  ...
  */
  ...
  VERIFY((q2 * q1).isApprox(q1 * q2, largeEps) ||
         !(q2 * q1 *
           v2).isApprox(q1.toRotationMatrix() * q2.toRotationMatrix() * v2));
...
void mapQuaternion(void) {
  ...
  // LValueBit flag
  // is used to determine whether we can return a coeff by reference or not,
  // which is not enough for Map<const ...>.
