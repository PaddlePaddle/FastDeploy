...
void check_symbolic_index() {
  ...
  VERIFY(is_same_type((fix<5>() - fix<3>()) * fix<9>() / (-fix<3>()),
                      fix<-(5 - 3) * 9 / 3>()));
  VERIFY(is_same_type((fix<5>() - fix<3>()) * fix<9>() / fix<2>(),
                      fix<(5 - 3) * 9 / 2>()));
  ...
  VERIFY_IS_EQUAL(((last * 5 - 2) / 3).eval(last = size - 1),
                  ((size - 1) * 5 - 2) / 3);
  VERIFY_IS_EQUAL(((last * fix<5> - fix<2>) / fix<3>).eval(last = size - 1),
                  ((size - 1) * 5 - 2) / 3);
  VERIFY_IS_EQUAL((-last * lastp1).eval(last = size - 1), -(size - 1) * size);
  VERIFY_IS_EQUAL((lastp1 - 3 * last).eval(last = size - 1),
                  size - 3 * (size - 1));
  VERIFY_IS_EQUAL(((lastp1 - 3 * last) / lastp1).eval(last = size - 1),
                  (size - 3 * (size - 1)) / size);
...
#if EIGEN_HAS_CXX14
  {
    ...
    VERIFY_IS_APPROX(int(((x + 3) / y + z).eval(x = 6, y = 3, z = -13)),
                     (6 + 3) / 3 + (-13));
  }
#endif
}
