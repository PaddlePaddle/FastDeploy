...
void product_extra(const MatrixType& m) {
  ...
  // test negative strides
  {
    ...
    VERIFY_IS_APPROX(
        m3.noalias() = MatrixType(map1) * MatrixType(map2).adjoint(),
...
void bug_127() {
  ...
  // dot product because Product.h was using the
  // max-sizes to detect size 1 indicating vectors, and that didn't account for
  // 0-sized object with max-size 1.
...
void unaligned_objects() {
  ...
  // packets and then reassemble then.
  // There was a mistake in the computation of the valid range for fully
  // unaligned objects: in some rare cases,
  // memory was read outside the allocated matrix memory. Though the values were
  // not used, this might raise segfault.
