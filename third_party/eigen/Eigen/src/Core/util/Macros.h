...
// attribute
// value to 1024. The compiler assigns a default value of 256 when the attribute
// is not
// specified. This results in failures on the HIP platform, for cases when a GPU
// kernel
// without an explicit launch_bounds attribute is called with a
// threads_per_block value
// greater than 256.
//
// This is a regression in functioanlity and is expected to be fixed within the
// next
// couple of ROCm releases (compiler will go back to using 1024 value as the
// default)
//
...
// in HIP kernels
// EIGEN_CUDACC implies the CUDA compiler and is used to tweak Eigen code for
// use in CUDA kernels
//
// In most cases the same tweaks are required to the Eigen code to enable in
// both the HIP and CUDA kernels.
...
// In most cases we want to check if both macros are defined which can be done
// using the define below.
