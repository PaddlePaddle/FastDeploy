...
EIGEN_DEVICE_FUNC inline Packet pset(
    const typename unpacket_traits<Packet>::type (&a)[N] /* a */) {
  EIGEN_STATIC_ASSERT(unpacket_traits<Packet>::size == N,
...
// [numeric_limits<Scalar>::min_exponent-2,
// numeric_limits<Scalar>::max_exponent]
//
...
// The coefficients are: 0xbfc90f80, 0xb7354480, 0x2e74b9ee
  ...
  //    accuracy
  //    c = (A'*diag(w)*A)\(A'*diag(w)*(sin(x)-x)); # weighted LS, linear coeff
  //    forced to 1
  //    printf('%.64f\n %.64f\n%.64f\n', c(3), c(2), c(1))
  //
...
psqrt_complex(const Packet& a) {
  ...
  // equation for
  // a single complex square root of the number x + i*y. We want to find real
  // numbers
  // u and v such that
  //    (u + i*v)^2 = x + i*y  <=>
...
struct fast_accurate_exp2<float> {
  ...
  EIGEN_STRONG_INLINE Packet operator()(const Packet& x) {
    ...
    // evaluated in
    // single precision, and the remaining steps are evaluated with extra
    // precision using
    // double word arithmetic. C is an extra precise constant stored as a double
    // word.
    //
    // The polynomial coefficients were calculated using Sollya commands:
    // > n = 6;
