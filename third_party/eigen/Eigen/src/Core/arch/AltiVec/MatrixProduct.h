...
/**************************************************************************************************
 * TODO *
 * - Check StorageOrder on lhs_pack (the innermost second loop seems
...
// imaginary vector, this turned out
// to be faster than Eigen's usual approach of having real/imaginary pairs on a
// single vector. This constants then
// are responsible to extract from convert between Eigen's and MatrixProduct
// approach.
const static Packet4f p4f_CONJUGATE = {float(-1.0), float(-1.0), float(-1.0),
                                       float(-1.0)};
 ...
 *the packing leaves
 * the diagonal real, whatever is below it is copied from the respective upper
 ...
 *to be read by gemm using
 * it's respective rank-update instructions. The float32/64 versions are
 *different because at this moment
 * the size of the accumulator is fixed at 512-bits so you can't have a 4x4
 *accumulator of 64-bit elements.
 *
 * As mentioned earlier MatrixProduct breaks complex numbers into a real vector
 *and a complex vector so packing has
 * to take that into account, at the moment, we run pack the real part and then
 *the imaginary part, this is the main
 * reason why packing for complex is broken down into several different parts,
 *also the reason why we endup having a
 * float32/64 and complex float32/64 version.
 **/
template <typename Scalar, typename Index, int StorageOrder>
EIGEN_STRONG_INLINE std::complex<Scalar> getAdjointVal(
    Index i, Index j,
 ...
 * on special occasions it fills part of block with other parts of the matrix.
 *Two variables control
 * how PanelMode should behave: offset and stride. The idea is that those
 *variables represent whatever
 * is going to be the real offset and stride in the future and this is what you
 *should obey. The process
 * is to behave as you would with normal packing but leave the start of each
 *part with the correct offset
 * and the end as well respecting the real stride the block will have. Gebp is
 *aware of both blocks stride
 * and offset and behaves accordingly.
 **/

// General template for lhs complex packing.
template <typename Scalar, bool IsComplex, typename Index, typename DataMapper,
...
template <typename Scalar, typename Index, typename DataMapper, typename Packet,
          int StorageOrder, bool PanelMode>
struct lhs_pack {
...
struct rhs_cpack {
  EIGEN_STRONG_INLINE void operator()(std::complex<Scalar>* blockB,
...
template <typename Scalar, typename Index, typename DataMapper, typename Packet,
          int StorageOrder, bool PanelMode>
struct rhs_pack {
  EIGEN_STRONG_INLINE void operator()(Scalar* blockB, const DataMapper& rhs,
...
template <bool IsComplex, typename Index, typename DataMapper, typename Packet,
          typename PacketC, int StorageOrder, bool Conjugate, bool PanelMode>
struct lhs_cpack<double, IsComplex, Index, DataMapper, Packet, PacketC,
    ...
    for (j = 0; j + vectorSize <= rows; j += vectorSize) {
      ...
      for (; i + vectorSize <= depth; i += vectorSize) {
        ...
        if (StorageOrder == ColMajor) {
          ...
          block.packet[0] = vec_perm(cblock.packet[0].v, cblock.packet[2].v,
                                     p16uc_GETREAL64);  //[a1 a2]
          block.packet[1] = vec_perm(cblock.packet[1].v, cblock.packet[3].v,
          ...
          block.packet[0] = vec_perm(cblock.packet[0].v, cblock.packet[1].v,
                                     p16uc_GETREAL64);  //[a1 a2]
          block.packet[1] = vec_perm(cblock.packet[2].v, cblock.packet[3].v,
                                     p16uc_GETREAL64);  //[b1 b2]
        }
...
template <typename Index, typename DataMapper, typename Packet,
          typename PacketC, int StorageOrder, bool Conjugate, bool PanelMode>
struct rhs_cpack<double, Index, DataMapper, Packet, PacketC, StorageOrder,
                 Conjugate, PanelMode> {
  EIGEN_STRONG_INLINE void operator()(std::complex<double>* blockB,
    ...
    for (; j + 2 * vectorSize <= cols; j += 2 * vectorSize) {
      ...
      for (; i < depth; i++) {
        ...
        cblock.packet[0] =
            rhs.template loadPacket<PacketC>(i, j + 0);  //[a1 a1i]
        cblock.packet[1] =
            rhs.template loadPacket<PacketC>(i, j + 1);  //[b1 b1i]
        cblock.packet[2] =
            rhs.template loadPacket<PacketC>(i, j + 2);  //[c1 c1i]
        cblock.packet[3] =
            rhs.template loadPacket<PacketC>(i, j + 3);  //[d1 d1i]
...
EIGEN_STRONG_INLINE void pger(PacketBlock<Packet, 4>* acc, const Scalar* lhs,
                              const Packet* rhsV) {
  asm("#pger begin");
...
template <typename Scalar, typename Packet, bool NegativeAccumulate>
EIGEN_STRONG_INLINE void pger(PacketBlock<Packet, 1>* acc, const Scalar* lhs,
                              const Packet* rhsV) {
  Packet lhsV = pload<Packet>(lhs);
...
// real.
template <typename Scalar, typename Packet, bool ConjugateLhs,
          bool ConjugateRhs, bool LhsIsReal, bool RhsIsReal>
EIGEN_STRONG_INLINE void pgerc(PacketBlock<Packet, 4>& accReal,
                               PacketBlock<Packet, 4>& accImag,
                               const Scalar* rhs_ptr,
                               const Scalar* rhs_ptr_imag,
                               const Scalar* lhs_ptr,
                               const Scalar* lhs_ptr_imag, Packet& conj) {
  ...
  if (LhsIsReal) {
    ...
    accReal.packet[0] =
        psub<Packet>(accReal.packet[0], pmul<Packet>(rhsV1i, lhsVi));
    accReal.packet[1] =
        psub<Packet>(accReal.packet[1], pmul<Packet>(rhsV2i, lhsVi));
    accReal.packet[2] =
        psub<Packet>(accReal.packet[2], pmul<Packet>(rhsV3i, lhsVi));
    accReal.packet[3] =
        psub<Packet>(accReal.packet[3], pmul<Packet>(rhsV4i, lhsVi));
...
EIGEN_STRONG_INLINE void bscale(PacketBlock<Packet, 4>& acc,
...
template <typename Packet>
EIGEN_STRONG_INLINE void bscale(PacketBlock<Packet, 1>& acc,
...
template <typename Packet>
EIGEN_STRONG_INLINE void bscalec(PacketBlock<Packet, 4>& aReal,
  ...
  cReal.packet[0] =
      psub<Packet>(cReal.packet[0], pmul<Packet>(aImag.packet[0], bImag));
  cReal.packet[1] =
      psub<Packet>(cReal.packet[1], pmul<Packet>(aImag.packet[1], bImag));
  cReal.packet[2] =
      psub<Packet>(cReal.packet[2], pmul<Packet>(aImag.packet[2], bImag));
  cReal.packet[3] =
      psub<Packet>(cReal.packet[3], pmul<Packet>(aImag.packet[3], bImag));
...
EIGEN_STRONG_INLINE void bload(PacketBlock<Packet, 4>& acc,
...
template <typename DataMapper, typename Packet, typename Index, int N>
EIGEN_STRONG_INLINE void bload(PacketBlock<Packet, 8>& acc,
                               const DataMapper& res, Index row, Index col,
                               Index accCols) {
  acc.packet[0] = res.template loadPacket<Packet>(row + N * accCols, col + 0);
  acc.packet[1] = res.template loadPacket<Packet>(row + N * accCols, col + 1);
...
template <typename Scalar, typename Packet, typename Index>
EIGEN_STRONG_INLINE void MICRO_EXTRA_COL(const Scalar*& lhs_ptr,
                                         const Scalar*& rhs_ptr,
...
EIGEN_STRONG_INLINE void MICRO_EXTRA_ROW(const Scalar*& lhs_ptr,
                                         const Scalar*& rhs_ptr,
...
EIGEN_STRONG_INLINE void gemm_extra_row(
    const DataMapper& res, const Scalar* lhs_base, const Scalar* rhs_base,
    Index depth, Index strideA, Index offsetA, Index row, Index col, Index cols,
  ...
  if (remaining_depth == depth) {
    ...
    for (; k < depth; k++) {
      Packet rhsV[4];
      pbroadcast4<Packet>(rhs_ptr, rhsV[0], rhsV[1], rhsV[2], rhsV[3]);
      pger<Scalar, Packet, Index, false>(&accZero, lhs_ptr, rhsV,
                                         remaining_rows);
      lhs_ptr += remaining_rows;
      rhs_ptr += accRows;
    }
...
#define MICRO_DST                                                         \
  PacketBlock<Packet, 4> *accZero0, PacketBlock<Packet, 4> *accZero1,     \
      PacketBlock<Packet, 4> *accZero2, PacketBlock<Packet, 4> *accZero3, \
      PacketBlock<Packet, 4> *accZero4, PacketBlock<Packet, 4> *accZero5, \
      PacketBlock<Packet, 4> *accZero6, PacketBlock<Packet, 4> *accZero7

#define MICRO_COL_DST                                                     \
  PacketBlock<Packet, 1> *accZero0, PacketBlock<Packet, 1> *accZero1,     \
      PacketBlock<Packet, 1> *accZero2, PacketBlock<Packet, 1> *accZero3, \
      PacketBlock<Packet, 1> *accZero4, PacketBlock<Packet, 1> *accZero5, \
...
#define MICRO_SRC                                                            \
  const Scalar **lhs_ptr0, const Scalar **lhs_ptr1, const Scalar **lhs_ptr2, \
      const Scalar **lhs_ptr3, const Scalar **lhs_ptr4,                      \
      const Scalar **lhs_ptr5, const Scalar **lhs_ptr6,                      \
      const Scalar **lhs_ptr7

#define MICRO_ONE                                                       \
  MICRO<unroll_factor, Scalar, Packet, accRows, accCols>(               \
      &lhs_ptr0, &lhs_ptr1, &lhs_ptr2, &lhs_ptr3, &lhs_ptr4, &lhs_ptr5, \
      &lhs_ptr6, &lhs_ptr7, rhs_ptr, &accZero0, &accZero1, &accZero2,   \
      &accZero3, &accZero4, &accZero5, &accZero6, &accZero7);

#define MICRO_COL_ONE                                                   \
  MICRO_COL<unroll_factor, Scalar, Packet, Index, accCols>(             \
      &lhs_ptr0, &lhs_ptr1, &lhs_ptr2, &lhs_ptr3, &lhs_ptr4, &lhs_ptr5, \
      &lhs_ptr6, &lhs_ptr7, rhs_ptr, &accZero0, &accZero1, &accZero2,   \
      &accZero3, &accZero4, &accZero5, &accZero6, &accZero7, remaining_cols);
...
template <int N, typename Scalar, typename Packet, const Index accRows,
          const Index accCols>
EIGEN_STRONG_INLINE void MICRO(MICRO_SRC, const Scalar*& rhs_ptr, MICRO_DST) {
  Packet rhsV[4];
  pbroadcast4<Packet>(rhs_ptr, rhsV[0], rhsV[1], rhsV[2], rhsV[3]);
...
EIGEN_STRONG_INLINE void gemm_unrolled_iteration(
    ...
    const Packet& pAlpha) {
  const Scalar* rhs_ptr = rhs_base;
  const Scalar *lhs_ptr0, *lhs_ptr1, *lhs_ptr2, *lhs_ptr3, *lhs_ptr4, *lhs_ptr5,
      *lhs_ptr6, *lhs_ptr7;
...
template <int N, typename Scalar, typename Packet, typename Index,
          const Index accCols>
EIGEN_STRONG_INLINE void MICRO_COL(MICRO_SRC, const Scalar*& rhs_ptr,
                                   MICRO_COL_DST, Index remaining_rows) {
  Packet rhsV[1];
  rhsV[0] = pset1<Packet>(rhs_ptr[0]);
  asm("#unrolled pger? begin");
  MICRO_WORK
...
#undef MAX_UNROLL
    ...
    if (remaining_rows > 0) {
      gemm_extra_row<Scalar, Packet, DataMapper, Index, accRows>(
          res, lhs_base, rhs_base, depth, strideA, offsetA, row, col, cols,
...
template <typename LhsScalar, typename RhsScalar, typename Scalarc,
          ...
          typename RhsPacket, typename DataMapper, const int accRows,
          const int accCols, bool ConjugateLhs, bool ConjugateRhs,
...
EIGEN_STRONG_INLINE void gemm_complex(const DataMapper& res,
  ...
  for (; col + accRows <= cols; col += accRows) {
    const Scalar* rhs_base =
        blockB + ((advanceCols * col) / accRows) * strideB * accRows;
    const Scalar* lhs_base = blockA;

    Index row = 0;
    for (; row + accCols <= rows; row += accCols) {
#define MICRO()                                                            \
  pgerc<Scalar, Packet, ConjugateLhs, ConjugateRhs, LhsIsReal, RhsIsReal>( \
      accReal1, accImag1, rhs_ptr, rhs_ptr_imag, lhs_ptr1, lhs_ptr_imag1,  \
      conj);                                                               \
  lhs_ptr1 += accCols;                                                     \
  rhs_ptr += accRows;                                                      \
  if (!LhsIsReal) lhs_ptr_imag1 += accCols;                                \
  if (!RhsIsReal) rhs_ptr_imag += accRows;

      const Scalar* rhs_ptr = rhs_base;
      const Scalar* rhs_ptr_imag = rhs_ptr + accRows * strideB;
      const Scalar* lhs_ptr1 =
          lhs_base + ((advanceRows * row) / accCols) * strideA * accCols;
      ...
      if (!LhsIsReal) lhs_ptr_imag1 += accCols * offsetA;
      rhs_ptr += accRows * offsetB;
      if (!RhsIsReal) rhs_ptr_imag += accRows * offsetB;
      Index k = 0;
      for (; k + PEEL <= depth; k += PEEL) {
        prefetch(rhs_ptr);
      ...
      bscalec<Packet>(accReal1, accImag1, pAlphaReal, pAlphaImag, taccReal,
                      taccImag);
...
#undef MICRO
  ...
  if (remaining_cols > 0) {
    ...
    for (; row + accCols <= rows; row += accCols) {
      ...
      if (!LhsIsReal) lhs_ptr_imag += accCols * offsetA;
      rhs_ptr += remaining_cols * offsetB;
      if (!RhsIsReal) rhs_ptr_imag += remaining_cols * offsetB;
      ...
      for (Index k = 0; k < depth; k++) {
        for (Index arow = 0; arow < accCols; arow++) {
          ...
          for (int acol = 0; acol < remaining_cols; acol++) {
            ...
            if (!RhsIsReal) {
              ...
              if (ConjugateRhs) rhs_imag *= -1;
            } else {
              rhs_imag = (Scalar)0;
            }

            scalarAcc[arow][acol].real(scalarAcc[arow][acol].real() +
                                       lhs_real * rhs_real -
                                       lhs_imag * rhs_imag);
            scalarAcc[arow][acol].imag(scalarAcc[arow][acol].imag() +
                                       lhs_imag * rhs_real +
                                       lhs_real * rhs_imag);
          }
        }
        ...
        if (!LhsIsReal) lhs_ptr_imag += accCols;
      }
...
operator()(std::complex<float>* blockA, const DataMapper& lhs, Index depth,
           Index rows, Index stride, Index offset) {
  lhs_cpack<float, true, Index, DataMapper, Packet4f, Packet2cf, RowMajor,
            Conjugate, PanelMode>
      pack;
  pack(blockA, lhs, depth, rows, stride, offset);
}
...
operator()(std::complex<float>* blockA, const DataMapper& lhs, Index depth,
           Index rows, Index stride, Index offset) {
  lhs_cpack<float, true, Index, DataMapper, Packet4f, Packet2cf, ColMajor,
            Conjugate, PanelMode>
      pack;
  pack(blockA, lhs, depth, rows, stride, offset);
}
...
void gemm_pack_rhs<std::complex<float>, Index, DataMapper, nr, ColMajor,
                   Conjugate, PanelMode>::
operator()(std::complex<float>* blockB, const DataMapper& rhs, Index depth,
           Index cols, Index stride, Index offset) {
  rhs_cpack<float, Index, DataMapper, Packet4f, Packet2cf, ColMajor, Conjugate,
            PanelMode>
      pack;
  pack(blockB, rhs, depth, cols, stride, offset);
}
...
operator()(std::complex<float>* blockB, const DataMapper& rhs, Index depth,
           Index cols, Index stride, Index offset) {
  rhs_cpack<float, Index, DataMapper, Packet4f, Packet2cf, RowMajor, Conjugate,
            PanelMode>
      pack;
  pack(blockB, rhs, depth, cols, stride, offset);
}
...
operator()(std::complex<double>* blockA, const DataMapper& lhs, Index depth,
           Index rows, Index stride, Index offset) {
  lhs_cpack<double, true, Index, DataMapper, Packet2d, Packet1cd, RowMajor,
            Conjugate, PanelMode>
      pack;
  pack(blockA, lhs, depth, rows, stride, offset);
}
...
operator()(std::complex<double>* blockA, const DataMapper& lhs, Index depth,
           Index rows, Index stride, Index offset) {
  lhs_cpack<double, true, Index, DataMapper, Packet2d, Packet1cd, ColMajor,
            Conjugate, PanelMode>
      pack;
  pack(blockA, lhs, depth, rows, stride, offset);
}
...
operator()(std::complex<double>* blockB, const DataMapper& rhs, Index depth,
           Index cols, Index stride, Index offset) {
  rhs_cpack<double, Index, DataMapper, Packet2d, Packet1cd, ColMajor, Conjugate,
            PanelMode>
      pack;
  pack(blockB, rhs, depth, cols, stride, offset);
}
...
operator()(std::complex<double>* blockB, const DataMapper& rhs, Index depth,
           Index cols, Index stride, Index offset) {
  rhs_cpack<double, Index, DataMapper, Packet2d, Packet1cd, RowMajor, Conjugate,
            PanelMode>
      pack;
  pack(blockB, rhs, depth, cols, stride, offset);
}
...
void gebp_kernel<std::complex<float>, std::complex<float>, Index, DataMapper,
                 mr, nr, ConjugateLhs, ConjugateRhs>::
operator()(const DataMapper& res, const std::complex<float>* blockA,
           const std::complex<float>* blockB, Index rows, Index depth,
           Index cols, std::complex<float> alpha, Index strideA, Index strideB,
           Index offsetA, Index offsetB) {
...
void gebp_kernel<float, std::complex<float>, Index, DataMapper, mr, nr,
                 ConjugateLhs, ConjugateRhs>::
operator()(const DataMapper& res, const float* blockA,
           const std::complex<float>* blockB, Index rows, Index depth,
           Index cols, std::complex<float> alpha, Index strideA, Index strideB,
           Index offsetA, Index offsetB) {
  const int accRows = quad_traits<float>::rows;
...
operator()(const DataMapper& res, const std::complex<float>* blockA,
           const float* blockB, Index rows, Index depth, Index cols,
           std::complex<float> alpha, Index strideA, Index strideB,
           Index offsetA, Index offsetB) {
  const int accRows = quad_traits<float>::rows;
  const int accCols = quad_traits<float>::size;
  void (*gemm_function)(const DataMapper&, const std::complex<float>*,
...
void gebp_kernel<std::complex<double>, std::complex<double>, Index, DataMapper,
                 mr, nr, ConjugateLhs, ConjugateRhs>::
operator()(const DataMapper& res, const std::complex<double>* blockA,
           const std::complex<double>* blockB, Index rows, Index depth,
           Index cols, std::complex<double> alpha, Index strideA, Index strideB,
           Index offsetA, Index offsetB) {
...
operator()(const DataMapper& res, const std::complex<double>* blockA,
           const double* blockB, Index rows, Index depth, Index cols,
           std::complex<double> alpha, Index strideA, Index strideB,
           Index offsetA, Index offsetB) {
  const int accRows = quad_traits<double>::rows;
  const int accCols = quad_traits<double>::size;
  void (*gemm_function)(const DataMapper&, const std::complex<double>*,
