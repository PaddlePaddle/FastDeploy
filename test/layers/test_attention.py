    ...
    def __init__(
        ...
        self.req_to_token_pool = type(
            ...
            (),
            {
                ...
                "req_to_token": paddle.zeros([max_batch_size, max_context_len], dtype=paddle.int32),
            },
        )
...
class TestNativePaddleAttentionBackend(unittest.TestCase):
    ...
    def _mock_write_to_req_to_token_pool(self, batch_size, seq_len, page_size):
        ...
        self.model_runner.req_to_token_pool.req_to_token[:batch_size, :seq_len] = self.req_to_token
    ...
    def _run_reference_forward(self, mode, q, k, v, layer, forward_batch, expected_shape):
    ...
    def _verify_output(self, output, expected_shape, output_ref=None):
        ...
        self.assertEqual(paddle.isnan(output).sum().item(), 0, "Output contains NaN values")
        ...
        if output_ref is not None:
            if not paddle.allclose(output, output_ref, atol=1e-1, rtol=0.0):
                ...
                diff_mask = ~paddle.isclose(output, output_ref, atol=1e-1, rtol=0.0)
                ...
                raise AssertionError("Attention output is not close to the torch native backend output")
    ...
    def _create_forward_batch(self, mode, q_len=None, prefix_len=0, page_size=1):
        ...
        if mode == ForwardMode.EXTEND:
            ...
            forward_batch = ForwardMeta(
                ...
                extend_prefix_lens=paddle.to_tensor([prefix_len] * self.batch_size),
                extend_seq_lens=paddle.to_tensor([q_len] * self.batch_size),
                seq_lens_cpu=paddle.to_tensor([total_len] * self.batch_size, place="cpu"),
                extend_prefix_lens_cpu=paddle.to_tensor([prefix_len] * self.batch_size, place="cpu"),
            ...
            if mode == ForwardMode.DECODE and page_size > 1:
                ...
                out_cache_start = (self.batch_size * self.seq_len // page_size + 1) * page_size
            ...
            forward_batch = ForwardMeta(
                batch_size=self.batch_size,
                input_ids=paddle.randint(0, 100, (self.batch_size, decode_len)),
                out_cache_loc=paddle.to_tensor([out_cache_start, out_cache_end]),
    ...
    def _run_attention_test(self, mode, q_len, prefix_len=0, page_size=1):
        ...
        output_ref = self._run_reference_forward(mode, q, k, v, layer, forward_batch, expected_shape)
    ...
    def test_forward_extend_with_prefix(self):
        ...
        self._run_attention_test(ForwardMode.EXTEND, q_len=extend_len, prefix_len=prefix_len)
